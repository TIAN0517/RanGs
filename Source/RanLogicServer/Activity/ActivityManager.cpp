...
void ActivityManager::AddPointsTest ()
{
	if ( !GLUseFeatures::GetInstance().IsUsingTestServer() ) return;

	if ( m_llRewardPoint[ACTIVITY_CLASS_LOW] < 50000 || m_llRewardPoint[ACTIVITY_CLASS_MEDIUM] < 50000 || m_llRewardPoint[ACTIVITY_CLASS_HIGH] < 50000 )
	{
		if ( m_llRewardPoint[ACTIVITY_CLASS_LOW] < 50000 )		AddUpRewardPoint ( 50000, ACTIVITY_CLASS_LOW );
		if ( m_llRewardPoint[ACTIVITY_CLASS_MEDIUM] < 50000 )	AddUpRewardPoint ( 50000, ACTIVITY_CLASS_MEDIUM );
		if ( m_llRewardPoint[ACTIVITY_CLASS_HIGH] < 50000 )		AddUpRewardPoint ( 50000, ACTIVITY_CLASS_HIGH );

		GLMSG::SNET_ACTIVITY_REWARD_POINT_SYNC msgPoint;
		msgPoint.nChaNum = m_pCharOwner->CharDbNum();
		msgPoint.llRewardPointHigh	= m_llRewardPoint[ACTIVITY_CLASS_HIGH];
		msgPoint.llRewardPointMedium= m_llRewardPoint[ACTIVITY_CLASS_MEDIUM];
		msgPoint.llRewardPointLow	= m_llRewardPoint[ACTIVITY_CLASS_LOW];
		m_pGaeaServer->SENDTOAGENT(&msgPoint);
	}

	if ( m_pCharOwner->m_nContributionPoint < 50000 )
	{
		m_pCharOwner->UpdateContributionPoint(50000, 50000);
		m_pCharOwner->SaveContributionPoint();
		m_pCharOwner->LoadExtraInfo();
	}
}
//
