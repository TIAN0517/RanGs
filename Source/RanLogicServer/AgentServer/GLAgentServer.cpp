...
HRESULT GLAgentServer::DropOutChar(DWORD GaeaID)
{
	...
	if ( pChar->m_sPMarket.IsPremiumMarket() && pChar->m_sPMarket.IsOpen() )
	{
		sc::writeLogInfo( "DropOutChar  IsPrivateMarketOpen IsOpen" );
		if ( !pChar->m_bOffVend )
		{
			sc::writeLogInfo( "DropOutChar  offline vend" );
