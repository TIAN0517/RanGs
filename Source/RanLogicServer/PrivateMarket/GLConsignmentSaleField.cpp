...
bool GLConsignmentSaleField::ReqItemAdd( NET_MSG_GENERIC* nmg, GLChar* pChar )
{
	...
	if ( private_market::GLPrivateMarketDefine::Instance()->GoldTrade() == false && RecvData.m_bySellType == (private_market::eSellType_ConsignmentSale_Gold || private_market::eSellType_ConsignmentSale_GoldBet)  ) 
		return false;
...
bool GLConsignmentSaleField::AckItemSendBackDB( NET_MSG_GENERIC* nmg )
{
    ...
    if( 0 < wSaleTurnCount )
    {
		...
		case private_market::eSellType_ConsignmentSale_RanPointBet:
            {
                ...
                pChar->PointShopPointAdd( 0, llAddPoint,0, pointshop::CONSIGNMENT_SALE );
            }
            break;
		case private_market::eSellType_ConsignmentSale_Gold:
		case private_market::eSellType_ConsignmentSale_GoldBet:
            {
                std::string strTemp;
                strTemp += sc::string::format( "%s : %s\r\n", ID2SERVERTEXT( "PRIVATE_MARKET_POINTSHOP_TRADE", 3 ), NS_UITEXTCONTROL::MAKE_MONEY_FORMAT( nTotalPrice, 3, "," ) );
                strTemp += sc::string::format( "%s : %s\r\n\r\n", ID2SERVERTEXT( "PRIVATE_MARKET_POINTSHOP_TRADE", 4 ), NS_UITEXTCONTROL::MAKE_MONEY_FORMAT( llFeeTemp, 3, "," ) );
    ...
    if( 0 < SaleItem.sItemcustom.wTurnNum )
    {
        ...
        {
            // 골드로 거래 했다면 돈을 담는다.
            if (SaleItem.SellType==(private_market::eSellType_ConsignmentSale_Gold||private_market::eSellType_ConsignmentSale_GoldBet))
...
bool GLConsignmentSaleField::AckItemPurchaseDB( NET_MSG_GENERIC* nmg )
{
	...
	if ( pChar )
	{
		...
		{
		case private_market::eSellType_ConsignmentSale_RanPoint:
			pChar->PointShopPointAdd( 0, RecvData.llItemPriceTotal,0, pointshop::CONSIGNMENT_SALE_ERROR );
			break;
		case private_market::eSellType_ConsignmentSale_Gold:
