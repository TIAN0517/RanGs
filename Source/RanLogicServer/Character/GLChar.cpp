...
void GLChar::VAR_BODY_POINT( const EMCROW emActorCrow, const DWORD dwActorID, const DWORD dwVarTypeFlag, int& nVar_HP, int& nVar_MP, int& nVar_SP )
{
	...
	if ( PartyID.isValidParty() == true )
	{
		GLMSG::SNET_PARTY_MBR_UPDATE NetMsgUp(PartyID,m_dwGaeaID, GetPosition(), m_sHP, GetLevel() );
		SendToParty(PartyID, &NetMsgUp);
		m_pGaeaServer->SENDTOAGENT(&NetMsgUp);
	}
}
...
void GLChar::ToDoRightAfterRecallDone()
{
    ...
    if ( m_mapID != m_mapIDPrevious )
    {
		...
		if ( m_pActivityMan ) m_pActivityMan->AddPointsTest();
    }
...
void GLChar::SENDRANKINGSINFO ( bool bPK, bool bLevel , bool bRich, bool bReset )
{
	GLMSG::SNET_GET_RANKINGS_FROMDB_FB NetMsgFB;
	if ( bReset )
	{
		NetMsgFB.emFB = EMREQ_RANKING_FROMDB_FB_RESET;
		SendToClient ( m_ClientSlot, &NetMsgFB );
	}

	if ( bPK )
	{
		RANKINGMAP_ITER pos = m_mapRankingsPK.begin();
		RANKINGMAP_ITER end = m_mapRankingsPK.end();
		for ( ;pos!=end; ++pos )
		{
			RANKING& sRANK = (*pos).second;
			if ( strlen( sRANK.szName ) > 0 )
			{
				NetMsgFB.sRank.bLevel = sRANK.bLevel;
				NetMsgFB.sRank.bRich = sRANK.bRich;
				NetMsgFB.sRank.bPK = sRANK.bPK;

				NetMsgFB.sRank.lMoney = sRANK.lMoney;
				NetMsgFB.sRank.dwCharID = sRANK.dwCharID;
				NetMsgFB.sRank.nOnline = sRANK.nOnline;
				NetMsgFB.sRank.wClass = sRANK.wClass;
				NetMsgFB.sRank.dwClub = sRANK.dwClub;
				NetMsgFB.sRank.wDeaths = sRANK.wDeaths;
				NetMsgFB.sRank.wKills = sRANK.wKills;
				NetMsgFB.sRank.wSchool = sRANK.wSchool;
				NetMsgFB.sRank.wLevel = sRANK.wLevel;
				StringCchCopy( NetMsgFB.sRank.szName, CHAR_SZNAME+1, sRANK.szName );
				std::tr1::shared_ptr< GLClubField > spClub = GetClub( sRANK.dwClub );
				if ( spClub )
				{
					DWORD dwSize = (DWORD) (sizeof(DWORD)*EMCLUB_MARK_SX*EMCLUB_MARK_SY);
					if ( spClub )
					{
						memcpy( NetMsgFB.sRank.m_aryMark, spClub->m_aryMark, dwSize);
						NetMsgFB.sRank.m_dwMarkVER = spClub->m_dwMarkVER;
					}
				}
				NetMsgFB.emFB = EMREQ_RANKING_FROMDB_FB_OK;
				SendToClient ( m_ClientSlot, &NetMsgFB );
			}
		}
	}
	...
	if ( bRich )
	{
		RANKINGMAP_ITER pos = m_mapRankingsRich.begin();
		RANKINGMAP_ITER end = m_mapRankingsRich.end();
		for ( ;pos!=end; ++pos )
		{
			RANKING& sRANK = (*pos).second;
			if ( strlen( sRANK.szName ) > 0 )
	...
	if ( bLevel )
	{
		RANKINGMAP_ITER pos = m_mapRankings.begin();
		RANKINGMAP_ITER end = m_mapRankings.end();
		for ( ;pos!=end; ++pos )
		{
			RANKING& sRANK = (*pos).second;
			if ( strlen( sRANK.szName ) > 0 )
	...
	NetMsgFB.emFB = EMREQ_RANKING_FROMDB_FB_END;
	SendToClient ( m_ClientSlot, &NetMsgFB );
}
...
void GLChar::PkRankAdd( DWORD dwKilled, DWORD dwKiller )
{
	PGLCHAR pCharKilled = m_pGaeaServer->GetCharByDbNum ( dwKilled );
	if ( !pCharKilled )	return;

	PGLCHAR pCharKiller = m_pGaeaServer->GetCharByDbNum( dwKiller );
	...
	SendToClient( &NetMsgBroadCast );
	SendMsgViewAround ( &NetMsgBroadCast );
}
