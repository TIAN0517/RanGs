...
HRESULT GLChar::MsgPMarketOpen(NET_MSG_GENERIC* nmg)
{
	...
	m_pPrivateMarket->SetSaleType ( (GLPrivateMarket::EM_SALE_TYPE) pITEM->sSuitOp.wReModelNum );
	// offline vend logic
	{
		GLMSG::SNETPC_PMARKET_OPEN NetMsg;
		NetMsg.bPremium = m_pPrivateMarket->IsPremiumMarket();
		SendToAgent ( &NetMsg );
		sc::writeLogInfo( sc::string::format("SNETPC_PMARKET_OPEN to agent IsPremium(%d)",pITEM->sSuitOp.wReModelNum) );
	}
	//
