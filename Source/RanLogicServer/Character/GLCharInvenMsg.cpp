...
HRESULT GLChar::MsgReqSlotToInven( NET_MSG_GENERIC* nmg )
{
	...
	SITEM* pSlotItem = GET_SLOT_ITEMDATA( pNetMsg->emFromSlot );
	if ( pSlotItem )
	{
...
// 	}
	...
	if ( sHold_BackUp.sItemCustom.IsNotStorage() )
		return E_FAIL;
...
HRESULT GLChar::MsgReqBuyFromNpc( NET_MSG_GENERIC* nmg )
{
	...
	else
	{
		...
		if ( pItem->BasicType() == ITEM_SUIT && pItem->GetItemLevel() >= EMITEM_LEVEL_RARE )
		{
			if( sNewItem.GENERATE_RANDOM_OPT( false ) )
				LogItemRandomOption( sNewItem );
		}
...
HRESULT GLChar::MsgReqInvenResetSkSt( NET_MSG_GENERIC* nmg )
{
	...
	/*if ( ReturnMoney == 0 )
	{
		...
		for ( WORD i=0; i<EMGRADE; ++i )
		{
			...
			// 불필요하게 복사하지 않도록 수정함
			itemcustomNew.Initialize( SNATIVEID(EMMARBLE_MID,i) );
			itemcustomNew.BornTimeSet( Time.GetTime() );
			itemcustomNew.GenTypeSet( EMGEN_SYSTEM );
			itemcustomNew.TurnNumSet ( wGRADE[i] );
			...
			m_pGaeaServer->PostSendSystemMail(
				...
				std::string( pITEM->GetName() ),
				std::string( strCombine.GetString() ) );
		}
	}
	else*/
	{
