...
void GLChar::PointShopBattlePointSub( const SNATIVEID& BuyItem, __int64 ItemPrice, int nTraceType )
{
	PointShopBattlePointSub( ItemPrice, BuyItem, pointshop::BUY_ITEM, nTraceType );
}

void GLChar::PointShopBattlePointSub( __int64 point, pointshop::EM_POINT_CHANGE_TYPE ePointChangeType, int nTraceType  )
{
	PointShopBattlePointSub( point, SNATIVEID(0,0), ePointChangeType, nTraceType );
}

void GLChar::PointShopBattlePointSub( __int64 point, const SNATIVEID& Item, pointshop::EM_POINT_CHANGE_TYPE ePointChangeType, int nTraceType )
{
	CHARACTER_POINT OldPoint = m_PointShopPoint;
	CHARACTER_POINT NewPoint = PointShopBattlePointSub( point, nTraceType );

	// Send Log
	PointShopPointLog(OldPoint, NewPoint, ePointChangeType, Item, point);
}

CHARACTER_POINT GLChar::PointShopBattlePointSub( __int64 point, int nTraceType )
{
	CHARACTER_POINT OldPoint = m_PointShopPoint;
	CHARACTER_POINT NewPoint = m_PointShopPoint;
	__int64 ItemPriceTemp = point;

	if( NewPoint.m_PointBattle >= ItemPriceTemp )
	{
		// 환불 불가 Point 가 가격보다 높기 때문에 차감하고 종료;
		NewPoint.m_PointBattle = NewPoint.m_PointBattle - ItemPriceTemp;
	}

	if( NewPoint.m_PointBattle < 0 )
	...
	PointShopPointDbSave( NewPoint.Refundable(), NewPoint.NotRefundable(), NewPoint.BattlePoint(), nTraceType, OldPoint.m_PointRefundable, OldPoint.m_PointNotRefundable, OldPoint.m_PointBattle, point );
...
bool GLChar::PointShopPointAdd(__int64 PointRefundable, __int64 PointNotRefundable, __int64 PointBattle, pointshop::EM_POINT_CHANGE_TYPE Type)
{
	...
	NewPoint.AddBattlePoint(PointBattle);
    PointShopPointSet(NewPoint);

    // DB 에 저장
	// type 추가 : 포인트 사라지는경우 추적을 위해서 수정 
    PointShopPointDbSave(NewPoint.Refundable(), NewPoint.NotRefundable(), NewPoint.BattlePoint(), 0, OldPoint.m_PointRefundable, OldPoint.m_PointNotRefundable, OldPoint.m_PointBattle, 0 );    
...
void GLChar::PointShopBuyItem( const SNATIVEID& BuyItem, __int64 ItemPrice, const SITEMCUSTOM& ItemCustom, const WORD wCurrency )
{    
	if ( wCurrency == pointshop::POINT_SHOP_PREMIUM_POINT )
		PointShopPointSub( BuyItem, ItemPrice,  pointshop::POINT_SUB_TYPE_SHOP_BUY );
	else
		PointShopBattlePointSub( BuyItem, ItemPrice,  pointshop::POINT_SUB_TYPE_SHOP_BUY );
...
void GLChar::PointShopPointDbSave(__int64 PointRefundable, __int64 PointNotRefundable, __int64 PointBattle, int nType, __int64 OriPointRefundable, __int64 OriPointNotRefundable, __int64 OriPointBattle, __int64 SubValue )
{
	if( PointRefundable == 0 && PointNotRefundable == 0 && PointBattle == 0 )
	{
		sc::writeLogError(sc::string::format("[TRACE POINT] m_CharDbNum : %1%, nType : %2%", m_CharDbNum,  nType));
	}

	sc::writeLogInfo(sc::string::format("[TRACE POINT] m_CharDbNum : %1%, nType : %2%, Refundable : %3%, NotRefundable : %4%, BattlePoint : %5% OriRefundable : %6%, OriNotRefundable : %7%, OriBattle : %8%,  SubValue : %9%", m_CharDbNum,  nType, PointRefundable, PointNotRefundable, OriPointRefundable, OriPointNotRefundable,OriPointBattle, SubValue ));
