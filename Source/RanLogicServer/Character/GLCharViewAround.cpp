...
HRESULT GLChar::UpdateViewAround()
{
	...
	int nAddViewRange(0);
	if ( m_bFlyCameraUse ) nAddViewRange = 5000;

	const int _maxViewRange( MAX_VIEWRANGE );
	BOUDRECT bRect(nPosition[0]+_maxViewRange+nAddViewRange, nPosition[1]+_maxViewRange+nAddViewRange, nPosition[0]-(_maxViewRange+nAddViewRange), nPosition[1]-(_maxViewRange+nAddViewRange));

	const int _maxViewRange2( GETVISIBLESIGHT() );
	BOUDRECT sRectOnlySee(nPosition[0]+_maxViewRange2+nAddViewRange, nPosition[1]+_maxViewRange2+nAddViewRange, nPosition[0]-(_maxViewRange2+nAddViewRange), nPosition[1]-(_maxViewRange2+nAddViewRange));
