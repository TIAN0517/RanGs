    ...
    case EM_RESURRECTION_SPECIAL_ITEM:  // 아이템을 사용한 특수 부활 ( 귀혼주 부활 );
        {
	        ...
	        // 아아템이 있는지?;
			SITEM* pITEM = (GLUseFeatures::GetInstance().IsUsingRenewInvenWindow()) ? GET_SLOT_ITEMDATA(SLOT_DECORATION) : GET_SLOT_ITEMDATA(SLOT_NECK);
	        if ( !pITEM )
	        {
			...
			(GLUseFeatures::GetInstance().IsUsingRenewInvenWindow()) ? ConsumeSlotItem(SLOT_DECORATION) : ConsumeSlotItem(SLOT_NECK);
