...
HRESULT	GLCharAG::MsgPMarketOpen( NET_MSG_GENERIC* nmg )
{
	...
	m_sPMarket.SetSaleType ( pNetMsg->bPremium ? GLPrivateMarket::EM_SALE_TYPE_PREMIUM : GLPrivateMarket::EM_SALE_TYPE_NORMAL );
	sc::writeLogInfo( "GLCharAG MsgPMarketOpen" );

	return S_OK;
}

HRESULT	GLCharAG::MsgPMarketClose( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPC_PMARKET_CLOSE* pNetMsg = (GLMSG::SNETPC_PMARKET_CLOSE*)nmg;

	m_sPMarket.DoMarketClose();
	sc::writeLogInfo( "GLCharAG MsgPMarketClose" );
