...
void CAgentServer::MsgChangePass( MSG_LIST* pMsg )
{
	...
	if (sizeof(CHINA_NET_CP_DATA) != pNml->Size())
	{
		sc::writeLogError( "ERROR:CHINA_NET_CP_DATA Wrong Message Size" );
		return;
	}
	// 복호화
	...
	m_Tea.decrypt( pNml->szCaptcha, USR_RAND_PASS_LENGTH+1 );

	TCHAR szCaptcha[USR_RAND_PASS_LENGTH+1] = {0};
	...
	StringCchCopy( szCaptcha, USR_RAND_PASS_LENGTH+1, pNml->szCaptcha );

	m_pClientManager->SetAccountPass( pMsg->dwClient, false );

	m_pDbManager->AddUserJob(
		db::DbActionPtr(
		new CAgentUserChangePassword(
		szUserid,
		szOPassword,
		szPassword,
		szPassword2,
...
void CAgentServer::MsgChangePassBack (NET_CP_FEEDBACK_DATA2* nlfd2)
{
	...
	nlfd.nType = CHINA_NET_MSG_PASS_FB;
...
void CAgentServer::MsgChangePin( MSG_LIST* pMsg )
{
	...
	if (sizeof(CHINA_NET_CPN_DATA) != pNml->Size())
	{
		sc::writeLogError("ERROR:CHINA_NET_CPN_DATA Wrong Message Size");
		return;
	}
	// 복호화
...
void CAgentServer::MsgChangePinBack (NET_CPN_FEEDBACK_DATA2* nlfd2)
{
	...
	nlfd.nType = CHINA_NET_MSG_PIN_FB;
...
void CAgentServer::MsgChangeEmail( MSG_LIST* pMsg )
{
	...
	if (sizeof(CHINA_NET_CE_DATA) != pNml->Size())
	{
		sc::writeLogError("ERROR:CHINA_NET_CE_DATA Wrong Message Size" );
		return;
	}
	// 복호화
	m_Tea.decrypt( pNml->szPassword, USR_PASS_LENGTH );
	m_Tea.decrypt( pNml->szPassword2, USR_PASS_LENGTH );
	m_Tea.decrypt( pNml->szCaptcha, USR_RAND_PASS_LENGTH+1 );
	m_Tea.decrypt( pNml->szUserEmail, USR_EMAIL_LENGTH+1 );

	TCHAR szCaptcha[USR_RAND_PASS_LENGTH+1] = {0};
	TCHAR szPassword[USR_PASS_LENGTH] = {0};
	TCHAR szPassword2[USR_PASS_LENGTH] = {0};
	TCHAR szUserid[USR_ID_LENGTH+1] = {0};
	TCHAR szUserEmail[USR_EMAIL_LENGTH+1] = {0};
	DWORD dwClient;
	...
	StringCchCopy( szPassword2, USR_PASS_LENGTH, pNml->szPassword2 );
	StringCchCopy( szCaptcha, USR_RAND_PASS_LENGTH+1, pNml->szCaptcha );
	StringCchCopy( szUserEmail, USR_EMAIL_LENGTH+1, pNml->szUserEmail );

	m_pClientManager->SetAccountPass( pMsg->dwClient, false );

	m_pDbManager->AddUserJob(
		db::DbActionPtr(
		new CAgentUserChangeEmail(
		szUserid, 
		szPassword,
		szPassword2,
		szUserEmail,
...
void CAgentServer::MsgChangeEmailBack (NET_CE_FEEDBACK_DATA2* nlfd2)
{
	...
	nlfd.nType = CHINA_NET_MSG_EMAIL_FB;
...
void CAgentServer::MsgTopUp( MSG_LIST* pMsg )
{
	...
	if (sizeof(CHINA_NET_TOPUP_DATA) != pNml->Size())
	{
		sc::writeLogError("ERROR:CHINA_NET_TOPUP_DATA Wrong Message Size" );
		return;
	}
	// 복호화
...
void CAgentServer::MsgTopUpBack (NET_TOPUP_FEEDBACK_DATA2* nlfd2)
{
	...
	nlfd.nType = CHINA_NET_MSG_TOPUP_FB;
...
void CAgentServer::MsgGameTimeCVT( MSG_LIST* pMsg )
{
	...
	if (sizeof(CHINA_NET_GTCVT_DATA) != pNml->Size())
	{
		sc::writeLogError("ERROR:CHINA_NET_GTCVT_DATA Wrong Message Size" );
		return;
	}
	// 복호화
...
void CAgentServer::MsgGameTimeCVTBack (NET_GTCVT_FEEDBACK_DATA2* nlfd2)
{
	...
	nlfd.nType = CHINA_NET_MSG_GT2VP_FB;
...
void CAgentServer::MsgPremiumPointCVT( MSG_LIST* pMsg )
{
	...
	if (sizeof(CHINA_NET_PPCVT_DATA) != pNml->Size())
	{
		sc::writeLogError("ERROR:CHINA_NET_PPCVT_DATA Wrong Message Size" );
		return;
	}
	// 복호화

	m_Tea.decrypt( pNml->szCaptcha, USR_RAND_PASS_LENGTH+1 );
	m_Tea.decrypt( pNml->szValue,  USR_RAND_PASS_LENGTH+1 );

	TCHAR szCaptcha[USR_RAND_PASS_LENGTH+1] = {0};
	TCHAR szValue[USR_RAND_PASS_LENGTH+1] = {0};
	TCHAR szUserid[USR_ID_LENGTH+1] = {0};
	DWORD dwClient;
...
void CAgentServer::MsgPremiumPointCVTBack (NET_PPCVT_FEEDBACK_DATA2* nlfd2)
{
	...
	nlfd.nType = CHINA_NET_MSG_PP2VP_FB;
...
void CAgentServer::MsgResetPin ( MSG_LIST* pMsg )
{
	...
	if (sizeof(CHINA_NET_RPN_DATA) != pNml->Size())
	{
		sc::writeLogError("ERROR:CHINA_NET_RPN_DATA Wrong Message Size" );
		return;
	}
	// 복호화
...
void CAgentServer::MsgResetPinBack ( NET_RPN_FEEDBACK_DATA2* nlfd2 )
{
	...
	nlfd.nType = CHINA_NET_MSG_RESETPIN_FB;
...
void CAgentServer::MsgResetPass ( MSG_LIST* pMsg )
{
	...
	if (sizeof(CHINA_NET_RPS_DATA) != pNml->Size())
	{
		sc::writeLogError("ERROR:CHINA_NET_RPS_DATA Wrong Message Size" );
		return;
	}
	// 복호화
	m_Tea.decrypt( pNml->szUserid, USR_ID_LENGTH+1 );
...
void CAgentServer::MsgResetPassBack ( NET_RPS_FEEDBACK_DATA2* nlfd2 )
{
	...
	nlfd.nType = CHINA_NET_MSG_RESETPASS_FB;
...
void CAgentServer::MsgResetPass2 ( MSG_LIST* pMsg )
{
	...
	if (sizeof(CHINA_NET_RPS2_DATA) != pNml->Size())
	{
		sc::writeLogError("ERROR:CHINA_NET_RPS2_DATA Wrong Message Size" );
		return;
	}
	// 복호화
	m_Tea.decrypt( pNml->szPassword2, USR_PASS_LENGTH );
...
void CAgentServer::MsgResetPassBack2 ( NET_RPS2_FEEDBACK_DATA2* nlfd2 )
{
	...
	nlfd.nType = CHINA_NET_MSG_RESETPASS2_FB;
...
* 일반 로그인 : id / pwd 입력시
*/
void CAgentServer::MsgRegister(DWORD dwClient, DWORD dwGaeaID, NET_MSG_GENERIC* nmg)
{		
	if (nmg == NULL)
		return;
	...
	//{
	//	sc::writeLogError(sc::string::format("US Register ERROR (ServicePrivider is not match GaeaID : %1%, SP : %2%)", dwGaeaID, m_ServiceProvider ));
	//	return;
	//}
	...
	NET_REGISTER_DATA* pPacket = (NET_REGISTER_DATA*) nmg;
	DWORD dwFlags    = 0;
	DWORD dwSndBytes = 0;

	if (sizeof(NET_REGISTER_DATA) != pPacket->Size())
	{
		sc::writeLogError("NET_REGISTER_DATA Wrong Message Size");
		return;
	}
	...
	m_Tea.decrypt(pPacket->m_szUserEmail, USR_EMAIL_LENGTH+1);
	m_Tea.decrypt(pPacket->m_szPassword, USR_PASS_LENGTH+1);
	m_Tea.decrypt(pPacket->m_szPassword2, USR_PASS_LENGTH+1);

	TCHAR szPassword[USR_PASS_LENGTH+1] = {0};
	TCHAR szPassword2[USR_PASS_LENGTH+1] = {0};
	TCHAR szUserid[USR_ID_LENGTH+1] = {0};
	TCHAR szUserEmail[USR_ID_LENGTH+1] = {0};

	StringCchCopy(szUserid, USR_ID_LENGTH+1, pPacket->m_szUserid);
	StringCchCopy(szUserEmail, USR_EMAIL_LENGTH+1, pPacket->m_szUserEmail);
	StringCchCopy(szPassword, USR_PASS_LENGTH+1, pPacket->m_szPassword);
	StringCchCopy(szPassword2, USR_PASS_LENGTH+1, pPacket->m_szPassword2);
	// 접속하려는 채널 검사
	...
	{
		NET_REGISTER_FEEDBACK_DATA		nlfd;
		nlfd.nType = NET_MSG_REGISTER_FB;
		nlfd.m_Result   = EM_REGISTER_FB_SUB_FAIL;
		SendClient(dwClient, &nlfd); // 결과전송
		CloseClient(dwClient, net::KICKOUT_UNDEFINE); // 접속종료

		sc::writeLogError(sc::string::format("MsgRegister Invalid Channel Number (Channel:%d)", nChannel ));
		return;
	}
...
void CAgentServer::MsgRegisterBack (NET_REGISTER_FEEDBACK_DATA2* nlfd2)
{
	NET_REGISTER_FEEDBACK_DATA		nlfd;
	nlfd.nType = NET_MSG_REGISTER_FB;
...
void CAgentServer::ThaiMsgLogin (DWORD dwClient, DWORD dwGaeaID, NET_MSG_GENERIC* nmg)
{
	...
	m_Bit.buf_decode (pBuffer+sizeof(NET_MSG_GENERIC), 
					  (tnld->Size())-sizeof(NET_MSG_GENERIC),
					  ck.nKeyDirection,
					  ck.nKey);
	*/
...
void CAgentServer::GsMsgLoginBack( GS_NET_LOGIN_FEEDBACK_DATA2* nlfd2 )
{
	...
	if (nlfd2->m_emResult == EM_LOGIN_FB_SUB_OK || nlfd2->m_emResult == EM_LOGIN_FB_KR_OK_USE_PASS || nlfd2->m_emResult == EM_LOGIN_FB_KR_OK_NEW_PASS)
	{
			...
			(strcmp(m_pClientManager->GetUserID(dwClient),   nlfd2->szUserid) == 0)  )
		{
			...
			::StringCchCopy(nlfd.szUserEmail, USR_EMAIL_LENGTH+1, (const char*) nlfd2->szUserEmail);
