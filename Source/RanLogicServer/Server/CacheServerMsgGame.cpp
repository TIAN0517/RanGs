...
void CacheServer::MsgCharReqPointFH(DWORD dwClient, NET_MSG_GENERIC* pMsg)
{
    ...
    else
    {
        ...
        m_pDbManager->GetChaPoint(ChaDbNum, PointRefundable, PointNotRefundable, PointBattle);

        NET_CHAR_POINT_HF MsgHF(ChaDbNum, CHARACTER_POINT(PointRefundable, PointNotRefundable, PointBattle));
        SendClient(dwClient, &MsgHF);
    }
}
