...
BOOL GLGaeaServer::MsgPointShopGmCharPointAddCF(DWORD dwClientID, DWORD dwGaeaID, NET_MSG_GENERIC* pMsg)
{
    ...
    pChar->PointShopPointAdd(0, AddPoint, 0, pointshop::GM_COMMAND);
    return TRUE;
}
...
{    
	...
	std::tr1::tuple< bool, __int64 > Result2 = m_pPointShop->Currency( BuyItemId );
	...
	__int64 ChaPoint = ( ItemCurrency == pointshop::POINT_SHOP_PREMIUM_POINT ) ? pChar->PointShopPoint() : pChar->PointShopBattlePoint();
    ...
    pChar->PointShopBuyItem( BuyItemId, ItemPrice, BuyItem, ItemCurrency );
    SENDTOCLIENT( dwClientID, &MsgFC );
    return TRUE;
}
...
BOOL GLGaeaServer::MsgPointShopPointCardCF(DWORD dwClientID, DWORD dwGaeaID, NET_MSG_GENERIC* pMsg)
{
    ...
    if (ItemType == ITEM_POINT_CARD_REFUND)
        pChar->PointShopPointAdd(PointCharge, 0, 0, pointshop::POINT_CARD);

    if (ItemType == ITEM_POINT_CARD_NOT_REFUND)
        pChar->PointShopPointAdd(0, PointCharge, 0, pointshop::POINT_CARD);    
