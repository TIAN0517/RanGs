...
void GLGaeaServer::initPacketFunc()
{
	...
	m_MsgFunc[NET_MSG_REQ_TUTORIAL_COMPLETE].Msgfunc = &GLGaeaServer::MsgChaTutorialComplete;
	m_MsgFunc[NET_MSG_REQ_TUTORIAL_COMPLETE_MOVE].Msgfunc = &GLGaeaServer::MsgChaTutorialCompleteMove;
	...
	m_MsgFunc[NET_MSG_GCTRL_REQ_STATSUPCMD].Msgfunc = &GLGaeaServer::MsgReqAddStats;

	m_MsgFunc[NET_MSG_REQ_CHAR_LOCK_CF].Msgfunc = &GLGaeaServer::MsgReqCharacterLock;
	m_MsgFunc[NET_MSG_REQ_CHAR_LOCK_FB].Msgfunc = &GLGaeaServer::MsgCharacterLockFB;

	m_MsgFunc[NET_MSG_GCTRL_GET_RANKINGS_FROMDB].Msgfunc = &GLGaeaServer::MsgReqRankingInfo;
...
BOOL GLGaeaServer::MsgChaTutorialComplete( DWORD dwClientID, DWORD dwGaeaID, NET_MSG_GENERIC* nmg )
{
	if( dwGaeaID == GAEAID_NULL )
		return FALSE;
	...
	pChar->m_bTutorial = false;

	return TRUE;
}
BOOL GLGaeaServer::MsgChaTutorialCompleteMove( DWORD dwClientID, DWORD dwGaeaID, NET_MSG_GENERIC* nmg )
{
	if( dwGaeaID == GAEAID_NULL )
		return FALSE;
	...
	if ( pChar->m_bTutorial )
		return FALSE;

	GLMSG::SNETPC_REQ_TUTORIAL_COMPLETE_MOVE* msg = ( GLMSG::SNETPC_REQ_TUTORIAL_COMPLETE_MOVE* )nmg;

	MoveSystem::SMOVE_INFO sMoveInfo;
	sMoveInfo.dwChaDBNum = pChar->CharDbNum();
	sMoveInfo.sMapID = msg->sMapID;
	sMoveInfo.dwGateID = msg->dwGateID;
...
BOOL GLGaeaServer::MsgReqAddStats ( DWORD dwClientID, DWORD dwGaeaID, NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPC_REQ_STATSUPCMD* pNetMsg =
		static_cast< GLMSG::SNETPC_REQ_STATSUPCMD* >( nmg );
	if ( NULL == pNetMsg )
		return FALSE;
	...
	pChar->MsgReqStatsUpCmd( nmg );

	return TRUE;
}

BOOL GLGaeaServer::MsgCharacterLockFB( DWORD dwClientID, DWORD dwGaeaID, NET_MSG_GENERIC* nmg )
{
	GLChar* pChar = GetChar( dwGaeaID );
	if ( NULL == pChar )
		return FALSE;
	...
	sc::writeLogInfo(std::string("MsgCharacterLockFB"));
}

BOOL GLGaeaServer::MsgReqCharacterLock( DWORD dwClientID, DWORD dwGaeaID, NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPC_REQ_CHAR_LOCK_CF* pNetMsg =
		static_cast< GLMSG::SNETPC_REQ_CHAR_LOCK_CF* >( nmg );
	if ( NULL == pNetMsg )
		return FALSE;
	...
	if ( !pNetMsg->szPassword )	return E_FAIL;

	CString strPassword( pNetMsg->szPassword );
	...
	sc::writeLogInfo(std::string("MsgReqCharacterLock"));
	...
	case EMCHARLOCK_INVENTORY_PASSWORD:
		{
			AddGameAdoJob(
				db::DbActionPtr(
				new db::ChaLockPassword( pChar->ClientSlot(), pNetMsg->szPassword, EMCHARLOCK_TYPE_INVENTORY, pChar, pNetMsg->bUnlock ) ) );	//mjeon.ado
				sc::writeLogInfo(std::string("ChaLockPassword EMCHARLOCK_INVENTORY_PASSWORD"));
		}
		break;
	case EMCHARLOCK_INVENTORY_CHANGE_PASSWORD:
		{
			if ( !pNetMsg->szPIN )	return E_FAIL;

			CString strPIN( pNetMsg->szPIN );
			...
			AddGameAdoJob(
				db::DbActionPtr(
				new db::ChaLockChangeResetPassword( pChar->ClientSlot(), pNetMsg->szPassword, pNetMsg->szPIN, EMCHARLOCK_TYPE_INVENTORY, pChar, false ) ) );	//mjeon.ado
				sc::writeLogInfo(std::string("ChaLockChangeResetPassword EMCHARLOCK_INVENTORY_CHANGE_PASSWORD"));
		}
		break;
	case EMCHARLOCK_INVENTORY_RESET_PASSWORD:
		{
			...
			AddGameAdoJob(
				db::DbActionPtr(
				new db::ChaLockChangeResetPassword( pChar->ClientSlot(), pNetMsg->szPassword, pNetMsg->szPIN, EMCHARLOCK_TYPE_INVENTORY, pChar, true ) ) );	//mjeon.ado
				sc::writeLogInfo(std::string("ChaLockChangeResetPassword EMCHARLOCK_INVENTORY_RESET_PASSWORD"));
		}
		break;
	...
	case EMCHARLOCK_LOCKER_PASSWORD:
		{
			AddGameAdoJob(
				db::DbActionPtr(
				new db::ChaLockPassword( pChar->ClientSlot(), pNetMsg->szPassword, EMCHARLOCK_TYPE_LOCKER, pChar, pNetMsg->bUnlock ) ) );	//mjeon.ado
				sc::writeLogInfo(std::string("ChaLockPassword EMCHARLOCK_LOCKER_PASSWORD"));
		}
		break;
	case EMCHARLOCK_LOCKER_CHANGE_PASSWORD:
		{
			...
			AddGameAdoJob(
				db::DbActionPtr(
				new db::ChaLockChangeResetPassword( pChar->ClientSlot(), pNetMsg->szPassword, pNetMsg->szPIN, EMCHARLOCK_TYPE_LOCKER, pChar, false ) ) );	//mjeon.ado
				sc::writeLogInfo(std::string("ChaLockChangeResetPassword EMCHARLOCK_LOCKER_CHANGE_PASSWORD"));
		}
		break;
	case EMCHARLOCK_LOCKER_RESET_PASSWORD:
		{
			...
			AddGameAdoJob(
				db::DbActionPtr(
				new db::ChaLockChangeResetPassword( pChar->ClientSlot(), pNetMsg->szPassword, pNetMsg->szPIN, EMCHARLOCK_TYPE_LOCKER, pChar, true ) ) );	//mjeon.ado
				sc::writeLogInfo(std::string("ChaLockChangeResetPassword EMCHARLOCK_LOCKER_RESET_PASSWORD"));
		}
		break;
	};
}
...
BOOL GLGaeaServer::MsgReqRankingInfo ( DWORD dwClientID, DWORD dwGaeaID, NET_MSG_GENERIC* nmg )
{
	GLMSG::SNET_GET_RANKINGS_FROMDB* pNetMsg =
		static_cast< GLMSG::SNET_GET_RANKINGS_FROMDB* >( nmg );
	if ( NULL == pNetMsg )
		return FALSE;
	...
	AddGameDbJob(
		db::DbActionPtr(
		new db::CGetRankings( pChar->CharDbNum() ) ) );	//mjeon.ado
		sc::writeLogInfo(std::string("MsgReqRankingInfo CGetRankings"));
