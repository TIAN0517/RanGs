...
public:
	...
	int SaveCharacterStreakData(SCHARDATA2* pChar);
...
//         const __time64_t& LogDate);
	...
	virtual int GetRankings( std::vector<RANKING> &v , bool bPK = false , bool bLevel = false , bool bRich = false ) { return 0; } 
	...
	int GetCharacterInfoStreakData(int nChaNum, SCHARDATA2* pChaData2);
	...
	virtual int	UserRegister(
                    ...
                    const TCHAR* szPasswd, 
                    const TCHAR* szPasswd2)	override;



	//userpanel
	virtual int UserChangeEmail(
		const TCHAR* szUsrID, 
		const TCHAR* szPasswd, 
		const TCHAR* szPasswd2,
		const TCHAR* szUsrEmail,
		const TCHAR* szCaptcha) override;

	virtual int UserChangePin(
		const TCHAR* szUsrID, 
	...
	virtual int UserResetPass(
		const TCHAR* szUsrID, 
		const TCHAR* szPasswd2,
		const TCHAR* szCaptcha) override;

	virtual int UserResetPass2(
		const TCHAR* szUsrID, 
		const TCHAR* szPasswd, 
		const TCHAR* szPasswd2,
		const TCHAR* szCaptcha) override;

	virtual int UserResetPin(
		const TCHAR* szUsrID, 
		const TCHAR* szUsrEmail,
		const TCHAR* szPasswd, 
		const TCHAR* szPasswd2,
	...
	virtual int UserGameTimeCVT(
		const TCHAR* szUsrID, 
		const DWORD dwGameTime,
		const TCHAR* szCaptcha) override;

	virtual int UserPremiumPointCVT(
		const TCHAR* szUsrID, 
		const TCHAR* szValue,
		const TCHAR* szCaptcha) override;

	virtual int UserTopUp(
		const TCHAR* szUsrID, 
		const TCHAR* szCode, 
		const TCHAR* szPin,
		const TCHAR* szCaptcha) override;
    ...
    virtual int GetChaPoint(int ChaDbNum, __int64& PointRefundable, __int64& PointNotRefundable, __int64& PointBattle) override;

    //! Character 의 PointShop Point 를 설정한다
    //! ChaDbNum : 캐릭터 DB 번호
    //! PointRefundable : 환불가능 포인트
    //! PointNotRefundable : 환불불가 포인트
    virtual int SetChaPoint(int ChaDbNum, const __int64& PointRefundable, const __int64& PointNotRefundable, const __int64& PointBattle) override;
	...
	virtual int ChaLockPassword(DWORD ChaDbNum, const char* ReqPassword, EMCHARLOCK_TYPE emLockType, PGLCHAR pCharacter, bool bUnlock = false)  override;
	virtual int ChaLockChangeResetPassword(DWORD ChaDbNum, const char* ReqPassword, const char* ReqPIN, EMCHARLOCK_TYPE emLockType, PGLCHAR pCharacter, bool bReset = false) override;
