...
int COdbcManager::SetChaPoint(int ChaDbNum, const __int64& PointRefundable, const __int64& PointNotRefundable, const __int64& PointBattle)
{
...
int COdbcManager::GetRankings( std::vector<RANKING> &v , bool bPK, bool bLevel, bool bRich )
{
	std::tr1::shared_ptr< db::ODBC_STMT > pConn = m_pGameDB->GetConnection();
	if ( !pConn  )
		return sc::db::DB_ERROR;

	TCHAR szTemp[512] = {0};
	if ( bLevel )
		_snprintf( szTemp, 512, "SELECT TOP 50 P.ChaExp,P.ChaMoney,P.ChaName,P.ChaLevel,P.ChaClass,P.ChaPkScore,P.ChaPkDeath,P.ChaNum,P.ChaSchool,P.ChaOnline,P.GuNum FROM RanUser.dbo.GSUserInfo U, RanGameS1.dbo.ChaInfo P WHERE P.UserNum = U.UserNum AND U.UserType < 11 AND U.UserBlockDate < GETDATE() AND P.ChaDeleted != 1 ORDER BY P.ChaLevel DESC "); /*P.ChaLevelUpDate ASC, P.ChaLevelWeb DESC*/

	if ( bPK )
		_snprintf( szTemp, 512, "SELECT TOP 500 P.ChaExp,P.ChaMoney,P.ChaName,P.ChaLevel,P.ChaClass,P.ChaPkScore,P.ChaPkDeath,P.ChaNum,P.ChaSchool,P.ChaOnline,P.GuNum FROM RanUser.dbo.GSUserInfo U, RanGameS1.dbo.ChaInfo P WHERE P.UserNum = U.UserNum AND U.UserType < 11 AND U.UserBlockDate < GETDATE() AND P.ChaDeleted != 1 ORDER BY P.ChaPkScore DESC, P.ChaPkDeath ASC");

	if ( bRich )
		_snprintf( szTemp, 512, "SELECT TOP 50 P.ChaExp,P.ChaMoney,P.ChaName,P.ChaLevel,P.ChaClass,P.ChaPkScore,P.ChaPkDeath,P.ChaNum,P.ChaSchool,P.ChaOnline,P.GuNum FROM RanUser.dbo.GSUserInfo U, RanGameS1.dbo.ChaInfo P WHERE P.UserNum = U.UserNum AND U.UserType < 11 AND U.UserBlockDate < GETDATE() AND P.ChaDeleted != 1  ORDER BY P.ChaMoney DESC");

	SQLCHAR    szName[CHAR_SZNAME+1] = {0}; SQLINTEGER cbszName = SQL_NTS;
	...
	SQLINTEGER nChaClub = 0, cbChaClub =SQL_NTS;
	...
	while(true)
	{
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{   
			::SQLGetData(pConn->hStmt, 2, SQL_C_SBIGINT, &llChaMoney,  0,             &cbChaMoney);
			::SQLGetData(pConn->hStmt, 3, SQL_C_CHAR, szName,   PURKEY_LENGTH+1, &cbszName);
			::SQLGetData(pConn->hStmt, 4, SQL_C_LONG, &nChaLevel,  0,             &cbChaLevel);
			::SQLGetData(pConn->hStmt, 5, SQL_C_LONG, &nChaClass,  0,             &cbChaClass);
			::SQLGetData(pConn->hStmt, 6, SQL_C_LONG, &nChaPkWin,  0,             &cbChaPkWin);
			::SQLGetData(pConn->hStmt, 7, SQL_C_LONG, &nChaPkLoss,  0,             &cbChaPkLoss);
			::SQLGetData(pConn->hStmt, 8, SQL_C_LONG, &nChaNum,  0,             &cbChaNum);
			::SQLGetData(pConn->hStmt, 9, SQL_C_LONG, &nChaSchool,  0,             &cbChaSchool);
			::SQLGetData(pConn->hStmt, 10, SQL_C_LONG, &nChaOnline,  0,             &cbChaOnline);
			::SQLGetData(pConn->hStmt, 11, SQL_C_LONG, &nChaClub,  0,             &cbChaClub);


			RANKING sTemp = RANKING();
			::StringCchCopy(sTemp.szName, CHAR_SZNAME+1, (const char*) szName);
			sTemp.lMoney = static_cast<LONGLONG> (llChaMoney);
			sTemp.wLevel  = static_cast<WORD> (nChaLevel);
			sTemp.wClass  = static_cast<WORD> (nChaClass);
			sTemp.wKills  = static_cast<WORD> (nChaPkWin);
			sTemp.wDeaths  = static_cast<WORD> (nChaPkLoss);
			sTemp.dwCharID = static_cast<DWORD> (nChaNum);
			sTemp.nChaNum = nNum;
			sTemp.wSchool = static_cast<WORD> (nChaSchool);
			sTemp.nOnline = static_cast<int> (nChaOnline);
			sTemp.dwClub = static_cast<DWORD> (nChaClub);
			sTemp.bRich	= bRich;
			sTemp.bLevel = bLevel;
			sTemp.bPK = bPK;
