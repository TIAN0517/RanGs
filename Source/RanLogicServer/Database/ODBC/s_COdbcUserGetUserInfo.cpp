	...
	NET_LOGIN_FEEDBACK_DATA2* pMsg )
{
	...
	SQLCHAR vUserEmail[USR_EMAIL_LENGTH+RSA_ADD+1]	  = {0}; SQLINTEGER cbUserEmail	   = SQL_NTS; // userpanel
...
//#elif defined( CH_PARAM )    
	...
	if (m_ServiceProvider == SP_CHINA)
	{
		...
		Query = 
			sc::string::format(
				"SELECT UserNum, UserType, PremiumDate, ChaRemain, ChaTestRemain, ChatBlockDate, PlayTime, OfflineTime, LastLoginDate, UserAge,UserEmail , UserPoint , UserPoint2 FROM dbo.UserInfo WHERE UserID='%s'",
				UserId.c_str());
	}
	else
	{
		Query = 
			sc::string::format(
				"SELECT UserNum, UserType, PremiumDate, ChaRemain, ChaTestRemain, ChatBlockDate, UserEmail , UserPoint , UserPoint2 , PlayTime FROM dbo.UserInfo WHERE UserID='%s'",
				UserId.c_str());
	}
#endif
		...
		if (m_ServiceProvider == SP_CHINA)
		{
			while(true)
			{
				...
				{	
					...
					::SQLGetData(spConn->hStmt, 11, SQL_C_CHAR, vUserEmail  ,	 USR_EMAIL_LENGTH+RSA_ADD+1 , &cbUserEmail);
					::SQLGetData(spConn->hStmt, 12, SQL_C_LONG,           &nUserPoint, 0, &cbUserPoint);
					::SQLGetData(spConn->hStmt, 13, SQL_C_LONG,           &nBattlePoint, 0, &cbBattlePoint); //userpanel
					...
					::StringCchCopy(pMsg->szUserEmail, USR_EMAIL_LENGTH+1, (const char*) vUserEmail); //userpanel
...
//#else
			while(true)
			{
				...
				{	
					...
					::SQLGetData(spConn->hStmt, 5, SQL_C_LONG,           &nChaTestRemain, 0, &cbChaTestRemain);
					::SQLGetData(spConn->hStmt, 6, SQL_C_TYPE_TIMESTAMP, &sChatBlockDate, 0, &cbChatBlockDate);
					::SQLGetData(spConn->hStmt, 7, SQL_C_CHAR, vUserEmail  ,	 USR_EMAIL_LENGTH+RSA_ADD+1 , &cbUserEmail);
					::SQLGetData(spConn->hStmt, 8, SQL_C_LONG,           &nUserPoint, 0, &cbUserPoint);
					::SQLGetData(spConn->hStmt, 9, SQL_C_LONG,           &nBattlePoint, 0, &cbBattlePoint); //userpanel
					::SQLGetData(spConn->hStmt, 10, SQL_C_LONG,           &nChinaGameTime, 0, &cbChinaGameTime);
	...
	GS_NET_LOGIN_FEEDBACK_DATA2* pMsg )
{
    ...
    std::string Query(
        sc::string::format(
            "SELECT UserNum, UserType, PremiumDate, ChaRemain, ChaTestRemain, ChatBlockDate, UserEmail , UserPoint , UserPoint2 , PlayTime FROM dbo.GSUserInfo WHERE UserID='%s'",
            szUserID));
    sReturn = ::SQLExecDirect(spConn->hStmt, (SQLCHAR*) Query.c_str(), SQL_NTS);
	if ((sReturn != SQL_SUCCESS) && (sReturn != SQL_SUCCESS_WITH_INFO)) 
	{
		...
		while (true)
		{
			...
			{	
				...
				CTime cTemp2(sChatBlockDate.year, sChatBlockDate.month,  sChatBlockDate.day,
                             sChatBlockDate.hour, sChatBlockDate.minute, sChatBlockDate.second);
				pMsg->tChatBlockTime = cTemp2.GetTime();
				pMsg->dwGameTime	= nChinaGameTime;
				pMsg->uUserPoints	= nUserPoint;
				pMsg->uBattlePoints	= nBattlePoint;
				::StringCchCopy(pMsg->szUserEmail, USR_EMAIL_LENGTH+1, (const char*) vUserEmail); //userpanel
			}
