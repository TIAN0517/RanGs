...
class ChaLockPassword : public DbAction
{
public:
	ChaLockPassword(DWORD ClientSlot, const char* ReqPassword, EMCHARLOCK_TYPE emLockType, PGLCHAR pCharacter, bool bUnlock = false);
	virtual ~ChaLockPassword();
	virtual int Execute(NetServer* pServer) override;

protected:
	std::string m_ReqPassword;
	EMCHARLOCK_TYPE m_emLockType;
	PGLCHAR m_pCharacter;
	bool m_bUnlock;
};

class ChaLockChangeResetPassword : public DbAction
{
public:
	ChaLockChangeResetPassword(DWORD ClientSlot, const char* ReqPassword, const char* ReqPIN, EMCHARLOCK_TYPE emLockType, PGLCHAR pCharacter, bool bReset = false);
	virtual ~ChaLockChangeResetPassword();
	virtual int Execute(NetServer* pServer) override;
...
class CGetRankings : public DbAction
{
public:
	CGetRankings( DWORD dwCharID ); 
    virtual ~CGetRankings() {};
    virtual int Execute(NetServer* pServer) override;

protected:
	DWORD	m_dwCharID;
    std::vector<RANKING> m_vRank;
    std::vector<RANKING> m_vRankPk;
    std::vector<RANKING> m_vRankRich;
};
