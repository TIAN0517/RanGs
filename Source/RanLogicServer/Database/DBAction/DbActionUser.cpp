...
//
//mjeon
//CAgentUserRegister has same code-flow as CAddLogServerState that was drawed in mjeon's note.
//
int CAgentUserRegister::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);
	...
	int nRetCode = 0;	
	int nResult = m_pDbManager->UserRegister(m_strUserID.GetString(),
	...
	NetMsgFB.nType = NET_MSG_REGISTER_FB;
	NetMsgFB.m_ClientSlot	= m_dwClientNum;
	...
	{
	case sc::db::DB_ERROR : // 시스템에러로 로그인 실패 (-1)
		NetMsgFB.m_emResult = EM_REGISTER_FB_SUB_FAIL;		
		sc::writeLogError(sc::string::format("CAgentUserRegister result %1% %2%", nResult, m_strUserID.GetString()));
		break;
	case 0 : // ID / PWD 불일치 (새로운 계정을 생성 또는 재입력요구)	
		NetMsgFB.m_emResult = EM_REGISTER_FB_SUB_EXIST;		
		sc::writeLogError(sc::string::format("CAgentUserRegister result %1% %2%", nResult, m_strUserID.GetString()));
		break;
	case 1 :
		NetMsgFB.m_emResult = EM_REGISTER_FB_SUB_OK;
		break;
	default : // 다른 모든경우 로그인 실패
		NetMsgFB.m_emResult = EM_REGISTER_FB_SUB_FAIL;		
		sc::writeLogError(sc::string::format("CAgentUserRegister result %1% %2%", nResult, m_strUserID.GetString()));
    ...
    pTemp->MsgRegisterBack( &NetMsgFB );
...
int CAgentUserChangePin::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);

	int nRetCode = 0;	
	int nResult = m_pDbManager->UserChangePin(m_strUserID.GetString(),
	...
	NetMsgFB.nType = CHINA_NET_MSG_PIN_FB;
	...
	StringCchCopy(NetMsgFB.szCaptcha,		USR_RAND_PASS_LENGTH+1, m_strCaptcha.GetString()); // ip
	...
	case sc::db::DB_ERROR : 
		{
			...
			sc::writeLogError(sc::string::format("CAgentUserCPIN result %d DB ERROR", nResult) );
			pTemp->MsgChangePinBack( &NetMsgFB );
		}break;
	case 0 :
		{
			NetMsgFB.nResult = EM_PIN_FB_SUB_FAIL;
			sc::writeLogError(sc::string::format("CAgentUserCPIN result %d UPDATE FAIL", nResult) );
			pTemp->MsgChangePinBack( &NetMsgFB );
		}break;
	default : 
		{
			NetMsgFB.nResult = EM_PIN_FB_SUB_OK;
			sc::writeLogError(sc::string::format("CAgentUserCPIN result ClientNum %d success!", m_dwClientNum) );
			pTemp->MsgChangePinBack( &NetMsgFB );
		}break;
	};
...
int CAgentUserChangePassword::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);

	int nRetCode = 0;	
	int nResult = m_pDbManager->UserChangePass(m_strUserID.GetString(),
	...
	NetMsgFB.nType = CHINA_NET_MSG_PASS_FB;
	NetMsgFB.nClient = m_dwClientNum;
	...
	case sc::db::DB_ERROR : 
		{
			...
			sc::writeLogError(sc::string::format("CAgentUserCPASS result %d DB ERROR", nResult));
			pTemp->MsgChangePassBack( &NetMsgFB );
		}break;
	case 0 :
		{
			NetMsgFB.nResult = EM_PASS_FB_SUB_FAIL;
			sc::writeLogError(sc::string::format("CAgentUserCPASS result %d UPDATE FAIL", nResult));
			pTemp->MsgChangePassBack( &NetMsgFB );
		}break;
	default : 
		{
			NetMsgFB.nResult = EM_PASS_FB_SUB_OK;
			sc::writeLogError(sc::string::format("CAgentUserCPASS result ClientNum %d success!", m_dwClientNum));
			pTemp->MsgChangePassBack( &NetMsgFB );
		}break;
	};
...
int CAgentUserChangeEmail::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);

	int nRetCode = 0;	
	int nResult = m_pDbManager->UserChangeEmail(m_strUserID.GetString(),
		m_strPasswd.GetString(),
		m_strPasswd2.GetString(),
		m_strUserEmail.GetString(),
		m_strCaptcha.GetString());
	...
	NetMsgFB.nType = CHINA_NET_MSG_EMAIL_FB;
	NetMsgFB.nClient = m_dwClientNum;

	StringCchCopy(NetMsgFB.szUserid,	USR_ID_LENGTH+1, m_strUserID.GetString()); // id
	StringCchCopy(NetMsgFB.szUserEmail,	USR_EMAIL_LENGTH+1, m_strUserEmail.GetString()); // id
	StringCchCopy(NetMsgFB.szUserpass,		USR_PASS_LENGTH+1, m_strPasswd.GetString()); // ip
	StringCchCopy(NetMsgFB.szUserpass2,		USR_PASS_LENGTH+1, m_strPasswd2.GetString()); // ip
	StringCchCopy(NetMsgFB.szCaptcha,		USR_RAND_PASS_LENGTH+1, m_strCaptcha.GetString()); // ip
	...
	case sc::db::DB_ERROR : 
		{
			...
			sc::writeLogError(sc::string::format("CAgentUserCEMAIL result %d DB ERROR", nResult));
			pTemp->MsgChangeEmailBack( &NetMsgFB );
		}break;
	case 0 :
		{
			NetMsgFB.nResult = EM_EMAIL_FB_SUB_FAIL;
			sc::writeLogError(sc::string::format("CAgentUserCEMAIL result %d UPDATE FAIL", nResult));
			pTemp->MsgChangeEmailBack( &NetMsgFB );
		}break;
	default : 
		{
			NetMsgFB.nResult = EM_EMAIL_FB_SUB_OK;
			sc::writeLogError(sc::string::format("CAgentUserCEMAIL result ClientNum %d success!", m_dwClientNum));
			pTemp->MsgChangeEmailBack( &NetMsgFB );
		}break;
	};
...
int CAgentUserTopUp::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);

	int nRetCode = 0;	
	int nResult = m_pDbManager->UserTopUp(m_strUserID.GetString(),
		m_strCode.GetString(),
		m_strPin.GetString(),
		m_strCaptcha.GetString());
	...
	NetMsgFB.nType = CHINA_NET_MSG_TOPUP_FB;
	NetMsgFB.nClient = m_dwClientNum;
	...
	case sc::db::DB_ERROR : 
		{
			...
			sc::writeLogError(sc::string::format("CAgentUserTOPUP result %d DB ERROR", nResult));
			pTemp->MsgTopUpBack( &NetMsgFB );
		}break;
	case 0 :
		{
			NetMsgFB.nResult = EM_TOPUP_FB_SUB_FAIL;
			sc::writeLogError(sc::string::format("CAgentUserTOPUP result %d TOPUP FAIL", nResult));
			pTemp->MsgTopUpBack( &NetMsgFB );
		}break;
	default : 
		{
			NetMsgFB.nResult = EM_TOPUP_FB_SUB_OK;
			sc::writeLogError(sc::string::format("CAgentUserTOPUP result ClientNum %d success!", m_dwClientNum));
			pTemp->MsgTopUpBack( &NetMsgFB );
		}break;
	};
...
int CAgentUserGameTimeCVT::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);

	int nRetCode = 0;	

	sc::writeLogError( "CAgentUserTOPUP" );
	int nResult = m_pDbManager->UserGameTimeCVT(m_strUserID.GetString(),
		m_dwGameTime,
		m_strCaptcha.GetString());

	// 전송할 구조체를 세팅한다.
	NET_GTCVT_FEEDBACK_DATA2 NetMsgFB;
	NetMsgFB.nType = CHINA_NET_MSG_GT2VP_FB;
	NetMsgFB.nClient = m_dwClientNum;
	...
	case sc::db::DB_ERROR : 
		{
			...
			sc::writeLogError(sc::string::format("CAgentUserGT2VP result %d DB ERROR", nResult));
			pTemp->MsgGameTimeCVTBack( &NetMsgFB );
		}break;
	case 0 :
		{
			NetMsgFB.nResult = EM_GT2VP_FB_SUB_FAIL;
			sc::writeLogError(sc::string::format("CAgentUserGT2VP result %d CVT FAIL", nResult));
			pTemp->MsgGameTimeCVTBack( &NetMsgFB );
		}break;
	default : 
		{
			NetMsgFB.nResult = EM_GT2VP_FB_SUB_OK;
			sc::writeLogError(sc::string::format("CAgentUserGT2VP result ClientNum %d success!", m_dwClientNum));
			pTemp->MsgGameTimeCVTBack( &NetMsgFB );
		}break;
	};
...
int CAgentUserPremiumPointCVT::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);

	int nRetCode = 0;	

	sc::writeLogError("CAgentUserCONVERTPP");
	int nResult = m_pDbManager->UserPremiumPointCVT(m_strUserID.GetString(),
		m_strValue.GetString(),
		m_strCaptcha.GetString());

	// 전송할 구조체를 세팅한다.
	NET_PPCVT_FEEDBACK_DATA2 NetMsgFB;
	NetMsgFB.nType = CHINA_NET_MSG_PP2VP_FB;
	NetMsgFB.nClient = m_dwClientNum;
	...
	case sc::db::DB_ERROR : 
		{
			...
			sc::writeLogError(sc::string::format("CAgentUserPP2VP result %d DB ERROR", nResult));
			pTemp->MsgPremiumPointCVTBack( &NetMsgFB );
		}break;
	case 0 :
		{
			NetMsgFB.nResult = EM_PP2VP_FB_SUB_FAIL;
			sc::writeLogError(sc::string::format("CAgentUserPP2VP result %d CVT FAIL", nResult));
			pTemp->MsgPremiumPointCVTBack( &NetMsgFB );
		}break;
	default : 
		{
			NetMsgFB.nResult = EM_PP2VP_FB_SUB_OK;
			sc::writeLogError(sc::string::format("CAgentUserPP2VP result ClientNum %d success!", m_dwClientNum));
			pTemp->MsgPremiumPointCVTBack( &NetMsgFB );
		}break;
	};
...
int CAgentUserResetPassword::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);

	int nRetCode = 0;	
	int nResult = m_pDbManager->UserResetPass(m_strUserID.GetString(),
		m_strPasswd2.GetString(),
		m_strCaptcha.GetString());

	// 전송할 구조체를 세팅한다.
	NET_RPS_FEEDBACK_DATA2 NetMsgFB;
	NetMsgFB.nType = CHINA_NET_MSG_RESETPASS_FB;
	NetMsgFB.nClient = m_dwClientNum;
	...
	case sc::db::DB_ERROR : 
		{
			...
			sc::writeLogError(sc::string::format("CAgentUserRPASS result %d DB ERROR", nResult));
			pTemp->MsgResetPassBack( &NetMsgFB );
		}break;
	case 0 :
		{
			NetMsgFB.nResult = EM_RESETPASS_FB_SUB_FAIL;
			sc::writeLogError(sc::string::format("CAgentUserRPASS result %d UPDATE FAIL", nResult));
			pTemp->MsgResetPassBack( &NetMsgFB );
		}break;
	default : 
		{
			NetMsgFB.nResult = EM_RESETPASS_FB_SUB_OK;
			sc::writeLogError(sc::string::format("CAgentUserRPASS result ClientNum %d success!", m_dwClientNum));
			pTemp->MsgResetPassBack( &NetMsgFB );
		}break;
	};
...
int CAgentUserResetPassword2::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);

	int nRetCode = 0;	
	int nResult = m_pDbManager->UserResetPass2(m_strUserID.GetString(),
		m_strPasswd.GetString(),
		m_strPasswd2.GetString(),
		m_strCaptcha.GetString());
	...
	NetMsgFB.nType = CHINA_NET_MSG_RESETPASS2_FB;
	NetMsgFB.nClient = m_dwClientNum;
...
int CAgentUserResetPin::Execute(NetServer* pServer)
{
	CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);

	int nRetCode = 0;	
	int nResult = m_pDbManager->UserResetPin(m_strUserID.GetString(),
	...
	NetMsgFB.nType = CHINA_NET_MSG_RESETPIN_FB;
	NetMsgFB.nClient = m_dwClientNum;
	...
	case sc::db::DB_ERROR : 
		{
			...
			sc::writeLogError(sc::string::format("CAgentUserRPIN result %d DB ERROR", nResult));
			pTemp->MsgResetPinBack( &NetMsgFB );
		}break;
	case 0 :
		{
			NetMsgFB.nResult = EM_RESETPIN_FB_SUB_FAIL;
			sc::writeLogError(sc::string::format("CAgentUserRPIN result %d UPDATE FAIL", nResult));
			pTemp->MsgResetPinBack( &NetMsgFB );
		}break;
	default : 
		{
			NetMsgFB.nResult = EM_RESETPIN_FB_SUB_OK;
			sc::writeLogError(sc::string::format("CAgentUserRPIN result ClientNum %d success!", m_dwClientNum) );
			pTemp->MsgResetPinBack( &NetMsgFB );
		}break;
	};
...
*/	
	...
	case 8 : // EM_LOGIN_FB_SUB_RANDOM_PASS
		...
		break;
	case 9 : 
		NetMsgFB.m_emResult = EM_LOGIN_FB_SUB_USAGE;		
...
int CAgentGsUserCheck::Execute(NetServer* pServer)
{
	...
	int nResult1 = m_pDbManager->PhUserClassCheck( m_strUserIP.c_str() );
