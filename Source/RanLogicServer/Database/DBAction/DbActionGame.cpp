...
int CharacterPointGet::Execute(NetServer* pServer)
{
	...
	__int64 PointBattle = 0;
    m_pDbManager->GetChaPoint(m_ChaDbNum, PointRefundable, PointNotRefundable, PointBattle);
    
    CHARACTER_POINT CharPoint(PointRefundable, PointNotRefundable, PointBattle);
...
ChaLockPassword::ChaLockPassword(DWORD ClientSlot, const char* ReqPassword, EMCHARLOCK_TYPE emLockType, PGLCHAR pCharacter, bool bUnlock )
: m_ReqPassword ( ReqPassword )
, m_bUnlock ( bUnlock )
, m_pCharacter ( pCharacter )
, m_emLockType ( emLockType )
{
	m_ClientSlot = ClientSlot;
}

ChaLockPassword::~ChaLockPassword()
{
}

int ChaLockPassword::Execute(NetServer* pServer)
{
	if ( m_pDbManager->GetConnectionType() != db::ADO )
	{
		sc::writeLogError( "ChaLockPassword::Execute. Use ADO." );
		return NET_ERROR;
	}

	int nRet = m_pDbManager->ChaLockPassword( m_pCharacter->CharDbNum(), m_ReqPassword.c_str(), m_emLockType, m_pCharacter, m_bUnlock );

	GLMSG::SNETPC_REQ_CHAR_LOCK_FB NetMsgDB2Fld;
	if ( nRet == sc::db::DB_OK )
	{
		NetMsgDB2Fld.bUnlock = m_bUnlock;
		NetMsgDB2Fld.bInventoryLock = m_pCharacter->m_bInventoryLock;
		NetMsgDB2Fld.bLockerLock = m_pCharacter->m_bLockerLock;
		NetMsgDB2Fld.emFB = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_FB_INVENTORY_PASSWORD_OK : EMCHARLOCK_FB_LOCKER_PASSWORD_OK;
		NetMsgDB2Fld.emReq = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_INVENTORY_PASSWORD : EMCHARLOCK_LOCKER_PASSWORD;
	}
	else
	{
		NetMsgDB2Fld.bUnlock = m_bUnlock;
		NetMsgDB2Fld.bInventoryLock = m_pCharacter->m_bInventoryLock;
		NetMsgDB2Fld.bLockerLock = m_pCharacter->m_bLockerLock;
		NetMsgDB2Fld.emFB = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_FB_INVENTORY_PASSWORD_FAIL : EMCHARLOCK_FB_LOCKER_PASSWORD_FAIL;
		NetMsgDB2Fld.emReq = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_INVENTORY_PASSWORD : EMCHARLOCK_LOCKER_PASSWORD;
		//NetMsgDB2Fld.emReason = (EMCHARLOCK_REASON);
	}
	CFieldServer* pTemp = reinterpret_cast<CFieldServer*> (pServer);
	pTemp->InsertMsg(m_ClientSlot, (char*) &NetMsgDB2Fld );
}

ChaLockChangeResetPassword::ChaLockChangeResetPassword(DWORD ClientSlot, const char* ReqPassword, const char* ReqPIN, EMCHARLOCK_TYPE emLockType, PGLCHAR pCharacter, bool bReset )
: m_ReqPassword ( ReqPassword )
, m_ReqPIN ( ReqPIN )
, m_bReset ( bReset )
, m_pCharacter ( pCharacter )
...
ChaLockChangeResetPassword::~ChaLockChangeResetPassword()
{
}

int ChaLockChangeResetPassword::Execute(NetServer* pServer)
{
	if ( m_pDbManager->GetConnectionType() != db::ADO )
	{
		sc::writeLogError( "ChaLockChangeResetPassword::Execute. Use ADO." );
		return NET_ERROR;
	}

	int nRet = m_pDbManager->ChaLockChangeResetPassword( m_pCharacter->CharDbNum(), m_ReqPassword.c_str(), m_ReqPIN.c_str(), m_emLockType, m_pCharacter, m_bReset );
	...
	if ( nRet == sc::db::DB_OK )
	{
		if ( m_bReset )
		{
			NetMsgDB2Fld.bInventoryLock = m_pCharacter->m_bInventoryLock;
			NetMsgDB2Fld.bLockerLock = m_pCharacter->m_bLockerLock;
			NetMsgDB2Fld.emFB = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_FB_INVENTORY_RESET_PASSWORD_OK : EMCHARLOCK_FB_LOCKER_RESET_PASSWORD_OK;
			NetMsgDB2Fld.emReq = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_INVENTORY_RESET_PASSWORD : EMCHARLOCK_LOCKER_RESET_PASSWORD;
		}
		else
		{
			NetMsgDB2Fld.bInventoryLock = m_pCharacter->m_bInventoryLock;
			NetMsgDB2Fld.bLockerLock = m_pCharacter->m_bLockerLock;
			NetMsgDB2Fld.emFB = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_FB_INVENTORY_CHANGE_PASSWORD_OK : EMCHARLOCK_FB_LOCKER_CHANGE_PASSWORD_OK;
			NetMsgDB2Fld.emReq = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_INVENTORY_CHANGE_PASSWORD : EMCHARLOCK_LOCKER_CHANGE_PASSWORD;
	...
	else
	{
		if ( m_bReset )
		{
			NetMsgDB2Fld.bReset = m_bReset;
			NetMsgDB2Fld.bInventoryLock = m_pCharacter->m_bInventoryLock;
			NetMsgDB2Fld.bLockerLock = m_pCharacter->m_bLockerLock;
			NetMsgDB2Fld.emFB = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_FB_INVENTORY_RESET_PASSWORD_FAIL : EMCHARLOCK_FB_LOCKER_RESET_PASSWORD_FAIL;
			NetMsgDB2Fld.emReq = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_INVENTORY_RESET_PASSWORD : EMCHARLOCK_LOCKER_RESET_PASSWORD;
		}
		else
		{
			...
			NetMsgDB2Fld.bInventoryLock = m_pCharacter->m_bInventoryLock;
			NetMsgDB2Fld.bLockerLock = m_pCharacter->m_bLockerLock;
			NetMsgDB2Fld.emFB = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_FB_INVENTORY_CHANGE_PASSWORD_FAIL : EMCHARLOCK_FB_LOCKER_CHANGE_PASSWORD_FAIL;
			NetMsgDB2Fld.emReq = (m_emLockType==EMCHARLOCK_TYPE_INVENTORY) ? EMCHARLOCK_INVENTORY_CHANGE_PASSWORD : EMCHARLOCK_LOCKER_CHANGE_PASSWORD;

		}
		//NetMsgDB2Fld.emReason = (EMCHARLOCK_REASON)nRet - 2;
	}
	CFieldServer* pTemp = reinterpret_cast<CFieldServer*> (pServer);
	pTemp->InsertMsg(m_ClientSlot, (char*) &NetMsgDB2Fld );
}

CGetRankings::CGetRankings(
						   DWORD dwCharID )
{
	m_dwCharID = dwCharID;
}

int CGetRankings::Execute( NetServer* pServer )
{
	m_pDbManager->GetRankings( m_vRank , true , false , false );

	CFieldServer* pTemp = reinterpret_cast<CFieldServer*> (pServer);
	if ( !pTemp ) return NET_ERROR;

	PGLCHAR pChar = pTemp->GetGLGaeaServer()->GetCharByDbNum( m_dwCharID );
	if ( pChar )
	{
		if (pChar->SETRANKINGS( m_vRank , true , false , false ))
			pChar->SENDRANKINGSINFO(true,false,false,true);
	}

	m_pDbManager->GetRankings( m_vRankPk, false , true , false );
	if ( pChar )
	{
		if (pChar->SETRANKINGS( m_vRankPk , false , true , false ))
			pChar->SENDRANKINGSINFO(false,true,false,false);
	}

	m_pDbManager->GetRankings( m_vRankRich, false , false , true );

	if ( pChar )
	{
		if ( pChar->SETRANKINGS( m_vRankRich , false , false , true ) )
			pChar->SENDRANKINGSINFO(false,false,true,false);
	}
