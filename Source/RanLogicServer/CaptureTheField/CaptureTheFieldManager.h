...
protected:
	...
	std::map<DWORD,PVP::CTF_RANKER>	m_mapClubRanking;		//Club-ranking map based on PvP performance
	std::map<DWORD,PVP::CTF_RANKER>	m_mapRanking;		//Club-ranking map based on PvP performance
	PVP::CTF_RANKER* GetRanker ( DWORD dwCharID );
	bool DeleteRanker ( DWORD dwCharID );
	PVP::CTF_RANKER* GetClubRanker ( DWORD dwClubID );
public:
	bool AddRanker ( DWORD dwCharID );
};
