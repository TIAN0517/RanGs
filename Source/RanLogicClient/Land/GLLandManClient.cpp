...
#include "../../RanGfxUI/OldUI/Display/NameDisplayMan.h"
#include "../../RanGfxUI/OldUI/Display/NameDisplay.h"
...
void GLLandManClient::RegistName( const CLIPVOLUME &CV )
{
	...
	INameDisplayMan *pDISP_NAME_MAN = m_pGaeaClient->GetInterface()->GetDispName();
	...
	if (pMYPET->IsVALID())
	{
		if ( pMYPET->IsVisibleDisplayName() )
		{
			...
			if ( pDISP_NAME_MAN ) pDISP_NAME_MAN->ADD_DISP_NAME ( sDISP_NAME, pMyCharacter->GetName() );
		}
	...
	// 소환수들의 Render 처리
	//!! SF_TODO
	m_pGaeaClient->GetInterface()->SummonSetCount(m_dwCOUNT);
			...
			if ( spSummon->IsVisibleDisplayName() )
			{
				...
				else
				{
					...
					if (pCHAR)
					{
						...
						if ( pDISP_NAME_MAN ) pDISP_NAME_MAN->ADD_DISP_NAME ( sDISP_NAME, pCHAR->GetName() );
					}
				}
			}
		}
