...
void GLGaeaClient::MsgClubDeathMatchPointUpdate(NET_MSG_GENERIC* pMsg)
{
    ...
    //	UI Refrash
	//!! SF_TODO
    GetInterface()->VisibleCDMRanking(true);
    GetInterface()->RefreashCDMRanking();
}
...
void GLGaeaClient::MsgServerClubDeathMatchInfo(NET_MSG_GENERIC* pMsg)
{
    ...
    {
		//!! SF_TODO
       GetInterface()->SetVisibleMiniMapClubDeathMatchTime( TRUE );
    }
    else
    {
        m_fClubDeathMatchTimer = 0.0f;
		//!! SF_TODO
       GetInterface()->SetVisibleMiniMapClubDeathMatchTime( FALSE );
        // 적대관계 모두 삭제
...
// 			pNetMsg->m_ReqChaDbNum,
		...
		GetInterface()->DOMODAL_CLUB_BATTLE_ARMISTICE_ASK( pNetMsg->dwClubID,
			pNetMsg->m_ReqChaDbNum,
			pClubBattle->m_szClubName,
			false );
	}
...
// 		NSWIDGET_SCRIPT::g_strFunc_Club_UpdateBattleRecords );
	...
	//!! SF_TODO
    GetInterface()->REFRESH_CLUB_STATE();
}
...
// 		pNetMsg->dwBattleTime,
	...
	GetInterface()->DOMODAL_CLUB_BATTLE_ASK( pNetMsg->dwClubCharID,
		pNetMsg->szClubName,
		pNetMsg->dwBattleTime,
		false );
}
...
void GLGaeaClient::MsgClubGuidBattleRemainBrd(NET_MSG_GENERIC* pMsg)
{
	...
	//!! SF_TODO
   GetInterface()->SetClubGuidBattleName( m_vecClubGuidBattleName );
}
...
void GLGaeaClient::MsgServerClubGuidBattleInfo(NET_MSG_GENERIC* pMsg)
{
    ...
    {
		//!! SF_TODO
        GetInterface()->SetVisibleMiniMapClubGuidBattleTime( TRUE );
    }
    else
    {
		//!! SF_TODO
        GetInterface()->SetVisibleMiniMapClubGuidBattleTime( FALSE );
        // 적대관계 모두 삭제
        GetCharacter()->DEL_PLAYHOSTILE_ALL();
    }
}
