...
public:
    ...
    bool InsertItem(SNATIVEID ItemId, __int64 ItemPrice, WORD Currency);
	...
	std::tr1::tuple<bool, __int64> ExistItem_C(const SNATIVEID& ItemId);
    
    //! 아이템가격
    //! 결과값이 0 이면 판매하지 않는 아이템이다
	__int64 ItemPrice(const SNATIVEID& ItemId);
	__int64 ItemCurrency(const SNATIVEID& ItemId);
