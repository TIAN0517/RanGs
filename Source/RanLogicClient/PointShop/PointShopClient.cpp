...
CREATE INDEX Idx_ItemCurrency  ON PointShopItem (ItemCurrency); ";
             ...
             pParent = (ticpp::Element*) pParent->NextSiblingElement(false))
        {
                 ...
                 pChild = (ticpp::Element*) pChild->NextSiblingElement(false))
            {
                ...
                if (ValueName == "ITEM")
                {
					...
					pChild->GetAttribute("C", &Currency);
                    InsertItem(SNATIVEID(ItemMid, ItemSid), Price, Currency);
                }
            }
        }
    }
...
{
    return InsertItem(Item.ItemId(), Item.Price(), Item.Currency());
}

bool PointShopClient::InsertItem(SNATIVEID ItemId, __int64 ItemPrice, WORD Currency)
{
	...
	if ( Currency < 1 || Currency > 2 )
	{
		...
		sc::writeLogError(
			sc::string::format(
			"PointShopClient::InsertItem Currency %1%",
			Currency));
		return false;
	}
    ...
    std::string Query(
        sc::string::format(
        "INSERT INTO PointShopItem(ItemMid, ItemSid, ItemPrice, ItemType, ItemSuit, ItemName, ItemCurrency) VALUES (%1%, %2%, %3%, %4%, %5%, '%6%', %7%);",
        ItemId.Mid(),
        ItemId.Sid(),
        ItemPrice,
        pItem->BasicType(),
...
{    
    std::string Query("SELECT ItemMid, ItemSid, ItemPrice, ItemType, ItemSuit, ItemName, ItemCurrency FROM PointShopItem");
    ...
    if (pTable.m_pTable)
    {
        ...
        for (int loop=0; loop<nRowCount; ++loop)
        {
			...
			WORD Currency         = rTable.GetValue<WORD>(6);

            SellItemClient Item(Mid, Sid, Price, Type, Suit, Name, Currency);
...
__int64 PointShopClient::ItemCurrency(const SNATIVEID& ItemId)
{
	std::tr1::tuple<bool, __int64> Result = ExistItem_C(ItemId);    
	return std::tr1::get<1>(Result);
}
...
//! 판매하는 아이템인가?
std::tr1::tuple<bool, __int64> PointShopClient::ExistItem_C(const SNATIVEID& ItemId)
{
	std::string Query(
		sc::string::format(
		"SELECT ItemCurrency FROM PointShopItem WHERE ItemMid=%1% AND ItemSid=%2%",
		ItemId.Mid(),
		ItemId.Sid()));
...
const int PointShopClient::AddCartList( const SITEMCUSTOM &sItemCustom )
{
	...
	__int64 nItemCurrency = ItemCurrency(sItemCustom.GetNativeID());
	
	//가격이 0인 경우는 판매하지 않는 아이템
	if( 0 == nItemPrice) 
		return -1;

	m_vecCartList.push_back( SellItem( sItemCustom.GetNativeID(), nItemPrice, nItemCurrency) );
