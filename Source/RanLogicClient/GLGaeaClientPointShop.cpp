...
// #endif
	...
	GetInterface()->SetForceClosePointShopWindow( TRUE );
}
...
// #endif
	...
	GetInterface()->PointShopSetPoint( m_pCharacter->PointShopPoint() );
	GetInterface()->PointShopSetBattlePoint( m_pCharacter->PointShopBattlePoint() );
}
...
bool GLGaeaClient::PointShopBuyItem(const SNATIVEID& ItemId)
{
	...
	std::tr1::tuple< bool, __int64 > Result2 = m_pPointShop->ExistItem_C( ItemId );
	if ( false == std::tr1::get< 0 >( Result2 ) )
	{
		PrintConsoleText(
			sc::string::format(
			ID2GAMEINTEXT("POINT_SHOP_NO_SELL_ITEM"),
	...
	__int64 ItemCurrency = std::tr1::get< 1 >( Result2 );
	//__int64 ChaPoint = m_pCharacter->PointShopPoint();
	__int64 ChaPoint = ( ItemCurrency == pointshop::POINT_SHOP_PREMIUM_POINT ) ? m_pCharacter->PointShopPoint() : m_pCharacter->PointShopBattlePoint();
	...
	m_pInterface->GetModalWindow()->SetModalData( ItemId.Mid(),	ItemId.Sid() );
	m_pInterface->DoModal( sc::string::format ( ID2GAMEINTEXT("MODAL_POINT_SHOP_BUY") , pItem->GetName(), ( ItemCurrency == pointshop::POINT_SHOP_PREMIUM_POINT ) ? sc::string::format (ID2GAMEINTEXT("POINT_SHOP_PRICE"), ItemPrice).c_str() : sc::string::format (ID2GAMEINTEXT("BATTLEPOINT_SHOP_PRICE"), ItemPrice).c_str()  ), UI::MODAL_QUESTION, UI::YESNO, UI::MODAL_POINT_SHOP_BUY );
    return true;
}
...
void GLGaeaClient::MsgPointShopBuyItemCF(NET_MSG_GENERIC* pMsg)
{
	...
	m_pInterface->ProgressDisplayShowPulblic(Message);
	m_pInterface->PointShopResetCartItem(pPacket->BuyItem());
}
