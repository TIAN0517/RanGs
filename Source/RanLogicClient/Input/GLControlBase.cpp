...
void GLControlBase::Input( float fTime, float fElapsedTime )
{
		...
		for ( ; iter!=iterEnd; ++iter )
		{
			...
			if ( !CheckDuplicationShockcut(sKey) )
			{
				...
				if ( (dwKeyState != 0) && (dwKeyState != DXKEY_IDLE) )
				{
					...
					if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup(KEY_SETTING_WINDOW) )
					{
					    pKeyCaps->fnProc( this, (*pKeyCaps) );
					    pKeyCaps->fElapsedTime = 0.0f;
					    bNonEvent = false;
	...
	if ( !pInputString || pInputString->IsOn() )
        return;
...
const bool GLControlBase::IsShortcutsBlock( float fTime, float fElapsedTime )
{
	...
	if ( m_pGaeaClient->GetTutorial()->IsTutorial() && !m_pGaeaClient->GetTutorial()->IsCanShortcutTutorial() )
	{
		return true;
	}
...
void GLControlBase::DefaultCameraUpdate( float fTime, float fElapsedTime )
{
	...
	{
		cViewPort.SetTutorialCamRot(vRotate);
		cViewPort.SetTutorialCameraPos(vRotate);
		cViewPort.SetTutorialCamZoom(fZoom);
	}
}
...
BOOL GLControlBase::CharBlockCursorUpdate(void)
{
	...
	if ( NSGUI::IsCharMoveBlock() )  
	{
		if ( m_pGaeaClient->GetInterface()->IsItemDecompose() )
...
void GLControlBase::DropHoldItem()
{
	...
	if( true == holditem::HaveHoldItem() && ( dwDropKeyStateTotal & ( DXKEY_DOWNED | DXKEY_UP | DXKEY_DUP ) ) )
	{
		...
		if( pCharacter->CheckHoldToField( vDropPos ) == S_OK )
		{
			...
			else if ( pItem->BasicType() == ITEM_QITEM )
			{
			...
			else
			{
				...
				if ( pItem->GetItemLevel() >= EMITEM_LEVEL_RARE )
				{
					m_pGaeaClient->GetInterface()->ModalWindowSetModalData ( vDropPos );
					m_pGaeaClient->GetInterface()->DoModal( ID2GAMEINTEXT("MODAL_REQ_HOLD_TO_FILED"), UI::MODAL_QUESTION, UI::OKCANCEL, UI::MODAL_REQ_HOLD_TO_FILED );
				}
				else
				{
					pCharacter->ReqHoldToField( vDropPos );
...
const bool GLControlBase::IsCameraBlock()
{
	...
	if ( !m_pGaeaClient->GetTutorial()->IsCanMouseControlInTutorial() ) return true;
