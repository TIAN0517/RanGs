...
void GLCaptureTheFieldClient::AddResultDataClub( const SCTFRESULTDATA& sData )
{
	SCTFRESULTDATA* pNewData = new SCTFRESULTDATA(sData);

	m_vecResultDataClub.push_back( pNewData );

	if ( m_vecResultDataClub.size() > 1 )
	{
		using namespace boost::lambda;
		std::inplace_merge( m_vecResultDataClub.begin(), m_vecResultDataClub.end()-1, m_vecResultDataClub.end(), (*boost::lambda::_1) < (*boost::lambda::_2) );
	}

	//! 이미 정보가 있다면 랭킹을 구한다
	if ( !m_MyClubResultData.IsNull() )
	{
		UINT nRankAdd = (sData < m_MyClubResultData ) ? 1 : 0;

		m_MyResultRankingClub += nRankAdd;
	}
	//! 이름이 같다면 나의 정보에 값을 넣는다.
	else if ( m_pGaeaClient->GetCharacter()->GetClubName() == pNewData->strNAME )
	{
		m_MyClubResultData = (*pNewData);

		//! 현재 나의 랭킹을 구함
		using namespace boost::lambda;
		VEC_RESULTDATA_ITER finditer = std::find_if( m_vecResultDataClub.begin(), m_vecResultDataClub.end(), (*boost::lambda::_1) == sData );
		m_MyResultRankingClub = (UINT)std::distance(m_vecResultDataClub.begin(), finditer);
	}
}
