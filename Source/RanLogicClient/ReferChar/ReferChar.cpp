...
namespace ReferChar
{
	...
	bool CClient::Request( int nServer, const std::string& strName )
	{
		...
		/*if( SQLiteSelectData( m_nReferServer, m_sData, m_sDataAddition ) )
		{
			m_bComplete = true;
			m_pGaeaClient->GetInterface()->UiRefresh( REFERCHAR_WINDOW );
		}*/
	...
	void CClient::ReceiveMsgInfo( NET_MSG_GENERIC* pMsg )
	{
		...
		//m_pGaeaClient->GetInterface()->ReferCharWindowOpen( m_sData.m_szName );
	}
