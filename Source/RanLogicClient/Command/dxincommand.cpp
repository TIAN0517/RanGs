...
bool GameCommand::command( const std::string& Command )
{
	...
	else if ( strCommand == "/dex" )
	{
		...
		DWORD value = ( DWORD ) atoi( strPARAM_01.c_str() );
		m_pGaeaClient->GetCharacter()->ReqStatsUpCmd( EMDEX, value);
		return true;
	}
	else if ( strCommand == "/pow" ) 
	{
		size_t VecSize = SplitVec.size();
		if ( VecSize < 2 )
		{
		...
		DWORD value = ( DWORD ) atoi( strPARAM_01.c_str() );
		m_pGaeaClient->GetCharacter()->ReqStatsUpCmd( EMPOW, value);
		return true;
	}
	else if ( strCommand == "/int" ) 
	{
		...
		DWORD value = ( DWORD ) atoi( strPARAM_01.c_str() );
		m_pGaeaClient->GetCharacter()->ReqStatsUpCmd( EMSPI, value);
		return true;
	}
	else if ( strCommand == "/vit" )
	{
		...
		DWORD value = ( DWORD ) atoi( strPARAM_01.c_str() );
		m_pGaeaClient->GetCharacter()->ReqStatsUpCmd( EMSTR, value);
		return true;
	}
	else if ( strCommand == "/stm" )
	{
		...
		DWORD value = ( DWORD ) atoi( strPARAM_01.c_str() );
		m_pGaeaClient->GetCharacter()->ReqStatsUpCmd( EMSTA, value);
		return true;
	}
