...
//		GLWidgetScript::GetInstance().LuaCallFunc< void >(
		...
		m_pInterface->ProgressDisplayShowPulblic( strAlarm );
	}
...
// 		NSWIDGET_SCRIPT::g_strFunc_Competition_UpdateCTF );
	m_pInterface->CompetitionCTFUpdate();
 }
...
// 		NSWIDGET_SCRIPT::g_strFunc_CTFNotifyInfo, "-n", (int)UI::EMPROGRESSDISPLAY_TYPE_CTF_START);
	...
	m_pInterface->ProgressDisplayShow(UI::EMPROGRESSDISPLAY_TYPE_CTF_START,"");
}
...
// 	GLWidgetScript::GetInstance().LuaCallFunc< void >(
	...
	m_pInterface->ProgressDisplayShow(UI::EMPROGRESSDISPLAY_TYPE_CTF_STOP,"");
// 
// 	GLWidgetScript::GetInstance().LuaCallFunc< void >(
...
void GLGaeaClient::MsgCaptureTheFieldStatistics(NET_MSG_GENERIC* pMsg)
{
	...
	for ( UINT i=0; i<pPacket->nHowMany; ++i )
	{
		...
		if ( Data.dwClubDBNum != CLUB_NULL )	
		{
			sResultData.strNAME		  = Data.szClubName;
			sResultData.dwClubID	  = Data.dwClubDBNum;
		...
		if ( Data.dwClubDBNum != CLUB_NULL )	m_pCaptureTheField->AddResultDataClub( sResultData );
		else m_pCaptureTheField->AddResultData( sResultData );
	}
...
void GLGaeaClient::MsgCaptureTheFieldRankingToAll(NET_MSG_GENERIC* pMsg)
{
	...
	sPrevState.vecRESULTDATACLUB.clear();

	GLMSG::SNET_CTF_RANKING_FAC *pPacket = (GLMSG::SNET_CTF_RANKING_FAC *)pMsg;

	for ( UINT i=0; i<pPacket->nHowMany; ++i )
	{
		if ( pPacket->arrRanking[i].dwClubID != CLUB_NULL )
				sPrevState.vecRESULTDATACLUB.push_back(pPacket->arrRanking[i]);
		else	sPrevState.vecRESULTDATA.push_back(pPacket->arrRanking[i]);
	}
}
