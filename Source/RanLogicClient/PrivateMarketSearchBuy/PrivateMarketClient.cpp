...
void PrivateMarketClient::LoadSearchFileFromITEM()
{
	...
	if ( dwFILEVER>=GLItemMan::ENCODE_VER )
		SFile.SetEncodeType(EMBYTECRYPT_OLD);

	if ( dwFILEVER>=GLItemMan::VERSION )
	...
	for (DWORD i=0; i<dwNum; i++)
	{
		...
		{
			AfxMessageBox( sc::string::format("There was a problem with Loading SITEM. Ver %d Encode Ver %d ( Current Ver %d ) %s",dwFILEVER,
			GLItemMan::ENCODE_VER,GLItemMan::VERSION,GLogicData::GetInstance().GetItemFileName()).c_str() );
			return;	
		}
