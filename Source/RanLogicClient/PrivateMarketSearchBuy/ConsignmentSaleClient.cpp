...
void ConsignmentSaleClient::Reset()
{
	...
	m_pGaeaClient->GetInterface()->SetPageWaiting( false );
} 
...
{   
	...
	if( private_market::GLPrivateMarketDefine::Instance()->GoldTrade() == false && (SellType == private_market::eSellType_ConsignmentSale_Gold || SellType == private_market::eSellType_ConsignmentSale_GoldBet ) )
		return;
...
void ConsignmentSaleClient::AckRegistItem( const private_market::ConsignmentSaleItem& SaleItem )
{
    ...
    else
    {
		...
		//!! SF_TODO
        m_pGaeaClient->GetInterface()->PrivattMarketWindowRanPointPageRefresh();
    }
