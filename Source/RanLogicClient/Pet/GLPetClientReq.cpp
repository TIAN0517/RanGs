...
void GLPetClient::ReqInputName ( WORD wPosX, WORD wPosY )
{
	...
	m_pGaeaClient->GetInterface()->DoModal ( ID2GAMEINTEXT("INVEN_CHAR_RENAME"), UI::MODAL_INPUT, UI::EDITBOX, UI::MODAL_PET_RENAME );
...
void GLPetClient::ReqInputColor ( WORD wPosX, WORD wPosY )
{
	...
	m_pGaeaClient->GetInterface()->DoModal ( ID2GAMEINTEXT("MODAL_PETCOLOR_INFO"), UI::MODAL_INFOMATION, UI::OK, UI::MODAL_PETCOLOR_INFO );
...
void GLPetClient::ReqInputStyle ( WORD wPosX, WORD wPosY )
{
	...
	m_pGaeaClient->GetInterface()->UiShowGroupFocus( PETSTYLECARD_WINDOW );
            ...
            else if ( sTarget.emCrow == CROW_ITEM )
            {
				...
				if ( pItem->sBasicOp.emItemType==ITEM_QITEM ) 
				{
					if ( dwGetItemFlag&EMPETSKILL_GETQBOX )							// Q박스 비트연산 
					{
						...
						if(  true == m_pGaeaClient->GetInterface()->QBoxButtonGetEnable() )	// QBox Lock이 걸려 있지 않는 경우
							bDetect = true;
					}
					//bDetect = true;
					//확인용 코드
