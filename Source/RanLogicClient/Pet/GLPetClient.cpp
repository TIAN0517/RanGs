...
HRESULT GLPetClient::Create ( const PGLPET pPetData, D3DXVECTOR3 vPos, D3DXVECTOR3 vDir, boost::shared_ptr<NavigationMesh> spNavi, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	//!! SF_TODO
	m_pGaeaClient->GetInterface()->SetPetHungryItem( m_pOwner->m_sPetCardNativeID );
	m_pGaeaClient->GetInterface()->SetPetHungry();
	m_pGaeaClient->GetInterface()->UiShowGroupBottom( PET_HUNGRY );
...
void GLPetClient::Delete()
{
	...
	m_pGaeaClient->GetInterface()->UiHideGroup( PET_HUNGRY );
}
...
HRESULT GLPetClient::FrameMove( GLLandManClient* pLandManClient, float fTime, float fElapsedTime )
{
	...
	m_pGaeaClient->GetInterface()->PetSatiationUpdate ( m_nFull );
