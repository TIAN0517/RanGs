...
{
	//!! SF_TODO
	m_pGaeaClient->GetInterface()->ToggleWindowTenderItem(false);
	m_pGaeaClient->GetInterface()->ToggleWindowMasterTransfer(false);
...
void GLPartyTenderManagerClient::FrameMove_LeaderShare(const GLPartyID& PartyID)
{
	...
	if ( pPartyManager->isMaster() == false )
	{
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->ToggleWindowMasterTransfer(false);
		m_pGaeaClient->GetInterface()->OpenPartyTenderNotifyButton(false);
		return;
	}

	//!! SF_TODO
	if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup(PARTY_DISTRIBUTION_WINDOW) == TRUE )
		return;
	...
	{		
		...
		{
			//!! SF_TODO
			m_pGaeaClient->GetInterface()->PartyTenderNotify( true );
			return;
		}
	}

	//!! SF_TODO
	m_pGaeaClient->GetInterface()->PartyTenderNotify( false );
}

void GLPartyTenderManagerClient::FrameMove_ItemDice(void)
{
	//!! SF_TODO
	if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup(PARTY_TENDER_WINDOW) == TRUE )
		return;
...
{	
	...
	//!! SF_TODO
 	if( bItemNew )
	{
		m_pGaeaClient->GetInterface()->OpenPartyTenderNotifyButton(true);
		m_pGaeaClient->GetInterface()->PartyTenderNotify( true );
 	}
	if ( !bIsRangeItem )
	{
...
{
	//!! SF_TODO
	if( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup(PARTY_TENDER_WINDOW) && !m_pGaeaClient->GetInterface()->UiIsVisibleGroup(PARTY_DISTRIBUTION_WINDOW) )
		return;
