...
BOOL GLPartyClient::MsgPartyFNewUpdate(NET_MSG_GENERIC* nmg)
{
	...
	m_bQBoxEnable = true;
	//!! SF_TODO
	//m_pGaeaClient->GetInterface()->QBoxButtonSetEnable(true);
...
BOOL GLPartyClient::MsgPartyLinkJoinANS( NET_MSG_GENERIC* nmg )
{
	...
	//	링크 정보를 셋팅한다.
	{
		...
		NetChatFB.emType = CHAT_TYPE_PARTY_RECRUIT;
...
BOOL GLPartyClient::MsgPartyMbrItemPickUpLink(NET_MSG_GENERIC* nmg)
{
	...
	//!! SF_TODO
	m_pGaeaClient->GetInterface()->DisplayChatPartyPickupLinkMessage( pMember->m_szName, pNetMsg->sItem );
