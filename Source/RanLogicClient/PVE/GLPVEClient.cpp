...
void GLPVEClient::AckPVEEntrance(NET_MSG_GENERIC* pMsg)
{
    ...
    if (CastNetMsg(pMsg, NetMsg))
    {
        ...
        if( m_pGaeaClient )
        {
			...
			if (NetMsg.byResult == GLMSG::ENTRANCE_ACK_NONE)
				//m_pGaeaClient->GetInterface()->SetPVEEntranceState( true, m_indun.dwMapID );
				m_pGaeaClient->GetInterface()->SetPVEEntranceConfirm(true);
			else
				m_pGaeaClient->GetInterface()->SetPVEEntranceConfirm(false);
        }
    }
}
...
void GLPVEClient::MsgPVEStageResult(NET_MSG_GENERIC* pMsg)
{
    ...
    if (CastNetMsg(pMsg, NetMsg))
    {
		...
		//	, "-n", NetMsg.nTimer);
		m_pGaeaClient->GetInterface()->SetPVEStageResult();
    }
}
...
void GLPVEClient::FrameMove(float fElapsedTime)
{
    ...
    if (fTimer >= 1.f)
    {
		...
		m_pGaeaClient->GetInterface()->UpdatePVEMapTable();
    }
