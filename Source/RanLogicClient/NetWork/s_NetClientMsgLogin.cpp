...
int CNetClient::SndRegister(const TCHAR* szUserID, const TCHAR* szUserEmail, const TCHAR* szUserPassword, const TCHAR* szUserPassword2, const TCHAR* szRandomPassword, int nChannel )
{
	if (nChannel < 0 || nChannel >= MAX_CHANNEL_NUMBER)
		nChannel = 0;

	NET_REGISTER_DATA nld(nChannel,szRandomPassword,szUserPassword,szUserPassword2,szUserID,szUserEmail);
		
	nld.m_nChannel  = nChannel;
	...
	// 얻어진 해쉬값을 복사해 넣는다.
	StringCchCopy( nld.m_szUserid,   USR_ID_LENGTH+1, szUserID ); // ID 를 복사해 넣는다.
	StringCchCopy( nld.m_szUserEmail,   USR_EMAIL_LENGTH+1, szUserEmail ); // ID 를 복사해 넣는다.
	StringCchCopy( nld.m_szPassword, USR_PASS_LENGTH, szUserPassword );
	StringCchCopy( nld.m_szPassword2, USR_PASS_LENGTH, szUserPassword2 );
	StringCchCopy( nld.m_szRandomPassword, USR_RAND_PASS_LENGTH+1, szRandomPassword);
	
	// 넣어진 모든 값들을 암호화 한다.
	m_Tea.encrypt( nld.m_szUserid, USR_ID_LENGTH+1 );
	m_Tea.encrypt( nld.m_szUserEmail, USR_EMAIL_LENGTH+1 );
	m_Tea.encrypt( nld.m_szPassword, USR_PASS_LENGTH );
	m_Tea.encrypt( nld.m_szPassword2, USR_PASS_LENGTH );
	m_Tea.encrypt( nld.m_szRandomPassword, USR_RAND_PASS_LENGTH+1 );
							 ...
							 const TCHAR* szCaptcha )
{
	...
	StringCchCopy( nld.szCaptcha,   USR_RAND_PASS_LENGTH+1, szCaptcha ); 
	...
	m_Tea.encrypt( nld.szCaptcha, USR_RAND_PASS_LENGTH+1 );
	m_Tea.encrypt( nld.szPassword, USR_PASS_LENGTH );
							   ...
							   const TCHAR* szCaptcha )
{
	...
	StringCchCopy( nld.szCaptcha,   USR_RAND_PASS_LENGTH+1, szCaptcha ); 
	StringCchCopy( nld.szUserEmail,   USR_EMAIL_LENGTH+1, szUserEmail ); 
	StringCchCopy( nld.szPassword, USR_PASS_LENGTH, szUserPassword ); //strPass.c_str()
	StringCchCopy( nld.szPassword2, USR_PASS_LENGTH, szUserPassword2 ); //strPass2.c_str()

	m_Tea.encrypt( nld.szCaptcha, USR_RAND_PASS_LENGTH+1 );
	m_Tea.encrypt( nld.szUserEmail, USR_EMAIL_LENGTH+1 );
	m_Tea.encrypt( nld.szPassword, USR_PASS_LENGTH );
	m_Tea.encrypt( nld.szPassword2, USR_PASS_LENGTH );
								   ...
								   const TCHAR* szCaptcha )
{
	...
	StringCchCopy( nld.szValue,   USR_RAND_PASS_LENGTH+1, szValue ); 
	StringCchCopy( nld.szCaptcha,   USR_RAND_PASS_LENGTH+1, szCaptcha ); 

	m_Tea.encrypt( nld.szValue, USR_RAND_PASS_LENGTH+1 );
	m_Tea.encrypt( nld.szCaptcha, USR_RAND_PASS_LENGTH+1 );
	return Send((char *) &nld);
}
							...
							const TCHAR* szCaptcha )
{
	...
	CString str;
	if ( sc::string::generateRandomString( str, 8 , 3, 2, 3 ) )
	{	
