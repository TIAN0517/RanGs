...
HRESULT DxGameStage::ReSizeWindow ( WORD wWidth, WORD wHeight )
{
	...
	//GfxInterfaceBase::GetInstance()->SetResolution(MAKELONG(wHeight,wWidth));
	m_pGaeaClient->GetInterface()->UiSetResolution(MAKELONG(wHeight,wWidth));
...
// 	    RANPARAM::SETCHARID(m_CharJoinData.m_CharData2.m_szName);
	...
	if ( m_pGaeaClient->GetCharacter()->m_dwUserLvl >= USER_GM3 ) DxViewPort::GetInstance().SetZoomOut( 400.0f );
...
HRESULT DxGameStage::FinalCleanup ()
{
	...
	m_pGaeaClient->GetInterface()->UiFinalCleanup();
...
HRESULT DxGameStage::FrameMove ( float m_fTime, float m_fElapsedTime, BOOL bNotRendering )
{
	...
	m_pGaeaClient->GetInterface()->UiFrameMove(m_pd3dDevice, m_fElapsedTime );
...
HRESULT DxGameStage::Render ()
{
	...
	{
		PROFILE_BEGIN("CInnerInterface::Render");
		m_pGaeaClient->GetInterface()->UiRender( m_pd3dDevice );
		PROFILE_END("CInnerInterface::Render");
	}
...
void DxGameStage::MsgProcess (NET_MSG_GENERIC* nmg)
{
	...
	case NET_MSG_CHAT_BLOCK :
		{
			if( m_pGlobalStage->GetActiveStage() == m_pGlobalStage->GetGameStage() )	m_pGaeaClient->MsgGame ( nmg );
...
void DxGameStage::MsgLobbyCharPutonEx(NET_MSG_GENERIC* nmg)
{
    ...
    BOOST_FOREACH( const GLMSG::CHARPUTON_EX_PACK_DATA& value, vecRecv )
    {
		...
		if ( m_pCharJoin->CheckReceive(m_bLoginProcessLog) )
		{
			...
			m_pGlobalStage->GetLobyStage()->MsgProcess( &nmg );
		}
    }
