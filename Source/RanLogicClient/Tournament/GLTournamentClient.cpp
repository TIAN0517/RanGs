...
/*
	...
	void GLTouranmentClient::FrameMove(float fElapsedTime)
	{
		...
		//!! SF_TODO
		if(m_pGaeaClient->GetInterface()->UiIsVisibleGroup(COMPETITION_TOURNAMENTENT_INDICATOR))
		{
			...
			while(true)
			{
				...
				if(m_pGaeaClient->GetInterface()->GetIndicatorTime() == _sec)
					break;
				else
					m_pGaeaClient->GetInterface()->SetIndicatorTime(_sec,isBlink,true);
...
// 	}
	...
	const bool CALLBACK GLTouranmentClient::_messageProcedure_InfoUpdate(const GLMSG::NET_MATCHING_MESSAGE* const _pMessage)
	{
        ...
        if ( _mapStatus == EMMATCHING_CONTENTS_OPENED )
        {
			...
			//!! SF_TODO
            m_pGaeaClient->GetInterface()->CompetitionNotifyButtonSetAlarm(_scriptID, sc::string::format( ID2GAMEWORD("COMEPTITION_NOTIFY_BUTTON", 0), szName).c_str());
        }
        else
        {
			//!! SF_TODO
            m_pGaeaClient->GetInterface()->CompetitionNotifyButtonSetAlarm(_scriptID, NULL);
        }
	...
	/*	GLWidgetScript::GetInstance().LuaCallFunc< void >(
		...
		m_pGaeaClient->GetInterface()->SetFightBegin( int( _pStartMessage->Counting ) );
	...
	const bool GLTouranmentClient::_messageProcedure_Tournament_ClientINFO(const GLMSG::NET_MATCHING_MESSAGE* const _pMessage)
	{
		...
		for (;biter != m_ClientMatchingStatus.end(); ++biter )
		{
			...
			if(_Node->ScriptID == _pStartMessage->ScriptID)
			{
                    ...
                    if ( _Node->ClientStatus != _pStartMessage->MapStatus )
                    {
                        ...
                        for (;_infoIter != m_VecMatchingInfo.end(); ++_infoIter )
                        {
                            ...
                            if ( _info->m_ScriptID == _pStartMessage->ScriptID )
                            {
								...
								//!! SF_TODO
                                m_pGaeaClient->GetInterface()->CompetitionNotifyButtonSetAlarm(_pStartMessage->ScriptID, sc::string::format( ID2GAMEWORD("COMEPTITION_NOTIFY_BUTTON", 1), szName).c_str());
                                break;
                            }
                        }
                    }
                ...
                {
					//!! SF_TODO
                    m_pGaeaClient->GetInterface()->CompetitionNotifyButtonSetAlarm(_pStartMessage->ScriptID, NULL);
                }
	...
	const bool GLTouranmentClient::_messageProcedure_Tournament_game_popup(const GLMSG::NET_MATCHING_MESSAGE* const _pMessage)
	{
		...
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->SetInstancePopUp(&Receipt);
	...
	const bool GLTouranmentClient::_messageProcedure_Tournament_faction_info(const GLMSG::NET_MATCHING_MESSAGE* const _pMessage)
	{
		...
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->SetTournamentIndicatorType(m_dwObserveType,m_dw1FactionID,m_dw2FactionID);
	...
	const bool GLTouranmentClient::_messageProcedure_Tournament_timelimit(const GLMSG::NET_MATCHING_MESSAGE* const _pMessage)
	{
		...
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->SetIndicatorTime(_pStartMessage->nSec,_pStartMessage->bBlank,_pStartMessage->bShow);
		return true;
	}
	...
	const bool GLTouranmentClient::_messageProcedure_Tournament_Win(const GLMSG::NET_MATCHING_MESSAGE* const _pMessage)
	{
		...
		m_pGaeaClient->GetInterface()->OpenTournamentWinLose(true);

	/*	GLWidgetScript::GetInstance().LuaCallFunc< void >(
			NSWIDGET_SCRIPT::g_strFunc_ConftDisplay,
			"-n", (int) NSWIDGET_SCRIPT::EMCONFT_DISP_WIN );*/

		m_pGaeaClient->GetInterface()->SetFightEnd( CONFT_WIN_INDEX );
	...
	const bool GLTouranmentClient::_messageProcedure_Tournament_Lose(const GLMSG::NET_MATCHING_MESSAGE* const _pMessage)
	{
		...
		m_pGaeaClient->GetInterface()->OpenTournamentWinLose(false);

		/*GLWidgetScript::GetInstance().LuaCallFunc< void >(
			NSWIDGET_SCRIPT::g_strFunc_ConftDisplay,
			"-n", (int) NSWIDGET_SCRIPT::EMCONFT_DISP_LOSE );*/

		m_pGaeaClient->GetInterface()->SetFightEnd( CONFT_LOSE_INDEX );
		return true;
	}
...
// 		m_ObserveJoinTime = -1;
	...
	void GLTouranmentClient::HideLeftTOPControlGroup()
	{
		//!! SF_TODO
		//UI 다 숨기자
		m_pGaeaClient->GetInterface()->UiHideGroup(ADMIN_MESSAGE_DISPLAY);
		m_pGaeaClient->GetInterface()->UiHideGroup(LEFTTOP_CONTROL_GROUP);
		m_pGaeaClient->GetInterface()->UiHideGroup(BASIC_INFO_VIEW);
		m_pGaeaClient->GetInterface()->UiHideGroup(QUICK_SKILL_TRAY_TAB_WINDOW);
		m_pGaeaClient->SetChaseCam(STARGETID(),true);
	}
	...
	const bool GLTouranmentClient::_messageProcedure_Tournament_Out_Tournament(const GLMSG::NET_MATCHING_MESSAGE* const _pMessage)
	{
		...
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->OpenTournamentUI_HUD(false);
...
//UiHideGroup
		...
		if(m_ClientType == MatchingSystem::TOURNAMENT_OBSERVER)
		{
			//!! SF_TODO
			//UI 다 숨기자
			m_pGaeaClient->GetInterface()->UiShowGroupFocus(ADMIN_MESSAGE_DISPLAY);
			m_pGaeaClient->GetInterface()->UiShowGroupFocus(LEFTTOP_CONTROL_GROUP);
			m_pGaeaClient->GetInterface()->UiShowGroupFocus(BASIC_INFO_VIEW);
			m_pGaeaClient->GetInterface()->UiShowGroupFocus(QUICK_SKILL_TRAY_TAB_WINDOW);
			m_pGaeaClient->SetChaseCam(STARGETID(),false);
		...
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->HideAllTournamentWindow();
		...
		case EM_BETTING_TWICE:
			{
				...
				//!! SF_TODO
				m_pGaeaClient->GetInterface()->DoModal(
					MessageBoxName,
					UI::MODAL_INFOMATION,
					UI::OK,
					UI::MODAL_DO_NOTING );
...
*/
	...
	const bool GLTouranmentClient::_messageProcedure_join_ask(const NET_MATCHING_MESSAGE* const _pMessage)
	{
		...
		if ( _pJoinMessage->bComplete )
		{
			...
			m_pGaeaClient->GetInterface()->OpenTournamentJoinWindow( false , 0 );

		}
		// 입장할지말지 질의하는 부분이다;
		else
		{
			m_pGaeaClient->GetInterface()->OpenTournamentJoinWindow( true , _pJoinMessage->fTime );
		}
	...
	const bool GLTouranmentClient::_messageProcedure_Notify_at_first(const NET_MSG_GENERIC* const _pMessage)
	{
        ...
        if ( isSame && ((MATCHING_CONTENTS_STATUS)_pInfoMessage.wContentsStatus != pMatchinginfo->m_ContentsStatus) )
        {
            ...
            if ( (MATCHING_CONTENTS_STATUS)_pInfoMessage.wContentsStatus == EMMATCHING_CONTENTS_OPENED )
            {
				...
				//!! SF_TODO
                m_pGaeaClient->GetInterface()->CompetitionNotifyButtonSetAlarm(_pInfoMessage.dwScriptID, sc::string::format( ID2GAMEWORD("COMEPTITION_NOTIFY_BUTTON", 0), szName).c_str());
            }
            else
            {
				//!! SF_TODO
                m_pGaeaClient->GetInterface()->CompetitionNotifyButtonSetAlarm(_pInfoMessage.dwScriptID, NULL);
            }
        }        
    ...
    const bool GLTouranmentClient::_messageProcedure_Tournament_Indicator_UI_Info(const NET_MSG_GENERIC* const _pMessage)
    {
		...
		m_pGaeaClient->GetInterface()->SetTournamentIndicatorInfo( _pMessage );
