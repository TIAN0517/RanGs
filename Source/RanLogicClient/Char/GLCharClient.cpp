	...
	case GLAT_IDLE:
		{
			...
			else
			{
				...
				if ( pWING && pWING->sBasicOp.IsWING() && !m_sVehicle.m_pSkinChar  )
				{
					emMType = AN_FLY;
					emSType = AN_SUB_01;
				}*/
				...
				{
					const SITEMCLIENT &sITEMCLIENT = GET_SLOT_ITEM ( SLOT_WAISTBAND );
					...
					{
						if ( sITEMCLIENT.nidDISGUISE != NATIVEID_NULL() )
						{
							pWING = GLogicData::GetInstance().GetItem ( sITEMCLIENT.nidDISGUISE );
