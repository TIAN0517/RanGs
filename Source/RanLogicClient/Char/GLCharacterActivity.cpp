...
// 			NSWIDGET_SCRIPT::g_strFunc_StudentRecord_Update_Act );
		...
		m_pGaeaClient->GetInterface()->UpdateActivityWindow();
		m_pGaeaClient->GetInterface()->UpdateActivityPoint();
...
// 	GLWidgetScript::GetInstance().LuaCallFunc< void >( 
    ...
    if ( pActivity->m_id.wMainID != ACTIVITY_MID_LEVEL )
    {
		...
		//!! SF_TODO
        if ( GLUseFeatures::GetInstance().IsUsingActivity() &&m_pGaeaClient->GetInterface()->IsActivityProgressDisplay() && (nValue != nValueBase) )
        {
