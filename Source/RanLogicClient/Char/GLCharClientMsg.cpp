	...
	case NET_MSG_GCTRL_PKCOMBO_BRD:
		{
			...
			if ( m_CharData.m_Base.sPKCOMBOCOUNT.IsACTIVE() &&  m_CharData.m_Base.sPKCOMBOCOUNT.nCount > 3 )
			{
				CString strTEXT;
				int nIndex = m_CharData.m_Base.sPKCOMBOCOUNT.nCount - 1;
				if ( nIndex > 14 ) nIndex = 14;
				strTEXT.Format("'%s' is now '%s'",GetName(),PKStreak[nIndex].GetString());
				m_pGaeaClient->PrintConsoleText (strTEXT.GetString());
				m_pGaeaClient->GetInterface()->ProgressDisplayShowPulblic ( strTEXT.GetString(), ITEM_INFOR_COLOR::dwAddAbility );
			}
