...
// 			"-n, -n", wPosX, wPosY );
		m_pGaeaClient->GetInterface()->SetStorageChargeOpen( wPosX, wPosY );
        break;
...
// 			wPosX, wPosY );
		...
		m_pGaeaClient->GetInterface()->ToggleWindowProductReset( pItem->sDrugOp.GetCureVolumn(), wPosX, wPosY );
		break;
...
// 			NSWIDGET_SCRIPT::g_strFunc_PrivateMarket_TitleMsgBox_Open );
		...
		m_pGaeaClient->GetInterface()->SetPrivateMarketMake();
    }
}
	...
	{ // 그렇지 않다면 확인 UI를 띄움;
		...
		case EMSKILLFACT_TYPE_EXP_LUNCHBOX:
			{				
				...
				{
					m_pGaeaClient->GetInterface()->GetModalWindow()->SetModalData( wPosX, wPosY );
					m_pGaeaClient->GetInterface()->DoModal( sc::string::format( ID2GAMEINTEXT("SKILLFACT_ASK_OVERWRITE_1"),pPreviousSkill->GetName()), UI::MODAL_QUESTION, UI::OKCANCEL, UI::MODAL_SKILLFACT_LUNCHBOX_OVERWRITE );
				}
				// 위 두 조건 어디에도 만족하지 않는다면 아래 교체 확인 UI를 띄움;
				else
				{
					m_pGaeaClient->GetInterface()->GetModalWindow()->SetModalData( wPosX, wPosY );
					m_pGaeaClient->GetInterface()->DoModal( sc::string::format( ID2GAMEINTEXT("SKILLFACT_ASK_OVERWRITE_0"),pPreviousSkill->GetName(), pSkill->GetName() ), UI::MODAL_QUESTION, UI::OKCANCEL, UI::MODAL_SKILLFACT_LUNCHBOX_OVERWRITE );
				}
			}
			break;
		case EMSKILLFACT_TYPE_LUNCHBOX:			
			{
				bool bCHECKLB[SKILLFACT_SIZE_LUNCHBOX] = { false, false };
				...
				const SNATIVEID& previousSkillID_2nd = GetSkillFactId(SKILLFACT_INDEX_LUNCHBOX_START + 1);
				GLSKILL* pPreviousSkill_2nd = GLSkillMan::GetInstance().GetData(previousSkillID_2nd);
				// 기존에 걸려 있는 효과가 유효하지 않다면 적용 요청;
				if ( pPreviousSkill_2nd == NULL )
				{
					bCHECKLB[1] = true;
				}
				// 위 두 조건 어디에도 만족하지 않는다면 아래 교체 확인 UI를 띄움;
				if ( bCHECKLB[0] || bCHECKLB[1] )
				{
					ReqInvenLunchBox(wPosX, wPosY);
				}
				else
...
HRESULT GLCharacter::ReqInvenChangeCountry ( WORD wPosX, WORD wPosY )
{
	...
	m_pGaeaClient->GetInterface()->DoModal( ID2GAMEINTEXT("CONFIRM_ITEM_COUNTRY_CHANGE_INFO"), UI::MODAL_QUESTION, UI::OKCANCEL, UI::MODAL_CONFIRM_CHANGE_COUNTRY );
...
void GLCharacter::ReqInvenDrugHoldPetRevive( const SITEM* pItem, WORD wPosX, WORD wPosY )
{
    ...
    {			
        ...
        // 여기서 팻부활메시지창을 띄운다.
		//!! SF_TODO
        if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( PET_REBIRTH_DIALOGUE ) ) // monster7j
        {
            m_pGaeaClient->GetInterface()->UiShowGroupFocus( PET_REBIRTH_DIALOGUE );
        }
    }
}
...
void GLCharacter::ReqStatsUpCmd( EMSTATS emStats, DWORD value )
{
	...
	if ( m_wStatsPoint < value){
		m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::RED, "Adding stats Failed! not enough stat points." );
		return;
	}

	switch( emStats )
	{
	case EMPOW: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Adding %d Pow.", value) );	break;
	case EMSTR: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Adding %d Vit.", value) );	break;
	case EMSPI: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Adding %d Int.", value) );	break;
	case EMDEX: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Adding %d Dex.", value) );	break;
	case EMSTA: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Adding %d Sta.", value) );	break;
	}
