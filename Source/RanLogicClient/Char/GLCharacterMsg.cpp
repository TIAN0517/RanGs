...
void GLCharacter::InitPacketFunc()
{
	...
	m_GLCharacterMsgFunc[NET_MSG_GCTRL_REQ_STATSUPCMD_FB].Msgfunc = &GLCharacter::MsgAddStatsCmd;

	m_GLCharacterMsgFunc[NET_MSG_GCTRL_UPDATE_PK_SCORE].Msgfunc = &GLCharacter::MsgPKKIncBrd;
	m_GLCharacterMsgFunc[NET_MSG_GCTRL_UPDATE_PK_DEATH].Msgfunc = &GLCharacter::MsgPKDIncBrd;

	m_GLCharacterMsgFunc[NET_MSG_REQ_TUTORIAL_ACK].Msgfunc      = &GLCharacter::MsgTutorialAck;

	m_GLCharacterMsgFunc[NET_MSG_REQ_CHAR_LOCK_FB].Msgfunc      = &GLCharacter::MsgCharLockFB;
	m_GLCharacterMsgFunc[NET_MSG_GCTRL_GET_RANKINGS_FROMDB_FB].Msgfunc      = &GLCharacter::MsgGetRankingsFB;
}
...
void GLCharacter::MsgGetRankingsFB ( NET_MSG_GENERIC* _pNetMsg )
{
	GLMSG::SNET_GET_RANKINGS_FROMDB_FB* pNetMsg = (GLMSG::SNET_GET_RANKINGS_FROMDB_FB*)_pNetMsg;
	switch ( pNetMsg->emFB )
	{
	case EMREQ_RANKING_FROMDB_FB_RESET:
		{
			m_pGaeaClient->GetInterface()->ReSetRankingInfo();
		}break;
	case EMREQ_RANKING_FROMDB_FB_END:
		{
			m_pGaeaClient->GetInterface()->SetRankingInfo();
		}break;

	case EMREQ_RANKING_FROMDB_FB_OK:
		{
			DWORD dwSize = (DWORD) (sizeof(DWORD)*EMCLUB_MARK_SX*EMCLUB_MARK_SY);
			RANKING RANK = RANKING();
			RANK.bLevel = pNetMsg->sRank.bLevel;
			RANK.bRich = pNetMsg->sRank.bRich;
			RANK.bPK = pNetMsg->sRank.bPK;

			RANK.lMoney = pNetMsg->sRank.lMoney;
			RANK.dwCharID = pNetMsg->sRank.dwCharID;
			RANK.wClass = pNetMsg->sRank.wClass;
			RANK.wDeaths = pNetMsg->sRank.wDeaths;
			RANK.wKills = pNetMsg->sRank.wKills;
			RANK.wSchool = pNetMsg->sRank.wSchool;
			RANK.wLevel = pNetMsg->sRank.wLevel;
			RANK.nOnline = pNetMsg->sRank.nOnline;
			RANK.dwClub = pNetMsg->sRank.dwClub;
			RANK.wLevel = pNetMsg->sRank.wLevel;
			memcpy( RANK.m_aryMark, pNetMsg->sRank.m_aryMark, dwSize);
			RANK.m_dwMarkVER = pNetMsg->sRank.m_dwMarkVER;
			StringCchCopy( RANK.szName, CHAR_SZNAME+1, pNetMsg->sRank.szName );

			if ( RANK.bLevel ) m_mapRankings.insert ( std::make_pair ( RANK.dwCharID, RANK ) );
			if ( RANK.bPK ) m_mapRankingsPK.insert ( std::make_pair ( RANK.dwCharID, RANK ) );
			if ( RANK.bRich ) m_mapRankingsRich.insert ( std::make_pair ( RANK.dwCharID, RANK ) );
		}
		break;
	}
}

void GLCharacter::MsgCharLockFB ( NET_MSG_GENERIC* pMsg )
{
	GLMSG::SNETPC_REQ_CHAR_LOCK_FB *pNetMsg = (GLMSG::SNETPC_REQ_CHAR_LOCK_FB *)pMsg;
	CString strText,strCombine;
	switch ( pNetMsg->emReq )
	{
	case EMCHARLOCK_INVENTORY_PASSWORD:
		{
			switch ( pNetMsg->emFB )
			{

			case EMCHARLOCK_FB_INVENTORY_PASSWORD_OK:
				{
					m_bInventoryLock = pNetMsg->bInventoryLock;
					if ( m_bInventoryLock )
					{
						strText = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",6);
					}
					else
					{
						strText = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",7);
					}
					strCombine = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",8);
					m_pGaeaClient->PrintConsoleTextDlg( sc::string::format(ID2GAMEINTEXT("EMCHARLOCK_FB_INVENTORY_PASSWORD_OK"),strText.GetString(),strCombine.GetString()) );
				}
				break;
			case EMCHARLOCK_FB_INVENTORY_PASSWORD_FAIL:
				{
					m_bInventoryLock = pNetMsg->bInventoryLock;
					switch ( pNetMsg->emReason )
					{

					};
					m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("EMCHARLOCK_FB_INVENTORY_PASSWORD_FAIL") );
				}
				break;
			};
		}
		break;
	case EMCHARLOCK_INVENTORY_CHANGE_PASSWORD:
		{
			switch ( pNetMsg->emFB )
			{
			case EMCHARLOCK_FB_INVENTORY_CHANGE_PASSWORD_OK:
				{
					m_bInventoryLock = pNetMsg->bInventoryLock;
					strText = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",7);
					strCombine = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",8);
					m_pGaeaClient->PrintConsoleTextDlg( sc::string::format(ID2GAMEINTEXT("EMCHARLOCK_FB_INVENTORY_CHANGE_PASSWORD_OK"),strText.GetString(),strCombine.GetString()) );
				}
				break;
			case EMCHARLOCK_FB_INVENTORY_CHANGE_PASSWORD_FAIL:
				{
					m_bInventoryLock = pNetMsg->bInventoryLock;
					switch ( pNetMsg->emReason )
					{
					...
					m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("EMCHARLOCK_FB_INVENTORY_CHANGE_PASSWORD_FAIL") );
				}
				break;
			};
		}
		break;
	case EMCHARLOCK_INVENTORY_RESET_PASSWORD:
		{
			switch ( pNetMsg->emFB )
			{
			case EMCHARLOCK_FB_INVENTORY_RESET_PASSWORD_OK:
				{
					m_bInventoryLock = pNetMsg->bInventoryLock;
					strText = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",7);
					strCombine = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",8);
					m_pGaeaClient->PrintConsoleTextDlg( sc::string::format(ID2GAMEINTEXT("EMCHARLOCK_FB_INVENTORY_RESET_PASSWORD_OK"),strText.GetString(),strCombine.GetString()) );
				}
				break;
			case EMCHARLOCK_FB_INVENTORY_RESET_PASSWORD_FAIL:
				{
					m_bInventoryLock = pNetMsg->bInventoryLock;
					switch ( pNetMsg->emReason )
					{
					...
					m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("EMCHARLOCK_FB_INVENTORY_RESET_PASSWORD_FAIL") );
				}
				break;
			};
		}
	...
	case EMCHARLOCK_LOCKER_PASSWORD:
		{
			switch ( pNetMsg->emFB )
			{
			case EMCHARLOCK_FB_LOCKER_PASSWORD_OK:
				{
					m_bLockerLock = pNetMsg->bLockerLock;
					if ( m_bLockerLock )
					{
						strText = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",6);
					}
					...
					strCombine = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",9);
					m_pGaeaClient->PrintConsoleTextDlg( sc::string::format(ID2GAMEINTEXT("EMCHARLOCK_FB_LOCKER_PASSWORD_OK"),strText.GetString(),strCombine.GetString()) );
				}
				break;
			case EMCHARLOCK_FB_LOCKER_PASSWORD_FAIL:
				{
					m_bLockerLock = pNetMsg->bLockerLock;
					switch ( pNetMsg->emReason )
					{
					...
					m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("EMCHARLOCK_FB_LOCKER_PASSWORD_FAIL") );
				}
				break;
			};
		}
	...
	case EMCHARLOCK_LOCKER_CHANGE_PASSWORD:
		{
			switch ( pNetMsg->emFB )
			{
			case EMCHARLOCK_FB_LOCKER_CHANGE_PASSWORD_OK:
				{
					m_bLockerLock = pNetMsg->bLockerLock;
					strText = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",7);
					strCombine = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",9);
					m_pGaeaClient->PrintConsoleTextDlg( sc::string::format(ID2GAMEINTEXT("EMCHARLOCK_FB_LOCKER_CHANGE_PASSWORD_OK"),strText.GetString(),strCombine.GetString()) );
				}
				break;
			case EMCHARLOCK_FB_LOCKER_CHANGE_PASSWORD_FAIL:
				{
					switch ( pNetMsg->emReason )
					{

					};
					m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("EMCHARLOCK_FB_LOCKER_CHANGE_PASSWORD_FAIL") );
				}
				break;
			};
		}
	...
	case EMCHARLOCK_LOCKER_RESET_PASSWORD:
		{
			switch ( pNetMsg->emFB )
			{
			case EMCHARLOCK_FB_LOCKER_RESET_PASSWORD_OK:
				{
					m_bLockerLock = pNetMsg->bLockerLock;
					strText = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",7);
					strCombine = ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",9);
					m_pGaeaClient->PrintConsoleTextDlg( sc::string::format(ID2GAMEINTEXT("EMCHARLOCK_FB_LOCKER_RESET_PASSWORD_OK"),strText.GetString(),strCombine.GetString()) );
				}
				break;
			case EMCHARLOCK_FB_LOCKER_RESET_PASSWORD_FAIL:
				{
					m_bLockerLock = pNetMsg->bLockerLock;
					switch ( pNetMsg->emReason )
					{
					...
					m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("EMCHARLOCK_FB_LOCKER_RESET_PASSWORD_FAIL") );
				}
				break;
			};
		}
...
void GLCharacter::MsgTutorialAck ( NET_MSG_GENERIC* pMsg )
{
	GLMSG::SNETPC_REQ_TUTORIAL_ACK *pNetMsg = (GLMSG::SNETPC_REQ_TUTORIAL_ACK *)pMsg;

	if ( GLUseFeatures::GetInstance().IsUsingTutorialMode() )
	{
		m_pGaeaClient->GetTutorial()->InitData();
		m_pGaeaClient->GetTutorial()->StartTutorialInClient( this );
	}
}
void GLCharacter::MsgAddStatsCmd  ( NET_MSG_GENERIC* pMsg )
{
	GLMSG::SNETPC_REQ_STATSUPCMD_FB *pNetMsg = (GLMSG::SNETPC_REQ_STATSUPCMD_FB *)pMsg;
	STATSUP_CMD(pNetMsg->emStats,pNetMsg->dwValue);
	m_wStatsPoint = pNetMsg->wStatsPoint;

	switch( pNetMsg->emStats )
	{
	case EMPOW: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Successfully Added %d Pow.", pNetMsg->dwValue) );	break;
	case EMSTR: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Successfully Added %d Vit.", pNetMsg->dwValue) );	break;
	case EMSPI: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Successfully Added %d Int.", pNetMsg->dwValue) );	break;
	case EMDEX: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Successfully Added %d Dex.", pNetMsg->dwValue) );	break;
	case EMSTA: m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format ("Successfully Added %d Sta.", pNetMsg->dwValue) );	break;
	}
}

void GLCharacter::MsgPKKIncBrd ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPC_UPDATE_PK_SCORE* pNetMsg = ( GLMSG::SNETPC_UPDATE_PK_SCORE* ) nmg;
	m_dwPKScore = pNetMsg->dwVal;
	m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::GREENYELLOW, sc::string::format (ID2GAMEINTEXT("UPDATE_PK_SCORE"), pNetMsg->szName) );
}

void GLCharacter::MsgPKDIncBrd ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPC_UPDATE_PK_DEATH* pNetMsg = ( GLMSG::SNETPC_UPDATE_PK_DEATH* ) nmg;
	m_dwPKDeath = pNetMsg->dwVal;
	m_pGaeaClient->PrintMsgText ( NS_UITEXTCOLOR::ORNAGERED, sc::string::format (ID2GAMEINTEXT("UPDATE_PK_DEATH"), pNetMsg->szName) );
}

void GLCharacter::MsgPKComboBrd ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pNetMsg->sCOMBO.nCount == 1 )		{	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_FIRST" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 0 );		 }
	else if ( pNetMsg->sCOMBO.nCount == 2 )	{	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_DOUBLE" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 1 );		 }
	else if ( pNetMsg->sCOMBO.nCount == 3 ) {	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_TRIPLE" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 2 );		 }
	else if ( pNetMsg->sCOMBO.nCount == 4 ) {	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_ULTRA" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 3 );		 }
	else if ( pNetMsg->sCOMBO.nCount == 5 ) {	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_RAMPAGE" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 4 );		 }
	else if ( pNetMsg->sCOMBO.nCount == 6 ) {	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_KILLING_SPREE" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 5 ); } 
	else if ( pNetMsg->sCOMBO.nCount == 7 ) {	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_DOMINATING" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 6 );	 }
	else if ( pNetMsg->sCOMBO.nCount == 8 ) {	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_MEGA_KILL" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 7 );	 }
	else if ( pNetMsg->sCOMBO.nCount == 9 ) {	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_UNSTOPPABLE" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 8 );	 }
	else if ( pNetMsg->sCOMBO.nCount == 10 ){	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_WICKED_SICK" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 9 );	 }
	else if ( pNetMsg->sCOMBO.nCount == 11 ){	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_MONSTER_KILL" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 10 ); }
	else if ( pNetMsg->sCOMBO.nCount == 12 ){	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_GODLIKE" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 11 );		 }
	else if ( pNetMsg->sCOMBO.nCount == 13 ){	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_HOLY_SHIT" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 12 );	 }
	else if ( pNetMsg->sCOMBO.nCount >= 14 ){	DxSoundLib::GetInstance()->PlaySound ( "PKCOMBO_OWNAGE" ); m_pGaeaClient->GetInterface()->SET_PKCOMBO( 13 );		 }
	...
	{
		int nIndex = m_sPKCOMBOCOUNT.nCount - 1;
		if ( nIndex > 14 ) nIndex = 14;
		m_pGaeaClient->PrintConsoleText (PKStreak[nIndex].GetString());

		if ( RANPARAM::emAroundDetail == EM_AROUND_ORIG )
		{
			const D3DXVECTOR3& vPosition = GetPosition();
			D3DXMATRIX matTrans;
			STARGETID sTarget( CROW_PC, m_dwGaeaID );
			D3DXMatrixTranslation ( &matTrans, vPosition.x, vPosition.y, vPosition.z );
			DxEffGroupPlayer::GetInstance().NewEffGroup ( "_StandBy.egp", matTrans, &sTarget, TRUE, TRUE );
		}
	}

}
void GLCharacter::MsgPKComboEndBrd ( NET_MSG_GENERIC* nmg )
{
