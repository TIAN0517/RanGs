	...
	case GLAT_IDLE:
		{
			...
			else
			{
				...
				/*SITEM* pWING = GET_SLOT_ITEMDATA ( SLOT_WAISTBAND ) ;

				if ( pWING && GET_SLOT_ITEM( SLOT_WAISTBAND ).IsWing() && !m_sVehicle.m_pSkinChar )
				{
					emMType = AN_FLY;
					emSType = AN_SUB_01;
				}*/

				if ( VALID_SLOT_ITEM(SLOT_WAISTBAND) )
				{
					SITEM* pWING = GET_SLOT_ITEMDATA ( SLOT_WAISTBAND ) ;

					if ( pWING && pWING->sBasicOp.IsWING() && !m_sVehicle.m_pSkinChar && !RF_DISGUISE( m_EffSkillVarSet ).IsOn()  )
					{
						emMType = AN_FLY;
						emSType = AN_SUB_01;
					}
					...
					{
						if ( GET_SLOT_ITEM ( SLOT_WAISTBAND ).nidDISGUISE != NATIVEID_NULL() )
						{
							pWING = GLogicData::GetInstance().GetItem ( GET_SLOT_ITEM ( SLOT_WAISTBAND ).nidDISGUISE );
	...
	case GLAT_MOVE:
		{
			...
			{
				emMType = AN_FLY;
				emSType = IsActState(EM_ACT_RUN) ? AN_SUB_03 : AN_SUB_02;
			}
			else emMType = IsActState(EM_ACT_RUN) ? AN_RUN : AN_WALK;*/
