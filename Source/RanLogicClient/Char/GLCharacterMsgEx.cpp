...
void GLCharacter::MsgAttackDamage(NET_MSG_GENERIC* pMsg)
{
	...
	if ( m_pGaeaClient->GetTutorial()->IsTutorial() )
	{
		...
		{
			m_pGaeaClient->GetTutorial()->CheckAttackForTutorial( pNetMsg->emTarCrow, pNetMsg->dwTarID );
		}
	}
...
void GLCharacter::MsgPutonChange(NET_MSG_GENERIC* pMsg)
{
	...
	m_pGaeaClient->GetInterface()->SetArmSwap( !pNetMsg->bUseArmSub );
    ...
    if ( m_dwUserLvl < USER_USER_GM )
    {
        ...
        if ( pMapNode )
        {
...

            {
                ...
                {
					//!! SF_TODO
                    m_pGaeaClient->GetInterface()->SetMapCheckType( UI::EMMAPCHECK_LEVELUP );
                    m_pGaeaClient->GetInterface()->UiShowGroupFocus ( MAP_REQUIRE_CHECK );
                }
            }
...
// 		}		
    ...
    if ( m_pGaeaClient->GetTutorial()->IsTutorial() )
    {
        if ( m_pGaeaClient->GetTutorial()->IsAttackStep() )
        {
            m_pGaeaClient->GetTutorial()->CheckAttackForTutorial( pNetMsg->sTargetID.emCrow, pNetMsg->sTargetID.GaeaId );
			m_pGaeaClient->GetTutorial()->m_sUseSkillID = pNetMsg->sSkillID;
        }
    }
}
...
void GLCharacter::MsgGetChaExtraInfo(NET_MSG_GENERIC* nmg)
{
	...
	int ContriBefore = m_nContributionPoint;
	m_nContributionPoint = pNetMsg->nContributionPoint;
	PrintConsoleText( sc::string::format( "ContributionPoint Update: %1% (+%2%)",m_nContributionPoint, m_nContributionPoint - ContriBefore ) );
}
	...
	case EMREQ_NEI_EX_SUCCESS	: 
		{
			...
			//!! SF_TODO
			CInnerInterface* pInter = dynamic_cast<CInnerInterface*>(m_pGaeaClient->GetInterface());
