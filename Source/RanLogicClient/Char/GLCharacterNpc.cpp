...
HRESULT GLCharacter::MsgNpcTalkBasic ( DWORD dwNpcGlobID, DWORD nAction, DWORD dwTalkID )
{
    ...
    case SNpcTalk::EM_STORAGE:
        {
			...
			//GLWidgetScript::GetInstance().LuaCallFunc< void >( 
				...
				//"-dw", dwNpcGlobID );
			m_pGaeaClient->GetInterface()->SetStorageWindowNpcOpen(spCrow->m_sNativeID,dwNpcGlobID);
        }
        break;
    case SNpcTalk::EM_MARKET:
        {
			...
			m_pGaeaClient->GetInterface()->SetMarketWindowOpen(spCrow->m_sNativeID,dwNpcGlobID);
        }
        break;
    ...
    case SNpcTalk::EM_BUSSTATION:
        {
			...
			//GLNpcTalkClient::GetInstance().SetNpcID( spCrow->m_sNativeID );
			//GLNpcTalkClient::GetInstance().SetNpcGlobalID( dwNpcGlobID );
			//m_pGaeaClient->GetInterface()->SetBusWindowOpen(spCrow->m_sNativeID,dwNpcGlobID)	
        }
        break;
    ...
    case SNpcTalk::EM_CLUB_NEW:
		{
			...
			m_pGaeaClient->GetInterface()->SetClubMake(dwNpcGlobID);
		}
		break;
    ...
    case SNpcTalk::EM_CLUB_STORAGE:
        {
			if( m_pGaeaClient->ReqGetClubStorage() )
			{
				...
				m_pGaeaClient->GetInterface()->SetClubStorageWindowOpen( spCrow->m_sNativeID, dwNpcGlobID );
			}
        }
        break;
    ...
    case SNpcTalk::EM_ITEM_REBUILD:	// ITEMREBUILD_MARK
		...
		m_pGaeaClient->GetInterface()->SetItemRebuildWindowNpcOpen( spCrow->m_sNativeID, dwNpcGlobID );
        }
        break;
	...
	case EMREQ_NPCTALK_FB_OK :
		{
			...
			m_pGaeaClient->GetInterface()->SetDialogueWindowOpen( pNetMsg->sNPCID, m_NpcLastClick, this );
		}
		break;
	}
}
...
void GLCharacter::MsgAddRebuyItem(NET_MSG_GENERIC* pMsg)
{
	...
	m_pGaeaClient->GetInterface()->DialogueWindowLoadRebuyList();
...
void GLCharacter::MsgCheckRebuySupplies(NET_MSG_GENERIC* pMsg)
{
	...
	//!! SF_TODO
    m_pGaeaClient->GetInterface()->DialogueWindowCheckRebuySupplies( RANPARAM::bCheckRebuySuppliesRebuyPage );
}