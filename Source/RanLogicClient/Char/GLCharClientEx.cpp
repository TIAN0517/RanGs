...
void GLCharClient::StartSkillProc ()
{
	...
	if ( RANPARAM::emAroundDetail != EM_AROUND_ORIG ) return;
...
void GLCharClient::UPDATE_ITEM ( )
{
    ...
    for ( int i=0; i<SLOT_NSIZE_S_2; i++ )
    {
		...
		//add costume move and attackspeed
		if ( sItemCustom.nidDISGUISE!=SNATIVEID(false) )
		{//add jitem
			SITEM* sItem = GLItemMan::GetInstance().GetItem( sItemCustom.nidDISGUISE );
			if ( sItem )
			{
				if ( sItemCustom.IsCSApplied())
				{
					//	Note : 변화율 효과.
					for ( INT k = 0; k < ITEM::SSUIT::ADDON_SIZE; ++k )
					{
						switch ( sItemCustom.sAddonInfo.m_sADDON[k].emTYPE )
						{
						case EMADDEX_INCR_HP:
						case EMADDEX_INCR_MP:
						case EMADDEX_INCR_SP:
						...
						case EMADDEX_INCR_MOVE_SPEED:
							if (emSLOT != SLOT_VEHICLE)
								m_fITEM_MOVE_R += sItemCustom.sAddonInfo.m_sADDON[k].fVALUE;
							break;
						case EMADDEX_INCR_ATTACK_SPEED:
							m_fITEMATTVELO_R	+= sItemCustom.sAddonInfo.m_sADDON[k].fVALUE;
							break;
						case EMADDEX_INCR_CRITICAL_RATE:
							break;
						case EMADDEX_INCR_CRUSHING_BLOW:
						...
						case EMADDEX_INC_MOVE_SPEED:
							if ( emSLOT != SLOT_VEHICLE )
								m_fITEM_MOVE += sItemCustom.sAddonInfo.m_sADDON[k].fVALUE;
							break;
						case EMADDEX_INC_ATTACK_SPEED:
							m_fITEMATTVELO += sItemCustom.sAddonInfo.m_sADDON[k].fVALUE;
							break;
						case EMADDEX_INC_CRITICAL_RATE:
							break;
						case EMADDEX_INC_CRUSHING_BLOW:
...
void GLCharClient::MsgSkillFactBrd(NET_MSG_GENERIC* pMsg)
{
    ...
    if ( pNetMsg->nVAR_HP < 0 )
    {
		...
		m_pGaeaClient->GetInterface()->SetDamage(
			vPos, -pNetMsg->nVAR_HP, DAMAGE_TYPE_NONE, 
			UI_UNDERATTACK );
