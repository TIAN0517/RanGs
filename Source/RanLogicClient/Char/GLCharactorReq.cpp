...
#include "../../RanGfxUI/OldUI/Interface/ChatType.h"
...
void GLCharacter::ReqToggleRun ()
{
	if ( IsActState(EM_ACT_RUN) )
	{
		...
		/*GLWidgetScript::GetInstance().LuaCallFunc< void >(
			...
			"-b", false );*/
		m_pGaeaClient->GetInterface()->GameMenuFlipRunButton( false );
	}
	else
	{
		...
		/*GLWidgetScript::GetInstance().LuaCallFunc< void >(
			NSWIDGET_SCRIPT::g_strFunc_QuickMenu_SetRun, 
			"-b", true );*/
		m_pGaeaClient->GetInterface()->GameMenuFlipRunButton( true );
	}	
...
HRESULT GLCharacter::ReqFieldTo ( const STARGETID &sTargetID, bool bPet )
{
	...
	//else
	{
		...
		if ( sTargetID.emCrow==CROW_ITEM )
		{
			...
			SITEM *pItem = GLogicData::GetInstance().GetItem(spItemDrop->sItemClient.sNativeID);
			if( false == m_pGaeaClient->GetInterface()->QBoxButtonGetEnable() )
			{
...
*/
	...
	else if ( holditem::HaveHoldItem() )
	{
        ...
        case holditem::WEAR_SLOT:
            {
				SITEM* pSlotItem = GET_SLOT_ITEMDATA( static_cast<EMSLOT>( holditem::GetHoldPosX() ) );
				if ( pSlotItem )
				{
					switch ( pSlotItem->sBasicOp.emItemType )
					{
					case ITEM_ARROW:
					case ITEM_CHARM:
					case ITEM_BULLET: 
					...
					default:
						{
							if ( m_bInventoryLock )
							{
								m_pGaeaClient->PrintConsoleText( "Equipment is Locked. Action Ignored." );
								return S_FALSE;
							}
						}
						break;
...
HRESULT GLCharacter::SelectiveformBoxOpen(WORD wPosX, WORD wPosY)
{
	...
	m_pGaeaClient->GetInterface()->OpenSelectiveformBox( pItem->sBox.vecItems );
}
...
HRESULT GLCharacter::ReqDisguise( WORD wPosX, WORD wPosY )
{
	...
	if ( pInvenItem->sItemCustom.IsRestirctCostume() ) 
	{
...
HRESULT GLCharacter::InvenHairStyleChange ( WORD wPosX, WORD wPosY )
{
	...
	m_pGaeaClient->GetInterface()->UiShowGroupFocus( HAIRSTYLECARD_WINDOW );
...
HRESULT GLCharacter::InvenHairColorChange ( WORD wPosX, WORD wPosY )
{
	...
	//m_pGaeaClient->GetInterface()->

	//m_wInvenPosX2 = wPosX;
	//m_wInvenPosY2 = wPosY;
	m_pGaeaClient->GetInterface()->DoModal( ID2GAMEINTEXT("MODAL_HAIRCOLOR_INFO"), UI::MODAL_INFOMATION, UI::OK, UI::MODAL_HAIRCOLOR_INFO );
...
HRESULT GLCharacter::InvenVehicleColorChange ( WORD wPosX, WORD wPosY )
{
	...
	//	"-s", strText.c_str() );
	m_pGaeaClient->GetInterface()->DoModal( ID2GAMEINTEXT("MODAL_VEHICLE_COLOR_INFO"), UI::MODAL_INFOMATION, UI::OK, UI::MODAL_VEHICLE_COLOR );
...
HRESULT GLCharacter::InvenFaceStyleChange ( WORD wPosX, WORD wPosY )
{
	...
	m_pGaeaClient->GetInterface()->UiShowGroupFocus( FACE_CHANGE_WINDOW );
...
*/
	...
	m_pGaeaClient->GetInterface()->DoModal( ID2GAMEINTEXT("MODAL_GENDER_CHANGE"),  UI::MODAL_INFOMATION, UI::YESNO, UI::MODAL_GENDER_CHANGE );	
...
HRESULT GLCharacter::InvenRename ( WORD wPosX, WORD wPosY )
{
	...
	m_pGaeaClient->GetInterface()->DoModal ( ID2GAMEINTEXT("INVEN_CHAR_RENAME"), UI::MODAL_INPUT, UI::EDITBOX, UI::MODAL_CHAR_RENAME );
...
HRESULT GLCharacter::RequestOpenResetSkillStats( const WORD wPosX, const WORD wPosY  )
{
	...
	if ( wReturnStats )
	{
		...
		case ITEM_SKP_STAT_RESET_A:
			m_pGaeaClient->GetInterface()->StatsResetWindowResetProperty( wReturnStats, UI::EMSTATSRESETWINDOW_TYPE_STATS, ITEM_SKP_STAT_RESET_A );
			break;
		...
		case ITEM_SKP_STAT_RESET_B:
			{
				bSelectable = true;
				m_pGaeaClient->GetInterface()->StatsResetWindowResetProperty( wReturnStats, UI::EMSTATSRESETWINDOW_TYPE_STATS_SELECT, ITEM_SKP_STAT_RESET_B );
			}
			break;
	...
	else
	{
		...
		m_pGaeaClient->GetInterface()->StatsResetWindowResetProperty( pItem->sDrugOp.dwCureVolume, UI::EMSTATSRESETWINDOW_TYPE_SKILL, pItem->sBasicOp.emItemType );
	}
	m_pGaeaClient->GetInterface()->StatsResetWindowRecalculate();
	m_pGaeaClient->GetInterface()->UiShowGroupFocus ( STATSRESET_WINDOW );
...
HRESULT GLCharacter::ReqSkillUp(const SNATIVEID& skill_id)
{
	...
	{
		//	정상적으로 발생 할 수 없는 상황.
		GLMSG::SNETPC_REQ_RNSKILLUP NetMsg;
		NetMsg.AddSkillID(skill_id);
		if ( 0 < NetMsg.GetCount() )	m_pGaeaClient->NETSENDTOFIELD(&NetMsg);
		return E_FAIL;
	}
...
{	
	...
	if ( ValidRebuildOpen() )	// ITEMREBUILD_MARK
		m_pGaeaClient->GetInterface()->SetItemRebuildWindowClose();

	//!! SF_TODO
	if ( ValidGarbageOpen() )	// 휴지통
		m_pGaeaClient->GetInterface()->CloseItemGarbageWindow();
...
{   
	...
	else
	{
		...
		*/
		...
		//!! SF_TODO
        m_pGaeaClient->GetInterface()->ResetCartListInDialogueWindowMarketPage( dwChannel, pSaleItem->wPosX, pSaleItem->wPosY );
		...
		if ( pSaleItem )
		{
			...
			else 
			{
				if( pInven->IsUserExInsertEnable(&m_cInventory, sBUYNID, pData->itemlist[i].wPosX, pData->itemlist[i].wPosY, 0, 0, pData->itemlist[i].wBuyNum ) ) {
					...
					//!! SF_TODO
					m_pGaeaClient->GetInterface()->ResetCartListInDialogueWindowMarketPage( pData->itemlist[i].dwChannel, pSaleItem->wPosX, pSaleItem->wPosY );
...
HRESULT GLCharacter::ReqRebuildMoveItem(const bool bMsg)
{
	...
	if ( m_sPreInventoryItem.VALID() )
	{
		...
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->SetRebuildWindowPreInvenItem( m_sPreInventoryItem.wPosX, m_sPreInventoryItem.wPosY );
		m_sPreInventoryItem.RESET();
	}
...
HRESULT GLCharacter::ReqRebuildMoveStamp()
{
	...
	{		
		...
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->SetRebuildWindowPreStampItem( m_sPreInventoryItem.wPosX, m_sPreInventoryItem.wPosY );
		m_sPreInventoryItem.RESET();
	}
...
// 				NSWIDGET_SCRIPT::g_strFunc_QuickMenu_SetQBoxLock,
			// 				"-b", !bQboxEnable );
			m_pGaeaClient->GetInterface()->QBoxButtonSetEnable(!bQboxEnable);
			return false;
		}
	}
	return true;
...
HRESULT GLCharacter::InvenUseTaxiCard( WORD wPosX, WORD wPosY )
{
	...
	m_pGaeaClient->GetInterface()->OPEN_TAXI_WINDOW( wPosX, wPosY );
...
HRESULT GLCharacter::ReqItemMix( DWORD dwNpcGlobID )
{
	...
	if ( !pProductRecipe ) 
	{
		...
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->SetItemMixResult( ID2GAMEINTEXT("EMITEM_MIX_FB_NOMIX") );
		return E_FAIL;
	}
	...
	if ( GetInvenMoney() < sProductRecipe.dwPrice ) 
	{
		...
		//!! SF_TODO
		m_pGaeaClient->GetInterface()->SetItemMixResult( ID2GAMEINTEXT("EMITEM_MIX_FB_NOMONEY") );
		return E_FAIL;
	}
...
{		
	...
	m_pGaeaClient->GetInterface()->SetPartyLinkOnce(nLinkIndex);
...
// 				(int) NSCostumeStat::EMQuestion_Replace, wPosX, wPosY );
	...
	if( pItem->IsBasicStatExist() )
	{
		...
		else
		{
			...
			m_pGaeaClient->GetInterface()->OpenCostumeStatWindow ( pCs->GetSrcItem(), pCs->GetDstItem() );
		}
...
HRESULT GLCharacter::ReqCharacterLock ( EMCHARLOCK emReq, const TCHAR * szPassword, const TCHAR * szPIN )
{
	if ( !szPassword )	
	{
		m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_INPUT_ERROR_1") );
		return E_FAIL;
	}

	CString strPassword( szPassword );

	size_t PassLength = strPassword.GetLength();
	if ( PassLength == 0 )
	{
		m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_LENGTH_1") );
		return S_FALSE;
	}
	else if ( PassLength < 4 )
	{
		m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_LENGTH_2") );
		return S_FALSE;
	}
	else if ( PassLength > ( 18 - 1 ) )
	{
		m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_LENGTH_2") );
		return S_FALSE;
	}

	if ( strPassword.FindOneOf(" ") != -1 ) 
	{
		m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_INPUT_ERROR_1") );
		return S_FALSE;
	}

	if ( STRUTIL::CheckString_Special ( strPassword ) )
	{
		m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_INPUT_ERROR_2") );
		return S_FALSE;
	}

	GLMSG::SNETPC_REQ_CHAR_LOCK_CF NetCharLock( emReq );
	StringCchCopy ( NetCharLock.szPassword, 18, szPassword );

	switch ( emReq )
	{
	case EMCHARLOCK_INVENTORY_PASSWORD:
		{
			NetCharLock.bUnlock = m_bInventoryLock;
		}
		break;
	case EMCHARLOCK_INVENTORY_CHANGE_PASSWORD:
		{
			if ( !szPIN )	
			{
				m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_INPUT_ERROR_1-0") );
				return E_FAIL;
			}

			CString strPIN( szPIN );

			size_t PassLength = strPIN.GetLength();
			if ( PassLength == 0 )
			{
				m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_LENGTH_1-0") );
				return S_FALSE;
			}
			else if ( PassLength < 4 )
			{
				m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_LENGTH_2-0") );
				return S_FALSE;
			}
			else if ( PassLength > ( 18 - 1 ) )
			{
			...
			if ( strPIN.FindOneOf(" ") != -1 ) 
			{
				m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_INPUT_ERROR_1-0") );
				return S_FALSE;
			}

			if ( STRUTIL::CheckString_Special ( strPIN ) )
			{
				m_pGaeaClient->PrintConsoleTextDlg( ID2GAMEINTEXT("WEAR_LOCK_ERROR_INPUT_ERROR_2-0") );
				return S_FALSE;
			}
			StringCchCopy ( NetCharLock.szPIN, 18, szPIN );
		}
		break;
	case EMCHARLOCK_INVENTORY_RESET_PASSWORD:
		{
			if ( !szPIN )	return E_FAIL;
	...
	case EMCHARLOCK_LOCKER_PASSWORD:
		{
			NetCharLock.bUnlock = m_bLockerLock;
		}
		break;
	...
	m_pGaeaClient->NETSENDTOFIELD( &NetCharLock );

	return S_OK;
}

HRESULT GLCharacter::ReqRankingsInfo ()
{
	if ( !IsValidBody() )						return E_FAIL;

	m_mapRankings.clear();
	m_mapRankingsPK.clear();
	m_mapRankingsRich.clear();

	GLMSG::SNET_GET_RANKINGS_FROMDB NetMsg;
	NetMsg.dwCharID = m_CharDbNum;
