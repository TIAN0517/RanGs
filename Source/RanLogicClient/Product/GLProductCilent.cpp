        ...
        else
        {
			...
			case Product::EStateOngoingMotion:
                {
					...
					if ( pNetMsg->nState == Product::EStateMotion )
					{
						...
						m_pGaeaClient->GetInterface()->GetProductProgress()->StartProduct(pProductRecipe->dwKey,pNetMsg->nCount);
					}
					else
					{
						//!! SF_TODO
            ...
            case Product::EStateRateFail:   // 확률 실패시
				...
				//	//GLWidgetScript::GetInstance().LuaCallFunc< void >( NSWIDGET_SCRIPT::g_strFunc_ProductRefresh );
					m_pGaeaClient->GetInterface()->MsgProductComplete();
					m_pGaeaClient->GetInterface()->UiRefresh ( PRODUCT_WINDOW );
                }
                break;/////////////////////////////////////////////////////////
