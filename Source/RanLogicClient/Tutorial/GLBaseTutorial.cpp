...
#include "../../RanGfxUI/OldUI/Quest/QuestWindow.h"
#include "../../RanGfxUI/OldUI/Quest/WindowRenewal/QuestWindowRenewal.h"
...
{
	if ( !GLUseFeatures::GetInstance().IsUsingTutorialMode() ) return S_FALSE;
...
bool GLBaseTutorial::IsCanShortcutTutorial()
{
	return m_bControlKeyBoard;
}
...
//#if 0
	...
	if ( !m_bTutorial ) return;

	if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_VIEW_STEP ) )
	{
		m_pGaeaClient->GetInterface()->OpenTutorialViewStep();
	}
	...
	case INTRODUCE_STEP:
		{
			...
			m_pGaeaClient->GetInterface()->SetTutorialViewStep( 0 );

			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_SKIP_BUTTON ) )
			{
				m_pGaeaClient->GetInterface()->UiShowGroupTop( TUTORIAL_SKIP_BUTTON );
			}

			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
			{
				m_pGaeaClient->GetInterface()->OpenTutorialDialogue();
	...
	case EXPLAIN_INTERFACE_STEP:
		{
			...
			if ( m_dwSubStep == ONE_ )
			{
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( EXPLAIN_INTERFACE ) )
				{
					m_pGaeaClient->GetInterface()->UiShowGroupTop ( EXPLAIN_INTERFACE );
				}
			}
			else if ( m_dwSubStep == TWO_ )
			{
				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( TUTORIAL_DIALOGUE_WINDOW );
				}
			}
			...
			else if ( m_dwSubStep == NINE_ )
			{
				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( EXPLAIN_INTERFACE ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup ( EXPLAIN_INTERFACE );
				}
			}
			else if ( m_dwSubStep >= TEN_ )
			{
				m_pGaeaClient->GetInterface()->SetTutorialViewStep( 1 );

				CString strName;
				strName.Format( "TUTORIAL_DIALOGUE_STEP_THREE_0" );
				m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT(strName) );
	...
	case MOUSECONTROL_AND_MOVE_STEP:
		{
			...
			{	// 마우스 컨트롤 좌, 우 배우기 단계.
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
				{
					m_pGaeaClient->GetInterface()->OpenTutorialDialogueExplainText();

					m_pGaeaClient->GetInterface()->SetTutorialDialogueExplainText( ID2GAMEINTEXT( "TUTORIAL_MODE_12" ) );
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( MOUSETIP_WINDOW ) )
				{
					const UIRECT& rcMouseTipWindow = m_pGaeaClient->GetInterface()->MouseTipGetGlobalPos();
					UIRECT rcChange = rcMouseTipWindow;

					rcChange.sizeX *= 0.8f;
					rcChange.sizeY *= 0.8f;

					m_pGaeaClient->GetInterface()->MouseTipResizeControl(rcMouseTipWindow, rcChange);
					m_pGaeaClient->GetInterface()->OpenMouseTip();
				}

				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_LEFT ) )
				{
					m_pGaeaClient->GetInterface()->OpenTutorialInterfaceLeft();
				}

				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_RIGHT ) )
				{
					m_pGaeaClient->GetInterface()->OpenTutorialInterfaceRight();
				}
			}
			...
			{	// 마우스 컨트롤 상, 하 배우기 준비 단계.
				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_LEFT ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( TUTORIAL_INTERFACE_LEFT );
				}

				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_RIGHT ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( TUTORIAL_INTERFACE_RIGHT );
				}

				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW );
				}

				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( MOUSETIP_WINDOW ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( MOUSETIP_WINDOW );
				}
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_THREE_3" ) );
				}
			}
			...
			{	// 마우스 컨트롤 상, 하 배우기 단계.
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_UP ) )
				{
					m_pGaeaClient->GetInterface()->OpenTutorialInterfaceUp();
				}

				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_DOWN ) )
				{
					m_pGaeaClient->GetInterface()->OpenTutorialInterfaceDown();
				}
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogueExplainText( ID2GAMEINTEXT( "TUTORIAL_MODE_13" ) );
			...
			{	// 마우스 컨트롤 줌 인, 아웃 배우기 준비 단계.
				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_UP ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( TUTORIAL_INTERFACE_UP );
				}

				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_DOWN ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( TUTORIAL_INTERFACE_DOWN );
				}
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_THREE_4" ) );
				}
			}
			...
			{	// 마우스 컨트롤 줌 인, 아웃 배우기 단계.
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_ZOOM_IN ) )
				{
					m_pGaeaClient->GetInterface()->OpenTutorialInterfaceZoomIn();
				}

				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_ZOOM_OUT ) )
				{
					m_pGaeaClient->GetInterface()->OpenTutorialInterfaceZoomOut();
				}
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogueExplainText( ID2GAMEINTEXT( "TUTORIAL_MODE_14" ) );
			...
			else if ( m_dwSubStep == TEN_ )
			{
				...
				m_pGaeaClient->GetInterface()->SetTutorialViewStep( 2 );
				...
				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_ZOOM_IN ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( TUTORIAL_INTERFACE_ZOOM_IN );
				}

				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_INTERFACE_ZOOM_OUT ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( TUTORIAL_INTERFACE_ZOOM_OUT );
				}
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogueIndex(5);

					m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_THREE_5" ) );
				}
			}
			...
			{	// 특정 위치 도달 확인.
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogueExplainText( ID2GAMEINTEXT( "TUTORIAL_MODE_0" ) );
...

				{
					...
					{
						if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( BALLOON_TEXT_WINDOW ) )
						{
							m_pGaeaClient->GetInterface()->UiHideGroup( BALLOON_TEXT_WINDOW );
						}
					}
					else
					{
						if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( BALLOON_TEXT_WINDOW ) )
						{
							m_pGaeaClient->GetInterface()->OpenBalloonText();
						}
			...
			{	// 특정 위치 이동 실습 단계 중간 대화창 띄우기.
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogueIndex(8);

					m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_THREE_8" ) );
				}
			}
			...
			else if ( m_dwSubStep == FOURTEEN_ )
			{
				...
				m_pGaeaClient->GetInterface()->SetTutorialViewStep( 3 );
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogueIndex(9);

					m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_THREE_9" ) );
				}
			}
			...
			else if ( m_dwSubStep == SIXTEEN_ )
			{
				...
				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( MINIMAP_TARGET ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( MINIMAP_TARGET );
				}
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogueIndex(11);

					m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_THREE_11" ) );
				}
			}
		}
		break;
	...
	case QUICKSLOT_ACTIONSLOT_STEP:
		if ( m_dwSubStep == ONE_ )
		{
			...
			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
			{
				...
				//m_pGaeaClient->GetInterface()->SetTutorialDialogueIndex(0);
				m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_FOUR_0" ) );
			}
		}
		else if ( m_dwSubStep == TWO_ )
		{
			...
			m_pGaeaClient->GetInterface()->SetTutorialViewStep( 4 );

			// 인벤토리 우선 닫기.
			if( GLUseFeatures::GetInstance().IsUsingRenewInvenWindow() )
			{
				if( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( INVENTORY_WINDOW_RENEW ) )
					m_pGaeaClient->GetInterface()->UiHideGroup( INVENTORY_WINDOW_RENEW );
			}
			else
			{
				if( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( INVENTORY_WINDOW ) )
					m_pGaeaClient->GetInterface()->UiHideGroup( INVENTORY_WINDOW );
			}
		...
		{	// 퀵슬롯 특정 아이템 등록 확인.
			...
			{
				// 아직 인벤토리를 열지 않았다면,
				if ( m_pGaeaClient->GetInterface()->IsInventoryOpen() )
				{
					m_bUICheck = true;
				...
				else
				{
					if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
					{
						...
						m_pGaeaClient->GetInterface()->SetTutorialDialogueExplainText( ID2GAMEINTEXT( "TUTORIAL_MODE_10" ) );
			...
			if ( m_pGaeaClient->GetInterface()->IsInventoryOpen() )
			{
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogueExplainText( ID2GAMEINTEXT( "TUTORIAL_MODE_1" ) );
				...
				if (m_pGaeaClient->GetInterface()->ExplainIsSlotBoxVisibleAtInven())
				{
					if (!m_pGaeaClient->GetInterface()->UiIsVisibleGroup(BALLOON_TEXT_WINDOW))
					{
						m_pGaeaClient->GetInterface()->OpenBalloonText();
			...
			{
				if ( !holditem::HaveHoldItem()/*pGLChar->VALID_HOLD_ITEM()*/ )
		...
		else if ( m_dwSubStep == FOUR_ )
		{
			...
			m_pGaeaClient->GetInterface()->SetTutorialViewStep( 5 );

			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
			{
				m_pGaeaClient->GetInterface()->OpenTutorialDialogue();

				m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_FOUR_3" ) );
			}
		}
		else if ( m_dwSubStep == FIVE_ )
		{
			...
			// 스킬창 우선 닫기.
			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( SKILL_WINDOW ) )
			{
				m_pGaeaClient->GetInterface()->UiHideGroup( SKILL_WINDOW );
			}
			//
		...
		else if ( m_dwSubStep == SIX_ )
		{
			...
			{	// 아직 스킬윈도우 열지 않았다면,
				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( SKILL_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SkillWindowSetVisiblePageForTutorial();
				}
				else
				{
					if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
					{
						...
						m_pGaeaClient->GetInterface()->SetTutorialDialogueExplainText( ID2GAMEINTEXT( "TUTORIAL_MODE_11" ) );
			...
			if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( SKILL_WINDOW ) )
			{
				...
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
				{
					...
					m_pGaeaClient->GetInterface()->SetTutorialDialogueExplainText( strTemp );
				...
				if ( m_pGaeaClient->GetInterface()->ExplainIsSlotBoxVisible(0))
				{
					if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( BALLOON_TEXT_WINDOW ) )
					{
						m_pGaeaClient->GetInterface()->OpenBalloonText();
		...
		else if ( m_dwSubStep == SEVEN_ )
		{
			...
			if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( SKILL_WINDOW ) )
			{
				...
				if ( m_pGaeaClient->GetInterface()->ExplainIsSlotBoxVisible(1) )
				{
					if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( BALLOON_TEXT_WINDOW ) )
					{
						m_pGaeaClient->GetInterface()->OpenBalloonText();
		...
		else if ( m_dwSubStep == EIGHT_ )
		{
			...
			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
			{
				...
				m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_FOUR_7" ) );
			}
		}
		else if ( m_dwSubStep == NINE_ )
		{
		...
		{	// 특정 위치 강제 이동.
			...
			float fDist = D3DXVec3Length( &( vTargetPt - vPos ) );

			if ( fDist <= 10.0f )	NextSubStep();
		}
		break;
	...
	case ATTACK_STEP:
		...
		{	// 대화문.
			...
			m_pGaeaClient->GetInterface()->SetTutorialViewStep( 6 );

			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
			{
				m_pGaeaClient->GetInterface()->OpenTutorialDialogue();

				m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_FIVE_0" ) );
			}
		}
		else if ( m_dwSubStep == TWO_ )
		{	// 목각인 HIT.
			...
			if ( m_pGaeaClient->GetInterface()->IsVisibleTargetInfoMouseOver() )
			{
				CString strTemp;
				strTemp.Format( ID2GAMEINTEXT( "TUTORIAL_TARGET", 0 ) );

				if ( m_pGaeaClient->GetInterface()->TargetInfoGetMouseOverName() == strTemp )
				{
					if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( BALLOON_TEXT_WINDOW ) )
					{
						m_pGaeaClient->GetInterface()->OpenBalloonText();
		...
		else if ( m_dwSubStep == THREE_ )
		{
			...
			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
			{
				...
				m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_FIVE_2" ) );
			}
		}
		...
		else if ( m_dwSubStep == FIVE_ )
		{
			...
			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
			{
				...
				m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_FIVE_3" ) );
			}
		}
		else if ( m_dwSubStep == SIX_ )
		{	// 목각인 스킬 공격.
		...
		else if ( m_dwSubStep == SEVEN_ )
		{
			...
			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
			{
				...
				m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_FIVE_4" ) );
			}
		}
		else if ( m_dwSubStep == EIGHT_ )
		{	// 목각인 스킬 공격.
		...
		else if ( m_dwSubStep == NINE_ )
		{
			...
			if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
			{
				...
				m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_FIVE_5" ) );
			}
		}
		else if ( m_dwSubStep == TEN_ )
		{
			...
			{
				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( i ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( i );
				}
			}
		...
		else if ( m_dwSubStep == ELEVEN_ )
		{
			...
			if ( m_bTutorial )
			{
				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW ) )
				{
					...
					STARGETID vTARID( CROW_PC, m_pGaeaClient->GETMYGAEAID() + 0 );
					D3DXMATRIX matTrans;
					D3DXMatrixTranslation( &matTrans, m_vClickMoveTargetPos[ 2 ].x, m_vClickMoveTargetPos[ 2 ].y, m_vClickMoveTargetPos[ 2 ].z );
				...
				{
					D3DXVECTOR3 vPOS = m_vClickMoveTargetPos[ 2 ];
					D3DXVECTOR3 vScreen = DxViewPort::GetInstance().ComputeVec3Project ( &vPOS, NULL );
				...
				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( MAPMOVE_DISPLAY ) || m_pGaeaClient->GetInterface()->UiIsVisibleGroup( MULTIMAPMOVE_DISPLAY ) )
				{
					m_pGaeaClient->GetInterface()->UiHideGroup( MAPMOVE_DISPLAY );
					m_pGaeaClient->GetInterface()->UiHideGroup ( MULTIMAPMOVE_DISPLAY );
				}
				...
				if ( !m_bClickMove[ 2 ] )
				{
					D3DXVECTOR3 vPos = m_pGaeaClient->GetCharacter()->GetPosition();
					vPos.y = 0.0f;
					D3DXVECTOR3 vTarPos = m_vClickMoveTargetPos[ 2 ];
					vTarPos.y = 0.0f;
					...
					if ( fDist <= 10.0f )
					{
						...
						if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_WINDOW ) )
						{
							...
							m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_END" ) );
						}
					}
				}
			}
		...
		else if ( m_dwSubStep == TWELVE_ )
		{
			...
			if ( m_bTutorial )
			{
				...
				GLMSG::SNETPC_REQ_TUTORIAL_COMPLETE NetTutoComplet;
				NetTutoComplet.dwChaNum = m_pGaeaClient->GetCharacter()->CharDbNum();
				m_pGaeaClient->NETSENDTOFIELD( &NetTutoComplet );
				

				m_bTutorial = false;
				m_pGaeaClient->GetCharacter()->m_bTutorial = false;
				...
				m_pGaeaClient->GetInterface()->ProgressDisplayShowPulblic( ID2GAMEINTEXT( "TUTORIAL_END_GREET" ) );
			}
		}
	...
	//		m_pGaeaClient->GetInterface()->SetTutorialViewStep( 7 );

	//		if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_SKIP_BUTTON ) )
	//		{
	//			m_pGaeaClient->GetInterface()->UiHideGroup( TUTORIAL_SKIP_BUTTON );
	//		}
	...
	//			m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_SIX_0" ) );
	//		}
	//	}
	//	else if ( m_dwSubStep == TWO_ )
	//	{
	...
	//		if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( MAP_WINDOW ) )
	//		{
	//			// Memo : 미니맵을 나중에 열어야, 미니맵 클릭시 미니맵 UI가 판단하게 된다.
	//			if (m_pGaeaClient->GetInterface()->MinimapTargetIsEnable())
	//			{
	//				if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( MINIMAP_TARGET ) )
	//					m_pGaeaClient->GetInterface()->UiShowGroupTop( MINIMAP_TARGET );
	//			}
	//			else
	//			{
	//				if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( MINIMAP_TARGET ) )
	...
	//		//
	//		if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( TUTORIAL_DIALOGUE_EXPLAIN_WINDOW )
	//			&& !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( SYSTEM_MESSAGE_WINDOW ) )
	//		{
	//			m_pGaeaClient->GetInterface()->OpenTutorialDialogueExplainText();
	...
	//		//		if ( !m_pGaeaClient->GetInterface()->UiIsVisibleGroup( BALLOON_SPACE_BAR ) )
	//		//		{
	//		//			m_pGaeaClient->GetInterface()->UiShowGroupTop( BALLOON_SPACE_BAR );
	//		//		}
	//		//	}
	//		//	else
	...
	//		//		if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( BALLOON_SPACE_BAR ) )
	//		//		{
	//		//			m_pGaeaClient->GetInterface()->UiHideGroup( BALLOON_SPACE_BAR );
	//		//		}
	//		//	}
	//		//}
	//		//else
	...
	//		m_pGaeaClient->GetInterface()->SetTutorialViewStep( 8 );
	...
	//			m_pGaeaClient->GetInterface()->SetTutorialDialogueIndex(1);

	//			m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_SIX_1" ) );
	//		}
	//	}
	//	else if ( m_dwSubStep == SIX_ )
	...
	//			GLMSG::SNETPC_REQ_TUTORIAL_COMPLETE_MOVE NetMsg;
	//			NetMsg.dwChaNum = m_pGaeaClient->GetCharacter()->CharDbNum();
	//			NetMsg.sMapID = MapID(SNATIVEID(22,0));
	//			NetMsg.dwGateID = 0;
	...
	//				{
	//					m_pGaeaClient->GetInterface()->SetTutorialDialogueIndex(0);
	//					m_pGaeaClient->GetInterface()->TutorialDialogueSoundIndexSet(46);

	//					m_pGaeaClient->GetInterface()->OpenTutorialDialogue();

	//					m_pGaeaClient->GetInterface()->SetTutorialDialogText( ID2GAMEINTEXT( "TUTORIAL_DIALOGUE_STEP_SEVEN_0" ) );
	//				}
	//			}
	//			else
	//			{
	...
	//		if ( m_dwSubStep == ONE_ )
	//		{
	//			if ( m_pGaeaClient->GetInterface()->UiIsVisibleGroup( DIALOGUE_WINDOW ) )
	//			{
	//				m_pGaeaClient->GetInterface()->UiHideGroup( DIALOGUE_WINDOW );
	//			}
	...
	//			{
	//				m_pGaeaClient->GetInterface()->SetTutorialDialogueIndex(0);
	//				m_pGaeaClient->GetInterface()->TutorialDialogueSoundIndexSet(42);
	//				m_pGaeaClient->GetInterface()->OpenTutorialDialogue();
	...
	//			if (!m_pGaeaClient->GetInterface()->UiIsVisibleGroup(QUEST_WINDOW))
	//			{
	//				m_pGaeaClient->GetInterface()->SetQuestWindowOpen(m_dwQuestUI);
	//				m_pGaeaClient->GetInterface()->SetQuestWindowShowPage(CQuestWindow::QUEST_LIST, true);
	//				m_pGaeaClient->GetInterface()->UiHideGroup(QUEST_WINDOW);
	//			}
	//		}
	//		else if ( m_dwSubStep == TWO_ )
	//		{
 ...
 //                   DWORD dwCurSounIndex = m_pGaeaClient->GetInterface()->GetTutorialDialogueSoundIndex();
 //                   dwCurSounIndex += 2;
 //                   m_pGaeaClient->GetInterface()->TutorialDialogueSoundIndexSet( dwCurSounIndex );
 //               }
	//		}
	//		else if ( m_dwSubStep == THREE_ )
	//		{
	...
	//				{
	//					m_pGaeaClient->GetInterface()->SetQuestWindowOpen( m_dwQuestUI );
	//					m_pGaeaClient->GetInterface()->SetQuestWindowShowPage ( CQuestWindow::QUEST_MAIN, true );
 ...
 //                           const long lResolution = CUIMan::GetResolution();
	...
	//						const UIRECT& rcWindow = m_pGaeaClient->GetInterface()->GetQuestWindowGlobalPos();
	//						const UIRECT& rcTarWindow = m_pGaeaClient->GetInterface()->QBoxButtonGetGlobalPos();

	//						D3DXVECTOR2 vPos;
	//						vPos.x = X_RES - rcWindow.sizeX * 1.001f;
	//						vPos.y = rcTarWindow.top - rcWindow.sizeY * 1.001f;

	//						m_pGaeaClient->GetInterface()->SetQuestWindowGlobalPos( vPos );
	//					}
	//				}
	//				else
	//				{
 ...
 //                       const UIRECT& rcWindow = m_pGaeaClient->GetInterface()->GetQuestWindowRenewalGlobalPos();
	//					const UIRECT& rcTarWindow = m_pGaeaClient->GetInterface()->QBoxButtonGetGlobalPos();
 ...
 //                       m_pGaeaClient->GetInterface()->SetQuestWindowRenewalGlobalPos( vPos );
	//				}
	//			}
	//			else
	//			{
	//				m_pGaeaClient->GetInterface()->SetQuestWindowAutoSelectPage( m_dwQuestUI );
	//				m_pGaeaClient->GetInterface()->SetQuestWindowShowPage ( CQuestWindow::QUEST_STEP, true );
 ...
 //                       const long lResolution = CUIMan::GetResolution();
	...
	if ( bCharMoveBlock )
		m_pGaeaClient->GetInterface()->SetTutorialDialogueEnable( false );
	else
		m_pGaeaClient->GetInterface()->SetTutorialDialogueEnable( true );
...
{
	ClientActor* pCrowActor = m_pGaeaClient->GetCrow( emCrow, dwID );
	if ( !pCrowActor )
	{
		m_pGaeaClient->PrintConsoleTextDlg( "Invalid ClientActor" );
		return;
	}

	PGLCROWCLIENT pCrow = (PGLCROWCLIENT)pCrowActor;
	if ( !pCrow )
	{
		m_pGaeaClient->PrintConsoleTextDlg( "Invalid Mob Target" );
		return;
	}
	
	SNATIVEID tempID = pCrow->m_pCrowData->m_sBasic.sNativeID;

	if ( tempID == NATIVEID_NULL())
	{
		m_pGaeaClient->PrintConsoleTextDlg( "Invalid Target Native ID" );
		return;
	}
