...
SerialMemory::SerialMemory(const char *_szZipFileName, const char *_szFileName, EMBYTECRYPT emCrypt /*= EMBYTECRYPT_NONE*/) 
	: BaseStream()
    , m_pBuffer(NULL)
	, m_nBufSize(0)
	, m_dwOffSet(0)
{
    ...
    OpenFile(std::string(_szZipFileName), std::string(_szFileName), emCrypt);
}
