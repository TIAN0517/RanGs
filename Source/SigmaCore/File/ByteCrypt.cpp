...
namespace BYTECRYPT
{
		...
		for ( int nArray=0; nArray<EMBYTECRYPT_SIZE; ++nArray )
		{
			for ( int i=0; i<256; ++i )
			{
				for ( int j=0; j<256; ++j )
				{
					...
					{
						//DebugSet::GetInstance().MsgBox( "%s Duplicate In:%d~%d Value:%x", strCrypt[nArray].c_str(), i, j, ARRAY_USE[nArray][i] );
						std::string str = sc::string::format("%s Duplicate In:%d~%d Value:%x", strCrypt[nArray].c_str(), i, j, ARRAY_USE[nArray][i] );
						//DebugSet::GetInstance()->LogToFile(str);
						::MessageBoxA( NULL, str.c_str(), "Byte Crypt Error!", MB_ICONEXCLAMATION | MB_OK );
					}
				}
			}
		}
	...
	void byte_encode ( BYTE* pbuffer, DWORD dwSize, EMBYTECRYPT emCRYPT )
	{
		if ( !pbuffer )
		{
			::MessageBoxA( NULL, "byte_encode Invalid buffer", "Byte Crypt Error!", MB_ICONEXCLAMATION | MB_OK );
		}
	...
	void byte_decode ( BYTE* pbuffer, DWORD dwSize, EMBYTECRYPT emCRYPT )
	{
		if ( !pbuffer )
		{
			::MessageBoxA( NULL, "byte_decode Invalid buffer", "Byte Crypt Error!", MB_ICONEXCLAMATION | MB_OK );
		}
