...
//      GetFileInfo(pZipFile, info);
		...
		//HANDLE hOutputFile;
		////if ( strcmp ( ZipFileName.c_str(),"widgetscript.temp") == 0 || strcmp ( ZipFileName.c_str(),"widgetscript.wsf") == 0 || strcmp ( ZipFileName.c_str(),"widgetscript.rcc") == 0 )
		//{
		//	std::string ZipFName = ZipFileName;

		//	std::string strPath ( sc::file::changeFileExt( ZipFName, "folder" ) );
		//	strPath += "\\";
		//	strPath.append( FileName );

		//	hOutputFile = ::CreateFile(strPath.c_str(), 
		...
		//	sc::writeLogError( sc::string::format( "CreateFile %1% zip name %2%",strPath.c_str(),ZipFileName ) );
		...
		//		//unzCloseCurrentFile(pZipFile);
		//		sc::writeLogError( "INVALID_HANDLE_VALUE" );
		//	}
		//}
        ...
        do
        {
            ...
            if (nRet > 0)
            {
				...
				//	{
				//		nRet = UNZ_ERRNO;
				//		sc::writeLogError( sc::string::format( "CreateFile %1% from zip name %2% succeed",FileName,ZipFileName ) );
				//	}
				//}
            }
        }
