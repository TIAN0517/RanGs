...
CBasicLineBox::CBasicLineBox (EngineDeviceMan* pEngineDevice)
    : CUIGroup(pEngineDevice)
	, m_pLeft( NULL )
    , m_pRight( NULL )
    , m_pTop( NULL )
    , m_pBottom( NULL )
    , m_bCalcResizeWindow( FALSE )
{
}
...
void CBasicLineBox::CreateNewBaseBoxEditBox ( char* szBoxControl )
{
	CreateBodyImage ( "BASIC_NEW_LINE_BOX_BODY_EDIT" );
	CreateLineImage ( "BASIC_NEW_LINE_BOX_LEFT_EDIT", "BASIC_NEW_LINE_BOX_RIGHT_EDIT", "BASIC_NEW_LINE_BOX_TOP_EDIT", "BASIC_NEW_LINE_BOX_BOTTOM_EDIT" );	
	ResizeControl ( szBoxControl );
}

void CBasicLineBox::CreateNewBaseBoxEditBoxOver ( char* szBoxControl )
{
	CreateBodyImage ( "BASIC_NEW_LINE_BOX_BODY_EDIT_OVER" );
	CreateLineImage ( "BASIC_NEW_LINE_BOX_LEFT_EDIT_OVER", "BASIC_NEW_LINE_BOX_RIGHT_EDIT_OVER", "BASIC_NEW_LINE_BOX_TOP_EDIT_OVER", "BASIC_NEW_LINE_BOX_BOTTOM_EDIT_OVER" );	
	ResizeControl ( szBoxControl );
}
...
void CBasicLineBox::CreateBaseBoxStatsReset ( char* szBoxControl )
{
	CreateBodyImage ( "BASIC_LINE_BOX_BODY_STATSRESET" );
	CreateLineImage ( "BASIC_LINE_BOX_LEFT_STATSRESET", "BASIC_LINE_BOX_RIGHT_STATSRESET", "BASIC_LINE_BOX_TOP_STATSRESET", "BASIC_LINE_BOX_BOTTOM_STATSRESET" );
	ResizeControl ( szBoxControl );
}

void CBasicLineBox::CreateBaseBoxPostBoxSendInput ( char* szBoxControl )
{
	CreateBodyImage ( "BASIC_LINE_BOX_BODY_POSTBOX_PAGE_SEND_INPUT" );
	CreateLineImage ( "BASIC_LINE_BOX_LEFT_POSTBOX_PAGE_SEND_INPUT", "BASIC_LINE_BOX_RIGHT_POSTBOX_PAGE_SEND_INPUT", "BASIC_LINE_BOX_TOP_POSTBOX_PAGE_SEND_INPUT", "BASIC_LINE_BOX_BOTTOM_POSTBOX_PAGE_SEND_INPUT" );
	ResizeControl ( szBoxControl );
}
...
void CBasicLineBox::ResizeWindow(const UIRECT& rcNew)
{
	if( m_bCalcResizeWindow )
	{
		...
		if( m_pLeft ) m_pLeft->SetLocalPos(rcChange);

		rcChange.sizeX	= 1;
		rcChange.sizeY	= rcNew.sizeY;
		rcChange.left	= rcNew.sizeX - 1;
		rcChange.top	= 0;
		rcChange.right	= rcChange.left + rcChange.sizeX;
		rcChange.bottom	= rcChange.top + rcChange.sizeY;

		if( m_pRight ) m_pRight->SetLocalPos(rcChange);

		rcChange.sizeX	= rcNew.sizeX - 2;
		rcChange.sizeY	= 1;
		rcChange.left	= 1;
		rcChange.top	= 0;
		rcChange.right	= rcChange.left + rcChange.sizeX;
		...
		if( m_pTop ) m_pTop->SetLocalPos(rcChange);

		rcChange.sizeX	= rcNew.sizeX - 2;
		rcChange.sizeY	= 1;
		rcChange.left	= 1;
		rcChange.top	= rcNew.sizeY - 1;
		rcChange.right	= rcChange.left + rcChange.sizeX;
		rcChange.bottom	= rcChange.top + rcChange.sizeY;
