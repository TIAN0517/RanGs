...
const char DxEffectReflect::NAME[] = "[ Plane reflection effect (simple) ]";
const DWORD DxEffectReflect::FLAG = _EFF_SINGLE_AFTER_1;
...
HRESULT	DxEffectReflect::CreateVB( LPDIRECT3DDEVICEQ pd3dDevice, DxMeshes* pmsMeshs, D3DXMATRIX& matWorld )
{
	...
	m_sReflectAABB.Create( pd3dDevice, (BYTE*)pSrcVertices, pSrcIndices, dwFaces, matWorld, VERTEX_WORLD::FVF );
...
void DxEffectReflect::SaveBuffer ( sc::SerialFile &SFile )
{
	SFile.BeginBlock();
	{
...
void DxEffectReflect::LoadBufferSet ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
...
void DxEffectReflect::LoadBuffer ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
	SFile >> dwBuffSize; //	읽지 않고 스킵용으로 사용되므로 실제 로드에서는 무의미.
...
HRESULT DxEffectReflect::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
