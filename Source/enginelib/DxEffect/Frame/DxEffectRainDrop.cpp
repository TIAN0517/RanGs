...
const char DxEffectRainPoint::NAME[] = "[ Raindrops, snow, fallen leaves ]";	// 벚꽃
...
void DxEffectRainPoint::OnCreateDevice_STATIC( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
void DxEffectRainPoint::OnDestroyDevice_STATIC()
{
...
void DxEffectRainPoint::OnResetDevice_STATIC( LPDIRECT3DDEVICEQ pd3dDevice )
{
	D3DCAPSQ d3dCaps;
...

	{
		...
		// 새로운 눈 효과 버텍스
		{
			...
			pd3dDevice->CreateVertexBuffer( 4 * sizeof(SNOWRENDER), 0, SNOWRENDER::FVF, D3DPOOL_MANAGED, &m_pNewSnowVB, NULL );
			VOID *pVertices;
		...
		// 새로운 낙엽 효과 버텍스
		{
			...
			pd3dDevice->CreateVertexBuffer( 4 * sizeof(LEAVESRENDER), 0, LEAVESRENDER::FVF, D3DPOOL_MANAGED, &m_pNewLeavesVB, NULL );
			VOID *pVertices;
	...
	OnResetDevice_FireFly_STATIC( pd3dDevice );
}

void DxEffectRainPoint::OnLostDevice_STATIC ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	OnLostDevice_FireFly_STATIC( pd3dDevice );
}
...
{
    m_textureResRain = TextureManager::GetInstance().LoadTexture(
        m_szRainTex,
        false,
    ...
    m_textureResCircle = TextureManager::GetInstance().LoadTexture(
        m_szCircleTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
    ...
    m_textureResRainDrop = TextureManager::GetInstance().LoadTexture(
        m_szRainDropTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
    ...
    m_textureResSnow = TextureManager::GetInstance().LoadTexture(
        m_szSnowTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
    ...
    m_textureResLeaves = TextureManager::GetInstance().LoadTexture(
        m_szLeavesTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
    ...
    m_textureResGround = TextureManager::GetInstance().LoadTexture(
        m_szGroundTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
    ...
    m_textureResRainG = TextureManager::GetInstance().LoadTexture(
        m_szRain_GTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
    ...
    m_textureResSnowG = TextureManager::GetInstance().LoadTexture(
        m_szSnow_GTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
	...
	//
	SetSeperateTexture();
	m_NewRainData.SetTexture(
        m_textureResRain.GetTexture(),
        m_textureResCircle.GetTexture(),
        m_textureResRainDrop.GetTexture(),
        m_textureResSnow.GetTexture(),
        m_textureResLeaves.GetTexture(),
        m_textureResGround.GetTexture(),
        m_textureResRainG.GetTexture(),
        m_textureResSnowG.GetTexture() );
...
{
    m_textureResRain.Clear();
    m_textureResCircle.Clear();
    m_textureResRainDrop.Clear();
    m_textureResSnow.Clear();
    m_textureResLeaves.Clear();
    m_textureResGround.Clear();
    m_textureResRainG.Clear();
    m_textureResSnowG.Clear();
...
//		vPos = D3DXVECTOR3( sc::Random::RandomNumber( m_vMin.x, m_vMax.x ), sc::Random::RandomNumber( 0.0f, m_fParticleYPos), sc::Random::RandomNumber( m_vMin.z, m_vMax.z ) );
//		vDir = D3DXVECTOR3( sc::Random::RandomNumber( -0.5f, 0.5f ), 0.0f, sc::Random::RandomNumber( -0.5f, 0.5f ) );
//
//		fDisappearYPos = sc::Random::RandomNumber( m_fMinDisappearYPos, m_fMaxDisappearYPos );
//		fFallSpeed = sc::Random::RandomNumber( -10.0f, -5.0f );
//		fWaveRate = sc::Random::RandomNumber( 4.0f, 10.0f );
//
...
float DxEffectRainPoint::CalcSurfaceArea(VERTEX_WORLD* pFV[3])
{
	float s;
	float len[3];
	...
	{
		float x,y,z;
		x = pFV[(i+1)%3]->vPos.x - pFV[i]->vPos.x;
		y = pFV[(i+1)%3]->vPos.y - pFV[i]->vPos.y;
		z = pFV[(i+1)%3]->vPos.z - pFV[i]->vPos.z;
...
//		GXRELEASE(*pEffectMesh);
	...
	for( DWORD j=0; j<Faces; j++ )
	{
		...
		while ( area>1.0f )
		{
			...
			s.x = Array[1]->vPos.x - Array[0]->vPos.x;
			s.y = Array[1]->vPos.y - Array[0]->vPos.y;
			s.z = Array[1]->vPos.z - Array[0]->vPos.z;
			t.x = Array[2]->vPos.x - Array[0]->vPos.x;
			t.y = Array[2]->vPos.y - Array[0]->vPos.y;
			t.z = Array[2]->vPos.z - Array[0]->vPos.z;
			pRainVertex[dwRainVert].x = Array[0]->vPos.x + s.x*r1 + t.x*r2;							// 1 정점 근처에 Vertex를 생성
			pRainVertex[dwRainVert].y = Array[0]->vPos.y + s.y*r1 + t.y*r2;
			pRainVertex[dwRainVert].z = Array[0]->vPos.z + s.z*r1 + t.z*r2;
...
void DxEffectRainPoint::SaveBuffer ( sc::SerialFile &SFile )
{
	//	읽지 않고 스킵용으로 사용됨.
	...
	if( m_bNewRainEffect )
	{
		...
		int nSize = static_cast<int> (m_NewRainData.vecParticleArea.size());
...
void DxEffectRainPoint::LoadBufferSet ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
...
void DxEffectRainPoint::LoadBuffer ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
...
HRESULT DxEffectRainPoint::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
	GASSERT(pd3dDevice);
