...
const char	DxEffectSpecular::NAME[] = "[ Specular (recommended) ]";
	...
	if ( pframeCur->pmsMeshs != NULL )
	{
		...
		while ( pmsMeshs != NULL )
		{
			if ( pmsMeshs->m_pLocalMesh || pmsMeshs->m_pDxOctreeMesh )
			{
				...
				{
					//	Note : 메터리얼, 텍스쳐 지정.
					pmsMeshs->m_rgMaterials[i].Power = 30.f;
					pmsMeshs->m_rgMaterials[i].Specular.r = 1.f;
					pmsMeshs->m_rgMaterials[i].Specular.g = 1.f;
					pmsMeshs->m_rgMaterials[i].Specular.b = 1.f;
					pmsMeshs->m_rgMaterials[i].Specular.a = 1.f;
					pd3dDevice->SetMaterial ( &( pmsMeshs->m_rgMaterials[i] ) );
					pd3dDevice->SetTexture ( 0, pmsMeshs->m_pTextureRes[pAttribTable[i].AttribId].GetTexture() );
