...
const char	DxEffectShadow::NAME[] = "[ Shadow Effect-Ground ]";
const DWORD DxEffectShadow::FLAG = _EFF_SINGLE_AFTER_0;
...
HRESULT DxEffectShadow::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
        m_textureResSSDay = TextureManager::GetInstance().LoadTexture(
            m_szTexDayTime,
            false,
		...
		if ( m_textureResSSDay.GetTexture() )
		{
			D3DSURFACE_DESC pDesc;
			m_textureResSSDay.GetTexture()->GetLevelDesc ( 0, &pDesc );
			m_dwTexSize = pDesc.Width;
		}

        m_textureResSSNight = TextureManager::GetInstance().LoadTexture(
            m_szTexNight,
            false,
            TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
            0,
		...
		if ( m_textureResSSNight.GetTexture() )
		{
			D3DSURFACE_DESC pDesc;
			m_textureResSSNight.GetTexture()->GetLevelDesc ( 0, &pDesc );
			m_dwTexSize = pDesc.Width;
		}
	}
...
HRESULT DxEffectShadow::DeleteDeviceObjects ()
{
    ...
    m_textureResSSDay.Clear();
    m_textureResSSNight.Clear();
...
HRESULT DxEffectShadow::Render ( DxFrame *pframeCur, LPDIRECT3DDEVICEQ pd3dDevice, DxFrameMesh *pFrameMesh, DxLandMan* pLandMan )
{
	...
	if ( DxShadowMap::GetInstance().GetShadowDetail() >= LAND_QT_MIDDLE )
	{
		...
		matLastWVP_D	= DxShadowMap::GetInstance().GetMatrix_OLD ();
	...
	if ( pLandMan )	
	{
		...
		if( (m_dwFlag&USESTATICSHADOW) && 
			DxEffectMan::GetInstance().IsDeviceStaticShadow() && 
			DxShadowMap::GetInstance().GetShadowDetail() >= LAND_QT_LOW )
		{
			m_sShadowAABB.RenderStatic( pd3dDevice, sCV, m_textureResSSDay.GetTexture(), TRUE );
		}

		if ( DxSurfaceTex::GetInstance().m_pShadowTex )
		{
	...
	else
	{
		if( (m_dwFlag&USESTATICSHADOW) && 
			...
			DxShadowMap::GetInstance().GetShadowDetail() >= LAND_QT_LOW )
		{
			m_sShadowAABB.RenderStatic( pd3dDevice, sCV, m_textureResSSDay.GetTexture(), FALSE );
		}
...
void DxEffectShadow::SaveBuffer ( sc::SerialFile &SFile )
{
	SFile.BeginBlock();
	{
...
void DxEffectShadow::LoadBufferSet ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
...
void DxEffectShadow::LoadBuffer ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
...
HRESULT DxEffectShadow::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
