...
const char	DxEffectNeon::NAME[] = "[ Direction indication, fine flow of texture ]";
...
void DXMATERIAL_NEON::SaveFile( sc::SerialFile &SFile )
{
    SFile << bUse;
    SFile.WriteBuffer( &vMoveTex, sizeof( D3DXVECTOR2 ) );
    SFile.WriteBuffer( &vScaleUV, sizeof( D3DXVECTOR2 ) );
    SFile << vColor;
    ...
    SFile.WriteBuffer( szNeonTex, sizeof( char ) * MAX_PATH );
}

void DXMATERIAL_NEON::LoadFile( sc::SerialFile &SFile )
{
    SFile >> bUse;
    SFile.ReadBuffer( &vMoveTex, sizeof( D3DXVECTOR2 ) );
    SFile.ReadBuffer( &vScaleUV, sizeof( D3DXVECTOR2 ) );
    SFile >> vColor;
    ...
    SFile.ReadBuffer( szNeonTex, sizeof( char ) * MAX_PATH );
}
...
void DxEffectNeon::ResetTexture ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
        m_pMaterials[i].m_textureResSrc = TextureManager::GetInstance().LoadTexture(
        ...
        m_pMaterials[i].m_textureResNeon = TextureManager::GetInstance().LoadTexture(
            m_pMaterials[i].szNeonTex,
            false,
            TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
            0,
            D3DFMT_UNKNOWN,
...
HRESULT DxEffectNeon::Render ( DxFrame *pframeCur, LPDIRECT3DDEVICEQ pd3dDevice, DxFrameMesh *pFrameMesh, DxLandMan* pLandMan )
{
	...
	for ( DWORD i=0; i<m_dwAttribTable; i++ )
	{
		...
		if ( pframeCur )
			pd3dDevice->SetMaterial ( &pframeCur->pmsMeshs->m_rgMaterials[ dwAttribld ] );
		...
		if ( m_pMaterials[ dwAttribld ].bUse )
		{
			...
			DxImageMove ( pd3dDevice, m_pMaterials[ dwAttribld ].m_textureResNeon.GetTexture(), 
		...
		pd3dDevice->SetTexture ( 0, m_pMaterials[ dwAttribld ].m_textureResSrc.GetTexture() );
...
HRESULT DxEffectNeon::DxAnalysis ( LPDIRECT3DDEVICEQ pd3dDevice, DxMeshes *pmsMeshs )
{
	...
	for ( DWORD i=0; i<m_dwMaterials; i++ )
	{
		...
		if ( !strlen(m_pMaterials[i].szNeonTex) )
			StringCchCopy( m_pMaterials[i].szNeonTex, MAX_PATH, pmsMeshs->m_strTextureFiles[i].GetString () );
...
HRESULT DxEffectNeon::CreateObject( LPDIRECT3DDEVICEQ pd3dDevice, DxMeshes *pmsMeshs )
{
	...
	{
		pNormalVert[i].vPos		= pVertices[i].vPos;
		pNormalVert[i].vNor		= pVertices[i].vNor;
		pNormalVert[i].vTex		= pVertices[i].vTex[0];
	}
	m_pSrcVB->Unlock ();
...
void DxEffectNeon::SaveBuffer ( sc::SerialFile &SFile )
{
	//	읽지 않고 스킵용으로 사용됨.
	SFile << sizeof(DWORD)+sizeof(OBJECT)*m_dwVertices
			...
			+sizeof(DWORD)+sizeof(DXMATERIAL_NEON_OLD)*m_dwMaterials;
...
void DxEffectNeon::LoadBufferSet ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
	OBJECT*		pTemp;
	WORD*		 pIndices;
...
void DxEffectNeon::LoadBuffer ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
	PBYTE pbBuff;
	PBYTE pbPoints;
...
HRESULT DxEffectNeon::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
	HRESULT hr;
