...
const char DxEffectCloth::NAME[] = "[ Clothes, tent effect ]";
...
HRESULT DxEffectCloth::AdaptToDxFrameChild ( DxFrame *pframeCur, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	//			StringCchCopy( m_szTexture, MAX_PATH, (char*)pmsMeshs->m_strTextureFiles->GetBuffer() );
...
HRESULT DxEffectCloth::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if( m_textureRes.GetTexture() )
	{
		...
		if( emType == EMTT_ALPHA_HARD )			m_bAlphaMap = TRUE;
		else if( emType == EMTT_ALPHA_SOFT )	m_bAlphaMap = TRUE;
		else if( emType == EMTT_ALPHA_SOFT01 )	m_bAlphaMap = TRUE;
		else if( emType == EMTT_ALPHA_SOFT02 )	m_bAlphaMap = TRUE;
...
HRESULT DxEffectCloth::Render ( DxFrame *pframeCur, LPDIRECT3DDEVICEQ pd3dDevice, DxFrameMesh *pFrameMesh, DxLandMan* pLandMan )
{
	...
	{
		DWORD dwVertexSizeFULL = m_dwNumVertices*DxDynamicVB::g_sVB_PNTI.GetFVFSize();
		...
		pd3dDevice->SetFVF( DxDynamicVB::g_sVB_PNTI.dwFVF );
...
{	
	...
	{
		CDebugSet::ToLogFile(
            std::string("DxEffectCloth::XFileLoad() -- pVB->Lock() -- Failed"));
		return;
	}
	...
	{
		CDebugSet::ToLogFile(
            std::string("DxEffectCloth::XFileLoad() -- pIB->Lock() -- Failed"));
		return;
	}
...
BOOL DxEffectCloth::CreateVertex( LPDIRECT3DDEVICEQ pd3dDevice, DWORD dwNumVertices, VERTEX* pVertices, DWORD dwNumIndices, WORD* pIndices )
{
	...
	{
		CDebugSet::ToLogFile(
            std::string("DxEffectCloth::CreateVertex() -- CreateIndexBuffer() -- Failed"));
		return FALSE;
	}
	...
	{
		CDebugSet::ToLogFile(
            std::string("DxEffectCloth::CreateVertex() -- pIB->Lock() -- Failed"));
		return FALSE;
	}
...
void DxEffectCloth::SaveBuffer ( sc::SerialFile &SFile )
{
	//	읽지 않고 스킵용으로 사용됨.
...
void DxEffectCloth::LoadBufferSet ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
	VERTEX*	pClothVertex;
	WORD*			pIndices;
...
void DxEffectCloth::LoadBuffer ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT(pd3dDevice);
...
HRESULT DxEffectCloth::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
