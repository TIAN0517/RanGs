...
const char DxEffectRenderState::NAME[] = "[ Adjust render, flicker ]";
...
void DxEffectRenderState::SaveBuffer ( sc::SerialFile &SFile )
{
	//	Size 저장 부분.
	SFile.BeginBlock ();
	{
...
void DxEffectRenderState::LoadBufferSet ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	if ( dwVer == VERSION )
	{
...
void DxEffectRenderState::LoadBuffer ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	if ( dwVer == VERSION )
	{
		DWORD dwBuffSize = SFile.ReadBlockSize (); //	읽지 않고 스킵용으로 사용되므로 실제 로드에서는 무의미.
...
HRESULT DxEffectRenderState::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
