...
const char	DxEffectRiver::NAME[] = "[ Water effect (big water) (refraction, reflection) ]";
...
{
    m_textureResDark = TextureManager::GetInstance().LoadTexture(
        m_szTexDark,
        false,
    ...
    m_textureResFlash = TextureManager::GetInstance().LoadTexture(
        m_szTexFlash,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
    ...
    m_textureRes1DMap = TextureManager::GetInstance().LoadTexture(
        m_sz1DMap,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
...
{
    m_textureResDark.Clear();
    m_textureResFlash.Clear();
    m_textureResNormal.Clear();
    m_textureRes1DMap.Clear();
...
HRESULT	DxEffectRiver::CreateVB ( LPDIRECT3DDEVICEQ pd3dDevice, DxMeshes* pmsMeshs, D3DXMATRIX& matWorld )
{
	...
	m_sWaterAABB.Create ( pd3dDevice, (BYTE*)pSrcVertices, pSrcIndices, dwFaces, matWorld, VERTEX_WORLD::FVF );
...
void DxEffectRiver::SaveBuffer ( sc::SerialFile &SFile )
{
	SFile.BeginBlock ();
	{
...
void DxEffectRiver::LoadBufferSet ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
...
void DxEffectRiver::LoadBuffer ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
	BOOL bExt;
...
HRESULT DxEffectRiver::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
