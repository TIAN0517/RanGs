...
const char DxEffectNature::NAME[] = "[ Natural phenomenon ]";
...
{
    m_textureResButter = TextureManager::GetInstance().LoadTexture(
        m_szButterTex,
        false,
    ...
    m_textureResShadow = TextureManager::GetInstance().LoadTexture(
        m_szShadowTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
    ...
    m_textureResBug = TextureManager::GetInstance().LoadTexture(
        m_szBugTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
...
{
    m_textureResButter.Clear();
    m_textureResShadow.Clear();
    m_textureResBug.Clear();
...
HRESULT DxEffectNature::Render ( DxFrame *pframeCur, LPDIRECT3DDEVICEQ pd3dDevice, DxFrameMesh *pFrameMesh, DxLandMan* pLandMan )
{
	...
	if ( m_pBugVB )
	{
		...
		pd3dDevice->SetTexture( 0, m_textureResBug.GetTexture() );
	...
	if ( m_pButterVB )
	{
		...
		pd3dDevice->SetTexture( 0, m_textureResButter.GetTexture() );
		...
		pd3dDevice->SetTexture( 0, m_textureResShadow.GetTexture() );
...
float DxEffectNature::CalcSurfaceArea(VERTEX_WORLD* pFV[3])
{
	float s;
	float len[3];
...
HRESULT DxEffectNature::CreateBaseMesh ( LPDIRECT3DDEVICEQ pd3dDevice, DxMeshes *pmsMeshs, D3DXMATRIX matWorld )
{
	...
	for( DWORD j=0; j<Faces; j++ )
	{
		...
		while (area>1.0f)
		{
			...
			{
				m_pBaseVert[dwRainVert].p.x = Array[0]->vPos.x + s.x*r1 + t.x*r2;							// 1 정점 근처에 Vertex를 생성
				m_pBaseVert[dwRainVert].p.y = Array[0]->vPos.y + s.y*r1 + t.y*r2;
				m_pBaseVert[dwRainVert].p.z = Array[0]->vPos.z + s.z*r1 + t.z*r2;
			}
