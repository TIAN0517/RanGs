...
const DWORD	DECAL_PROPERTY::VERSION	= 0x0101;				// 절대 새로운 버젼을 만들지 마세요.
const char	DECAL_PROPERTY::NAME[]	= "DECAL_PROPERTY";

DxEffSingle* DECAL_PROPERTY::NEWOBJ ( boost::shared_ptr<DxEffSingleGroup>& spThisGroup )
{
...
HRESULT DECAL_PROPERTY::SaveFile ( sc::SerialFile &SFile )
{
	SFile << TYPEID;
	SFile << VERSION;
...
HRESULT DECAL_PROPERTY::LoadFile ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bBackUpCache, BOOL bOldDecalUSE )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
...
HRESULT DECAL_PROPERTY::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bBackUpCache )
{
...
HRESULT DxEffectDecal::RestoreDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	pd3dDevice->CreateIndexBuffer ( 3 * (DxDynamicVB::PDTI_NUM*2) * sizeof(WORD), 0, D3DFMT_INDEX16, D3DPOOL_MANAGED, &m_pDecalIB, NULL );
...
HRESULT	DxEffectDecal::Render ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX &matComb, float fAlpha )
{
	HRESULT hr = S_OK;
	D3DXVECTOR3		v;
	...
	if ( m_dwFaces == 0 || m_vPrevPos!=vCenter )
	{
		if ( spLandMan )
		{
			DxDecal::GetInstance().CreateDecal ( vCenter, vNormal, vTangent, vAreaXYZ, spLandMan.get() );
    ...
    DWORD dwVertexSizeFULL = m_dwVerts*DxDynamicVB::g_sVB_I_24BYTE.GetFVFSize();
    ...
    pd3dDevice->DrawIndexedPrimitive ( D3DPT_TRIANGLELIST, uiStartVertex, 0, m_dwVerts, 0, m_dwFaces );
