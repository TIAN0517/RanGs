...
DxEffSingle* MESH_PROPERTY::NEWOBJ ( boost::shared_ptr<DxEffSingleGroup>& spThisGroup )
{
	...
	pEffect->m_SimMeshBlendRes[0] = m_SimMeshBlendRes[0];
	pEffect->m_SimMeshBlendRes[1] = m_SimMeshBlendRes[1];

	if ( m_dwFlag & USEBLENDMESH )
	{
		if ( !m_SimMeshRes.IsLoading() )
		{
			m_SimMeshRes = DxSimpleMeshMan::GetInstance().Load ( m_szMeshFile, TRUE, TRUE, TEXTURE_RESIZE_NONE, TEXTURE_EFFECT );
		}

		if ( !m_SimMeshBlendRes[0].IsLoading() )
		{
			m_SimMeshBlendRes[0] = DxSimpleMeshMan::GetInstance().Load ( m_szMeshFile1, TRUE, TRUE, TEXTURE_RESIZE_NONE, TEXTURE_EFFECT );
		}

		if ( !m_SimMeshBlendRes[1].IsLoading() )
		{
			m_SimMeshBlendRes[1] = DxSimpleMeshMan::GetInstance().Load ( m_szMeshFile2, TRUE, TRUE, TEXTURE_RESIZE_NONE, TEXTURE_EFFECT );
		}
	}
	else
	{
...
void MESH_PROPERTY::importMobileObjectFromXFile()
{
	if ( m_dwFlag & USEBLENDMESH )
	{
		...
		m_SimMeshBlendRes[0] = DxSimpleMeshMan::GetInstance().Load ( m_szMeshFile1, FALSE, TRUE, TEXTURE_RESIZE_NONE, TEXTURE_EFFECT );
		if ( m_SimMeshBlendRes[0].get() )
		{
			m_SimMeshBlendRes[0].get()->SaveFile();
		}

		m_SimMeshBlendRes[1] = DxSimpleMeshMan::GetInstance().Load ( m_szMeshFile2, FALSE, TRUE, TEXTURE_RESIZE_NONE, TEXTURE_EFFECT );
		if ( m_SimMeshBlendRes[1].get() )
		{
			m_SimMeshBlendRes[1].get()->SaveFile();
		}
	}
	else
	{
...
// -----------------------------------------------------------------------------------------------------------------------------------------
HRESULT MESH_PROPERTY::SaveFile ( sc::SerialFile &SFile )
{
	SFile << TYPEID;
	SFile << VERSION;
...
HRESULT MESH_PROPERTY::LoadFile ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bBackUpCache, BOOL bOldDecalUSE )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	workName = m_szMeshFile1;
	p = workName.find_last_of(".");
	if (p != workName.npos)
	{
		workName = workName.substr(0, p);
		workName += ".x";
	}
	strcpy ( m_szMeshFile1, workName.c_str() );

	workName = m_szMeshFile2;
	p = workName.find_last_of(".");
	if (p != workName.npos)
	{
		workName = workName.substr(0, p);
	...
	strcpy ( m_szMeshFile2, workName.c_str() );
...
// -----------------------------------------------------------------------------------------------------------------------------------------
HRESULT MESH_PROPERTY::InitDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bBackUpCache )
{
	...
	if ( m_dwFlag & USEBLENDMESH )
	{
		...
		//};
		//m_pMesh = DxSimpleMeshMan::GetInstance().BlendLoad ( pd3dDevice, szTex, dwFVF, TRUE );	
	}
	else
		m_SimMeshRes = DxSimpleMeshMan::GetInstance().Load ( m_szMeshFile, TRUE, TRUE, TEXTURE_RESIZE_NONE, TEXTURE_EFFECT );
