...
DxEffSingle* MOVEROTATE_PROPERTY::NEWOBJ ( boost::shared_ptr<DxEffSingleGroup>& spThisGroup )
{
...
HRESULT MOVEROTATE_PROPERTY::SaveFile ( sc::SerialFile &SFile )
{
	SFile << TYPEID;
	SFile << VERSION;
...
HRESULT MOVEROTATE_PROPERTY::LoadFile ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bBackUpCache, BOOL bOldDecalUSE )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
...
HRESULT MOVEROTATE_PROPERTY::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bBackUpCache )
{
	//	Note : 형제, 자식.
	//
...
HRESULT	DxEffectMoveRotate::Render ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX &matComb, float fAlpha )
{
	HRESULT hr = S_OK;
	...
	if ( m_dwFlag & USEGOTOCENTER )
	{
		...
		fTime = fTime * m_rThisGroup->m_fOut_Scale;
...
// 	else if ( m_dwFlag & USEPARENTROTATE )
// 	{
// 		// m_rThisGroup->m_matWorld;
// 		// D3DXMatrixMultiply( &matWorld, &matWorld, &m_rThisGroup->m_matWorld );
