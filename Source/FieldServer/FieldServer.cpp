...
void cmdStartOrStop(HWND hDlg)
{
	...
	{					
		...
		// [shhan][2015.01.30] stop 버튼 누를 시 여기로 들어오고 g_pServer->Stop() 에서 m_bReservationServerStop = true; 가 되며
		//						PostMessage( m_hWnd, WM_FINALCLEANUPFINISH, 0, 0 ); 으로 인해
		//						cmdFinalExit 호출된다.
		//						그것으로 인해 동시에 g_pServer->FinalStop();, SAFE_DELETE(g_pServer); 를 두군데서 하고 있는것. ( 문제가 된다.)
		//
		//						SAFE_DELETE(g_pServer); 에서 ~BOOST_OBJECT_POOL_LOCK_CHECKER 에 GASSERT 가 발생하였고,
		//						동시 2군데 해제작업은 되고 있었는데 Pause 가 되면서 문제가 발생했다. ( 이전에는 순식간에 한곳이 지워서 문제가 안되었나 봄. )
		//
		//						cmdFinalExit 에서 작업을 해주니 cmdStartOrStop 에서는 g_pServer->FinalStop();, SAFE_DELETE(g_pServer);를 해주지 않는 방향으로 하겠음.
		//
		//g_pServer->FinalStop();
...
void ShowPostSender(HWND hDlg)
{
	CFieldServerPostSender dlg( CWnd::FromHandle(hDlg), g_pServer );
	if ( dlg.DoModal() == IDOK )
	{
		return;
		//MessageBox ("Post Send.");
	}
}
