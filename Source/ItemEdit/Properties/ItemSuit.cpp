...
ON_BN_CLICKED(IDC_BUTTON_VAR, &CItemSuit::OnBnClickedButtonVar)
ON_BN_CLICKED(IDC_BUTTON_VOL, &CItemSuit::OnBnClickedButtonVol)
END_MESSAGE_MAP()
...
void CItemSuit::UpdateItems ()
{
	...
	if ( GLUseFeatures::GetInstance().IsUsingDataIntegrate() )
	{
		...
		SetWin_Text(this, IDC_EDIT_VAR, CGameTextMan::GetInstance().GetCommentText("ITEMVAR", m_pDummyItem->sSuitOp.sVARIATE.emTYPE));
		SetWin_Text(this, IDC_EDIT_VOL, CGameTextMan::GetInstance().GetCommentText("ITEMVOL", m_pDummyItem->sSuitOp.sVOLUME.emTYPE));
	...
	else
	{
		...
		SetWin_Text(this, IDC_EDIT_VAR, COMMENT::ITEMVAR[m_pDummyItem->sSuitOp.sVARIATE.emTYPE].c_str() );
		SetWin_Text(this, IDC_EDIT_VOL, COMMENT::ITEMVOL[m_pDummyItem->sSuitOp.sVOLUME.emTYPE].c_str() );
	...
	SetWin_Num_float ( this, IDC_EDIT_VAR_VAL, m_pDummyItem->sSuitOp.sVARIATE.fVariate );
	SetWin_Num_float ( this, IDC_EDIT_VOL_VAL, m_pDummyItem->sSuitOp.sVOLUME.fVolume );
...
bool CItemSuit::InverseUpdateItems ()
{
	...
	m_pDummyItem->sSuitOp.sVARIATE.fVariate = GetWin_Num_float ( this, IDC_EDIT_VAR_VAL );
	m_pDummyItem->sSuitOp.sVOLUME.fVolume = GetWin_Num_float ( this, IDC_EDIT_VOL_VAL );
...
void CItemSuit::SelectVarType( int nEditID )
{
	CSelectDialog dlg;
	if ( GLUseFeatures::GetInstance().IsUsingDataIntegrate() )
	{
		std::string strITEMVAR[EMVAR_SIZE];
		CGameTextMan::GetInstance().LoadHelpCommentString("ITEMVAR", strITEMVAR, CGameTextMan::EM_COMMENT_TEXT, EMVAR_SIZE);
		dlg.SetStringList( strITEMVAR, EMVAR_SIZE );
		dlg.m_nSelect = m_pDummyItem->sSuitOp.sVARIATE.emTYPE;
		if ( dlg.DoModal() == IDOK )
		{
			m_pDummyItem->sSuitOp.sVARIATE.emTYPE = (EMITEM_VAR) dlg.m_dwData;
			SetWin_Text( this, nEditID, strITEMVAR[m_pDummyItem->sSuitOp.sVARIATE.emTYPE].c_str() );
		}
	}
	else
	{
		dlg.SetStringList( COMMENT::ITEMVAR, EMVAR_SIZE );	
		dlg.m_nSelect = m_pDummyItem->sSuitOp.sVARIATE.emTYPE;
		...
		{
			m_pDummyItem->sSuitOp.sVARIATE.emTYPE = (EMITEM_VAR) dlg.m_dwData;
			SetWin_Text( this, nEditID, COMMENT::ITEMVAR[m_pDummyItem->sSuitOp.sVARIATE.emTYPE].c_str() );
		}
	}
}

void CItemSuit::SelectVolType( int nEditID )
{
	CSelectDialog dlg;
	if ( GLUseFeatures::GetInstance().IsUsingDataIntegrate() )
	{
		std::string strITEMVAR[EMVAR_SIZE];
		CGameTextMan::GetInstance().LoadHelpCommentString("ITEMVOL", strITEMVAR, CGameTextMan::EM_COMMENT_TEXT, EMVAR_SIZE);
		dlg.SetStringList( strITEMVAR, EMVAR_SIZE );
		dlg.m_nSelect = m_pDummyItem->sSuitOp.sVOLUME.emTYPE;
		if ( dlg.DoModal() == IDOK )
		{
			m_pDummyItem->sSuitOp.sVOLUME.emTYPE = (EMITEM_VAR) dlg.m_dwData;
			SetWin_Text( this, nEditID, strITEMVAR[m_pDummyItem->sSuitOp.sVOLUME.emTYPE].c_str() );
		}
	}
	else
	{
		...
		{
			m_pDummyItem->sSuitOp.sVOLUME.emTYPE = (EMITEM_VAR) dlg.m_dwData;
			SetWin_Text( this, nEditID, COMMENT::ITEMVOL[m_pDummyItem->sSuitOp.sVOLUME.emTYPE].c_str() );
		}
	}
}
void CItemSuit::OnBnClickedButtonVar()
{
	SelectVarType ( IDC_EDIT_VAR );
}

void CItemSuit::OnBnClickedButtonVol()
{
	SelectVolType ( IDC_EDIT_VOL );
}
