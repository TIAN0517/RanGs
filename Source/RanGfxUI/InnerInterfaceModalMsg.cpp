...
void CInnerInterface::ModalMsgProcess ( UIGUID nCallerID, DWORD dwMsg )
{
    ...
    */
	...
	case UI::MODAL_POINT_SHOP_BUY:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				int nMain = nOUTOFRANGE;
				int nSub = nOUTOFRANGE;
				m_pModalWindow->GetModalData ( &nMain, &nSub );

				pointshop::NET_PS_BUY_ITEM_CF MsgCF(SNATIVEID((WORD)nMain,(WORD)nSub));
	...
	case UI::MODAL_ITEMREBUILD_PREVQUESTION:
		ModalItemRebuildPrevQuestion(dwMsg);
		break;
	...
	case UI::MODAL_WEAR_CHANGE_PASSWORD :
		ModalInvenLockChangePassword ( dwMsg );
		break;
	case UI::MODAL_WEAR_LOCKUNLOCK_PASSWORD :
		ModalInvenLockUnlock ( dwMsg );
		break;
	case UI::MODAL_WEAR_RESET_PASSWORD :
		ModalInvenLockResetPassword ( dwMsg );
		break;

	case UI::MODAL_LOCKER_CHANGE_PASSWORD :
		ModalStorageLockChangePassword ( dwMsg );
		break;
	case UI::MODAL_LOCKER_LOCKUNLOCK_PASSWORD :
		ModalStorageLockUnlock ( dwMsg );
		break;
	case UI::MODAL_LOCKER_RESET_PASSWORD :
		ModalStorageLockResetPassword ( dwMsg );
		break;
...
void CInnerInterface::ModalPartyTarget(DWORD dwMsg) 
{
	...
	{
		IPartyWindowRenewal* const pPartyWindow(GetPartyWindowRenewal());
		if ( pPartyWindow )
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
...
void CInnerInterface::ModalItemRebuildPrevQuestion(DWORD dwMsg)
{
	if( UIMSG_MODAL_OK & dwMsg )
	{
		m_pGaeaClient->GetCharacter()->ReqRebuild_SelectCache();
	}
}
...
void CInnerInterface::ModalItemRebuildOkQuestion(DWORD dwMsg)
{
    if( UIMSG_MODAL_OK & dwMsg )
    {
		m_pGaeaClient->GetCharacter()->ReqRebuildTry( GetItemRebuildWindowNPCID(), GetItemRebuildWindowNPCGlobID() );
    }
}
...
void CInnerInterface::ModalInvenLockChangePassword(DWORD dwMsg)
{
	GLCharacter* pChar = m_pGaeaClient->GetCharacter();
	if ( NULL == pChar )
		return;
	if ( UIMSG_MODAL_OK & dwMsg )
	{
		pChar->ReqCharacterLock ( EMCHARLOCK_INVENTORY_CHANGE_PASSWORD, GetModalWindow()->GetEditBoxMan()->GetEditBox( CModalWindow::LOGIN_EDIT_PWD )->GetEditString().GetString(), GetModalWindow()->GetEditBoxMan()->GetEditBox( CModalWindow::LOGIN_EDIT_2ND_PWD )->GetEditString().GetString() );	
	}
}

void CInnerInterface::ModalInvenLockResetPassword(DWORD dwMsg)
{
	GLCharacter* pChar = m_pGaeaClient->GetCharacter();
	if ( NULL == pChar )
		return;
	if ( UIMSG_MODAL_OK & dwMsg )
	{
		pChar->ReqCharacterLock ( EMCHARLOCK_INVENTORY_RESET_PASSWORD, GetModalWindow()->GetEditBoxMan()->GetEditBox( CModalWindow::LOGIN_EDIT_PWD )->GetEditString().GetString(), GetModalWindow()->GetEditBoxMan()->GetEditBox( CModalWindow::LOGIN_EDIT_2ND_PWD )->GetEditString().GetString() );
	}
}

void CInnerInterface::ModalInvenLockUnlock(DWORD dwMsg)
{
	GLCharacter* pChar = m_pGaeaClient->GetCharacter();
	if ( NULL == pChar )
		return;
	if ( UIMSG_MODAL_OK & dwMsg )
	{
		pChar->ReqCharacterLock ( EMCHARLOCK_INVENTORY_PASSWORD, GetModalWindow()->GetEditString().c_str(), "" );
	}
}

void CInnerInterface::ModalStorageLockChangePassword (DWORD dwMsg)
{
	GLCharacter* pChar = m_pGaeaClient->GetCharacter();
	if ( NULL == pChar )
		return;
	if ( UIMSG_MODAL_OK & dwMsg )
	{
		pChar->ReqCharacterLock ( EMCHARLOCK_LOCKER_CHANGE_PASSWORD, GetModalWindow()->GetEditBoxMan()->GetEditBox( CModalWindow::LOGIN_EDIT_PWD )->GetEditString().GetString(), GetModalWindow()->GetEditBoxMan()->GetEditBox( CModalWindow::LOGIN_EDIT_2ND_PWD )->GetEditString().GetString() );
	}
}

void CInnerInterface::ModalStorageLockUnlock (DWORD dwMsg)
{
	GLCharacter* pChar = m_pGaeaClient->GetCharacter();
	if ( NULL == pChar )
		return;
	if ( UIMSG_MODAL_OK & dwMsg )
	{
		pChar->ReqCharacterLock ( EMCHARLOCK_LOCKER_PASSWORD, GetModalWindow()->GetEditString().c_str(), "" );
	}
}

void CInnerInterface::ModalStorageLockResetPassword (DWORD dwMsg)
{
	GLCharacter* pChar = m_pGaeaClient->GetCharacter();
	if ( NULL == pChar )
		return;
	if ( UIMSG_MODAL_OK & dwMsg )
	{
		pChar->ReqCharacterLock ( EMCHARLOCK_LOCKER_RESET_PASSWORD, GetModalWindow()->GetEditBoxMan()->GetEditBox( CModalWindow::LOGIN_EDIT_PWD )->GetEditString().GetString(), GetModalWindow()->GetEditBoxMan()->GetEditBox( CModalWindow::LOGIN_EDIT_2ND_PWD )->GetEditString().GetString() );
	}
}
