...
void CModalWindow::DoModal(const std::string& strText, int nModalTitle, int nModalType, UIGUID CallerID, bool bShowPassKey, bool bTwian)
{
	...
	if( bShowPassKey )
	{
		...
		else
		{
			...
			else if ( nModalType != UI::EDITBOX_CHECK_CHAR_LOCK_PWD )	// 2차 비밀번호가 설정되어있을 시
			{
				m_pPassKeyboard->SetVisibleSingle( FALSE );
				m_pRandPassKeyboard->SetVisibleSingle( TRUE );
			}
...
void CModalWindow::ReArrangeControl ( int nModalType,  BOOL bUseLimitedInput )
{
	...
	case UI::EDITBOX_CHECK_CHAR_LOCK_PWD:
		{
			{
				const UIRECT& rc2BUTTONPOS = m_p2BUTTONLEFT->GetLocalPos ();
				D3DXVECTOR2 vDummy ( rc2BUTTONPOS.left, rc2BUTTONPOS.top );
			...
			else if( nModalType == UI::EDITBOX_CHECK_PWD ) // 2차비번 
			{
				...
				m_pEditBoxMan->GetEditBox( LOGIN_EDIT_PWD )->SetLimitInput ( 4 );
				m_pEditBoxMan->GetEditBox( LOGIN_EDIT_2ND_PWD )->SetLimitInput ( 4 );
				m_pEditBoxMan->GetEditBox( LOGIN_EDIT_PWD )->DoUSE_NUMBER ( true );
				m_pEditBoxMan->GetEditBox( LOGIN_EDIT_2ND_PWD )->DoUSE_NUMBER ( true );
				SetInputText ( ID2GAMEWORD( "PASS_SET_TEXT", 0 ) );
				SetReInputText ( ID2GAMEWORD( "PASS_SET_TEXT", 1 ) );
			}
			else if( nModalType == UI::EDITBOX_CHECK_CHAR_LOCK_PWD ) // 2차비번 
			{
				m_pEditBoxMan->SetVisibleSingle( TRUE );
				m_pEditBoxMan->GetEditBox( LOGIN_EDIT_PWD )->SetLimitInput ( 20 );
				m_pEditBoxMan->GetEditBox( LOGIN_EDIT_2ND_PWD )->SetLimitInput ( 20 );
				m_pEditBoxMan->GetEditBox( LOGIN_EDIT_PWD )->DoUSE_NUMBER ( false );
				m_pEditBoxMan->GetEditBox( LOGIN_EDIT_2ND_PWD )->DoUSE_NUMBER ( false );
			}
			...
			if( nModalType != UI::EDITBOX_CHECK_PWD && nModalType != UI::EDITBOX_CHECK_CHAR_LOCK_PWD )
			{
			...
			if( !m_pPassKeyboard->IsVisible() ) // KR/GS
			{
				...
				else if ( nModalType == UI::EDITBOX_CHECK_CHAR_LOCK_PWD )
				{
					m_pEditBoxMan->BeginEdit();
				}
				else
...
{ 
	return ( m_nModalType == UI::EDITBOX_CHECK_PWD || m_nModalType == UI::EDITBOX_CHECK_CHAR_LOCK_PWD ) ? TRUE : FALSE; 
}

void CModalWindow::SetInputText ( std::string strText )
{
	if ( m_pTextBoxPassSetInput ) m_pTextBoxPassSetInput->SetOneLineText( strText.c_str() );
}
void CModalWindow::SetReInputText ( std::string strText )
{
	if ( m_pTextBoxPassSetReInput ) m_pTextBoxPassSetReInput->SetOneLineText( strText.c_str() );
}
