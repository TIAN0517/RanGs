...
void CCTFResultRanking::Update()
{
	...
	case EMMAINSORTTYPE_CLUB: pvecResult = &pCTFClient->GetResultDataClub(); break;
	}
	...
	PVP::SCTFRESULTDATA& MyClubData = pCTFClient->GetMyResultDataClub();
	...
	case EMMAINSORTTYPE_CLUB:
		{
			if ( m_pGaeaClient->GetCharacter()->m_ClubDbNum != CLUB_NULL )
			{
				break;
			}

			nMyRank = pCTFClient->GetMyResultRankingClub() + 1;
		}
		break;
	}
	...
	if ( m_emSortType == EMMAINSORTTYPE_CLUB )	m_pMyRank->SetData( nMyRank, &MyClubData );
}
...
void CCTFResultRanking::Update_SortClub()
{
	m_nScrollPos = 0;
	m_emSortType = EMMAINSORTTYPE_CLUB;
	m_pScroll->GetThumbFrame ()->SetPercent( 0.0f );
...
void CCTFResultRanking::RANKSLOT::CreateSubControl ( CCTFResultRanking* pBase )
{
...

	{
		...
		pIMAGECLUB->CreateSub ( this, "CTF_RESULT_RANKSLOT_IMAGE_BASE_CLUB", UI_FLAG_DEFAULT );
...
void CCTFResultRanking::RANKSLOT::SetData( const UINT nRank, const PVP::SCTFRESULTDATA* pResultData )
{
	...
	if ( pResultData->IsNull() && bMYRANK )
	{
		...
		if ( pResultData->dwClubID != CLUB_NULL )
		{
			...
			ppBase->GetClassText()->SetOneLineText( "Club", NS_UITEXTCOLOR::LIGHTGREY );
			pIMAGECLUB->Refresh();
		}
		else
		{
			pIMAGECLUB->SetVisibleSingle( FALSE );
			ppBase->GetClassText()->SetOneLineText( ID2GAMEWORD("CTF_RESULT_TEXT_BASE", 2), NS_UITEXTCOLOR::LIGHTGREY );
		}
		return;
	}
	...
	if ( pResultData->dwClubID == CLUB_NULL ) 
	{
		...
		pTEXTCONTRIBUTION->SetVisibleSingle( TRUE );
		pIMAGECLUB->SetVisibleSingle( FALSE );
		ppBase->GetClassText()->SetOneLineText( ID2GAMEWORD("CTF_RESULT_TEXT_BASE", 2), NS_UITEXTCOLOR::LIGHTGREY );
	}
	else
	{
		pTEXTCONTRIBUTION->SetVisibleSingle( FALSE );
...
void CCTFResultRanking::RANKSLOT::SetImagePosition( CUIControl* pImage, const UIRECT& rcBase, const UIRECT& rcParent )
{
	GASSERT(pImage);
