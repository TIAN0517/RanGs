...
void CDialogueWindow::LoadBasicTalk ( const DWORD dwIndex )
{
	if ( !m_spDialogueCase )
        return;
	...
	if ( GLUseFeatures::GetInstance().IsUsingDataIntegrate() )
	{
		...
		str = CGameTextMan::GetInstance().GetText(strTalkKey.GetString(), 0, CGameTextMan::EM_NPCTALK_TEXT).GetString();
	}
	else	str = m_spDialogueCase->GetBasicTalk ();
...
void CDialogueWindow::LoadButtonTalk ( bool bRandom, const DWORD dwIndex )
{
	...
	if (spNpcTalkControl)
	{
		...
		for ( ; pos != pos_end; ++pos )
		{
			...
			else	strAnswerTalk = spNpcTalk->GetTalk();			
...
void CDialogueWindow::LoadNode( DWORD dwNID, bool bRandom, DWORD dwRandomTime )
{
	...
	LoadButtonTalk ( bRandom, dwIndex );
}
