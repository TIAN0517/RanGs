...
CPointShopWindow::CPointShopWindow( GLGaeaClient* pGaeaClient, CInnerInterface* pInterface, EngineDeviceMan* pEngineDevice )
	...
	, m_pBattlePointText(NULL)
	, m_pCurBattlePointText(NULL)
...
void CPointShopWindow::CreateSubControl()
{
...

    {
		...
		pTextBox = new CBasicTextBox(m_pEngineDevice);
		pTextBox->CreateSub ( this, "POINT_SHOP_BATTLEPOINT_TEXT", UI_FLAG_DEFAULT );
		pTextBox->SetFont( pFont9Shadow );
		pTextBox->SetTextAlign( TEXT_ALIGN_RIGHT | TEXT_ALIGN_CENTER_Y );
		RegisterControl( pTextBox );
		m_pBattlePointText = pTextBox;
		m_pBattlePointText->ClearText();
		m_pBattlePointText->SetText( ID2GAMEWORD("POINT_SHOP_BATTLEPOINT_TEXT"), NS_UITEXTCOLOR::WHITE );

		pTextBox = new CBasicTextBox(m_pEngineDevice);
		pTextBox->CreateSub ( this, "POINT_SHOP_CUR_BATTLEPOINT_TEXT", UI_FLAG_DEFAULT );
		pTextBox->SetFont( pFont9Shadow );
		pTextBox->SetTextAlign( TEXT_ALIGN_LEFT | TEXT_ALIGN_CENTER_Y );
		RegisterControl( pTextBox );
		m_pCurBattlePointText = pTextBox;
    }
        ...
        if ( CHECK_MOUSE_IN ( dwMsg ) )
        {
			...
			else if ( CHECK_MOUSEIN_RBUPLIKE( dwMsg ) )
			{
				AddCartList( m_pItemList[ nIndex ]->GetItemID(),
					m_pItemList[ nIndex ]->GetItemPrice(), m_pItemList[ nIndex ]->GetItemCurrency() );
...
void CPointShopWindow::ReLoadSellItemNodes()
{
    ...
    for ( int i = 0; i < NUM_POINT_SHOP_ITEM_LIST_ONE_PAGE; i++ )
    {
        ...
        m_pItemList[ i ]->SetItem( SellItem.ItemId(), SellItem.m_ItemName, SellItem.Price(), SellItem.Currency() );
...
void CPointShopWindow::SetPointText( __int64 nPoint )
{
    ...
    m_pCurPointText->SetText( strPoint.c_str(), ITEM_INFOR_COLOR::dwRandomAbility );
}

void CPointShopWindow::SetBattlePointText( __int64 nPoint )
{
    m_pCurBattlePointText->ClearText();

    std::string strPoint = NS_UITEXTCONTROL::MAKE_MONEY_FORMAT( nPoint, 3, "," );
    m_pCurBattlePointText->SetText( strPoint.c_str(), NS_UITEXTCOLOR::ORANGE );
}
...
void CPointShopWindow::AddCartList( SNATIVEID ItemID, __int64 Price, WORD wCurrency )
{
    ...
    pointshop::SellItem SelectItem( ItemID, Price, wCurrency );
        ...
        if ( sWishListId != NATIVEID_NULL().dwID )
        {
            ...
            for ( int j = 0; j < nSellItemListSize; j++ )
            {
                ...
                if ( SellItem.ItemId().dwID == sWishListId )
                {
                    ...
                    pointshop::SellItem SelectItem( SellItem.ItemId(), SellItem.Price(), SellItem.Currency() );
...
void MyPointShopWindow::SetBattlePointText( __int64 nPoint )
{
	CPointShopWindow::SetBattlePointText( nPoint );
}
