...
void CPointShopNode::SetItem( SNATIVEID _sID, std::string _strName, __int64 _Price, WORD _Currency )
{
    m_sID = _sID;
    m_Name = _strName;
    m_Price = _Price;
	...
	std::string strPrice;
	if ( m_Currency == pointshop::POINT_SHOP_PREMIUM_POINT )
	strPrice = sc::string::format( ID2GAMEINTEXT("POINT_SHOP_PRICE"), NS_UITEXTCONTROL::MAKE_MONEY_FORMAT( _Price, 3, "," ) );
	else
	strPrice = sc::string::format( ID2GAMEINTEXT("BATTLEPOINT_SHOP_PRICE"), NS_UITEXTCONTROL::MAKE_MONEY_FORMAT( _Price, 3, "," ) );
