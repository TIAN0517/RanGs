...
void CForceRebirth::Update( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
    ...
    strText = sc::string::format( ID2GAMEINTEXT("NOTIFY_FORCE_REBIRTH"), (int)(m_fTime + 1.0f));
    ...
    if ( m_fTime <= 0.0f )
    {
        if ( m_pGaeaClient->GetCharacter()->IsDie() )
        {
			...
			if ( !m_pGaeaClient->GetCaptureTheField()->IsPlayer() || !m_pGaeaClient->GetCaptureTheField()->IsCTFMap( m_pGaeaClient->GetLandManClient()->GetBaseMapID()) ) SetForceRebirth ( 0.0f );
