...
CCharacterWindow::CCharacterWindow(GLGaeaClient* pGaeaClient, CInnerInterface* pInterface, EngineDeviceMan* pEngineDevice)
	...
	, m_pPKScoreTextStatic(NULL)
	, m_pPKDeathTextStatic(NULL)
	, m_pPKScoreText(NULL)
	, m_pPKDeathText(NULL)
    , m_nSelect(0)
{
	for ( int i = 0; i < PKCOMBO_SIZE; i++ ) m_pPKStreakTextStatic[i] = NULL;
	for ( int i = 0; i < PKCOMBO_SIZE; i++ ) m_pPKStreakText[i] = NULL;
}
...
void CCharacterWindow::CreateSubControl ()
{
	...
	{	//	Status
		...
		m_pPKScoreTextStatic = CreateStaticControl ( "CHARACTER_PK_SCORE_STATIC_TEXT", pFont, nAlignLeft );
		m_pPKScoreTextStatic->AddText ( ID2GAMEWORD("CHARACTER_STATUS_STATIC", 57), dwAddAbility );
		m_pPKScoreText = CreateStaticControl ( "CHARACTER_PK_SCORE_TEXT", pFont, nAlignLeft );

		m_pPKDeathTextStatic = CreateStaticControl ( "CHARACTER_PK_DEATH_STATIC_TEXT", pFont, nAlignLeft );
		m_pPKDeathTextStatic->AddText ( ID2GAMEWORD("CHARACTER_STATUS_STATIC", 58), NS_UITEXTCOLOR::RED );
		m_pPKDeathText = CreateStaticControl ( "CHARACTER_PK_DEATH_TEXT", pFont, nAlignLeft );

		CString strStreakS[PKCOMBO_SIZE] =
		{
			"CHARACTER_PK_STREAK_1_STATIC_TEXT",
			"CHARACTER_PK_STREAK_2_STATIC_TEXT",
			"CHARACTER_PK_STREAK_3_STATIC_TEXT",
			"CHARACTER_PK_STREAK_4_STATIC_TEXT",
			"CHARACTER_PK_STREAK_5_STATIC_TEXT",
			"CHARACTER_PK_STREAK_6_STATIC_TEXT",
			"CHARACTER_PK_STREAK_7_STATIC_TEXT",
			"CHARACTER_PK_STREAK_8_STATIC_TEXT",
		};

		CString strStreak[PKCOMBO_SIZE] =
		{
			"CHARACTER_PK_STREAK_1_TEXT",
			"CHARACTER_PK_STREAK_2_TEXT",
			"CHARACTER_PK_STREAK_3_TEXT",
			"CHARACTER_PK_STREAK_4_TEXT",
			"CHARACTER_PK_STREAK_5_TEXT",
			"CHARACTER_PK_STREAK_6_TEXT",
			"CHARACTER_PK_STREAK_7_TEXT",
			"CHARACTER_PK_STREAK_8_TEXT",
		};

		for ( int i = 0; i < PKCOMBO_SIZE; i++ )
		{
			m_pPKStreakTextStatic[i] = CreateStaticControl ( strStreakS[i].GetString(), pFont, nAlignLeft );
			m_pPKStreakTextStatic[i]->AddText ( ID2GAMEWORD("CHARACTER_STATUS_STATIC", 59 + i), dwSilver );
			m_pPKStreakText[i] = CreateStaticControl ( strStreak[i].GetString(), pFont8Shadow, nAlignLeft );
		}
