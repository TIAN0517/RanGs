...
void CCharacterWindow::SetPKScore ( const GLCHARLOGIC& sCharData )
{
	if ( m_pPKScoreText )
	{
		CString strCombine;
		strCombine.Format ( "%d", sCharData.m_dwPKScore );
		m_pPKScoreText->SetOneLineText ( strCombine, NS_UITEXTCOLOR::WHITE );
	}
	...
	if ( m_pPKDeathText )
	{
		...
		m_pPKDeathText->SetOneLineText ( strCombine, NS_UITEXTCOLOR::WHITE );
	}
}

void CCharacterWindow::SetPKStreak ( const GLCHARLOGIC& sCharData )
{
	for ( int i = 0; i < PKCOMBO_SIZE; i++ )
	{
		if ( m_pPKStreakText[i] )
		{
			CString strCombine;
			strCombine.Format ( "%d", sCharData.m_PKStreakData.nPKSTREAK[i] );
			m_pPKStreakText[i]->SetOneLineText ( strCombine, NS_UITEXTCOLOR::GOLD );
		}
	}
}
