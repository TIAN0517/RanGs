...
void MarketRPPage::CreateSubControl()
{
...

    {
		...
		m_pCheckBoxPoint_Bet = new CCheckBox( m_pEngineDevice ); 
		m_pCheckBoxPoint_Bet->CreateSub( this, "MARKET_RP_PAGE_R_BG2_CHECKBOX3", UI_FLAG_DEFAULT , ID_CHECKBOX_RANPOINT_BET );
		m_pCheckBoxPoint_Bet->CreateSubControl( "ANOTHER_CHARACTER_ALLWEAR_CHECK", "ANOTHER_CHARACTER_ALLWEAR_UNCHECK" );
		m_pCheckBoxPoint_Bet->SetCheck( FALSE );
		RegisterControl( m_pCheckBoxPoint_Bet );

		m_pCheckBoxMoney_Bet = new CCheckBox( m_pEngineDevice ); 
		m_pCheckBoxMoney_Bet->CreateSub( this, "MARKET_RP_PAGE_R_BG2_CHECKBOX4", UI_FLAG_DEFAULT , ID_CHECKBOX_RANMONEY_BET );
		m_pCheckBoxMoney_Bet->CreateSubControl( "ANOTHER_CHARACTER_ALLWEAR_CHECK", "ANOTHER_CHARACTER_ALLWEAR_UNCHECK" );
		m_pCheckBoxMoney_Bet->SetCheck( FALSE );
		RegisterControl( m_pCheckBoxMoney_Bet );
...
void MarketRPPage::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
    ...
    case ID_CHECKBOX_RANPOINT:
        if( bMouseIn && CHECK_LB_DOWN_LIKE( dwMsg ) )
        {
			...
			m_pCheckBoxMoney_Bet->SetCheckOff();
			m_pCheckBoxPoint_Bet->SetCheckOff();
	...
	case ID_CHECKBOX_RANPOINT_BET:
		if( bMouseIn && CHECK_LB_DOWN_LIKE( dwMsg ) )
		{
			...
			m_pCheckBoxMoney->SetCheckOff();
			m_pCheckBoxMoney_Bet->SetCheckOff();
			m_pCheckBoxPoint_Bet->SetCheckOn();
		}
		break;
	case ID_CHECKBOX_RANMONEY_BET:
		if( bMouseIn && CHECK_LB_DOWN_LIKE( dwMsg ) )
		{
			if( m_pPeriodComboBox->IsVisibleComboxRoolOver() || m_pPeriodGameMoney->IsVisibleComboxRoolOver() )
				return;
			...
			m_pCheckBoxMoney->SetCheckOff();
			m_pCheckBoxPoint_Bet->SetCheckOff();
			m_pCheckBoxMoney_Bet->SetCheckOn();
		}
		break;
        ...
        if( bMouseIn )
        {
            if( bMouseLBUP )
            {
				...
				else if ( m_sInvenItem.sItemCustom.IsExchange() == false )
                {
					...
					m_pCheckBoxMoney_Bet->SetVisibleSingle(FALSE);
...
void MarketRPPage::Reset()
{
	...
	if( private_market::GLPrivateMarketDefine::Instance()->GoldTrade() ) 
	{
		...
		m_pCheckBoxMoney_Bet->SetVisibleSingle(TRUE);
	}
	else
	{
	...
	m_pCheckBoxPoint_Bet->SetVisibleSingle(TRUE);
...
void MarketRPPage::Regist()
{
	...
	else if (m_pCheckBoxPoint_Bet->IsChecked())
		sellType = private_market::eSellType_ConsignmentSale_RanPointBet;
	else if (m_pCheckBoxMoney_Bet->IsChecked())
		sellType = private_market::eSellType_ConsignmentSale_GoldBet;
...
bool MarketRPPage::RegistCheck()
{
    ...
    if( ( m_pCheckBoxPoint->IsChecked() || m_pCheckBoxPoint_Bet->IsChecked() ) && llPrice < wMinPrice )
    {
