...
CCTFBattle::CCTFBattle(GLGaeaClient* pGaeaClient, CInnerInterface* pInterface, EngineDeviceMan* pEngineDevice)
...
CCTFBattle::~CCTFBattle ()
{
}

void CCTFBattle::CreateSubControl ()
{
	//! 라인박스
	{
...
void CCTFBattle::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	CUIGroup::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );
	Update();
}

void CCTFBattle::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	switch( ControlID )
	{
...
_STATUS_END:
	...
	//버튼
	{
		...
		if ( sState.IsJOINDENIED(pchar) )
		{
			...
			m_fnCallbackButton = boost::bind( &CCTFBattle::ReqJoin, this );
			goto _BUTTON_END;
		}
		...
		if ( pCTFClient->GetProgressCTF() == m_emCTFType )
		{
			...
			m_fnCallbackButton = boost::bind( &CCTFBattle::ReqExit, this );
		}
		else
		{
			if ( sState.bWAITJOIN )
			{
				...
				m_fnCallbackButton = boost::bind( &CCTFBattle::ReqCancel, this );
			}
			else
			{
...
void CCTFBattle::EnableJoinButton( const std::string& strLabel, const bool bEnable )
{
	if ( bEnable )
	{
...
CBasicTextBox* CCTFBattle::CreateStaticControl(const char* szControlKeyword, CD3DFontPar* pFont, int nAlign )
{
