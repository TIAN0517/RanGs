...
int	CTopUpPage::nLIMIT_CODE = 20;
int	CTopUpPage::nLIMIT_PIN = 20;
int	CTopUpPage::nLIMIT_CP = 7;

CTopUpPage::CTopUpPage (GLGaeaClient* pGaeaClient, COuterInterface* pInterface, EngineDeviceMan* pEngineDevice)
	...
	case TOPUP_OK:
		{
			if ( CHECK_KEYFOCUSED ( dwMsg ) || CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{
				...
				{
					 m_pInterface->DoModal ( ID2GAMEEXTEXT ( "TOPUPSTAGE_4" ) );
					return ;
				}
				
				if( !strRP.GetLength () )
				{
					 m_pInterface->DoModal ( ID2GAMEEXTEXT ( "TOPUPSTAGE_6" ) );
					return ;
				}
				
				if ( !CheckString ( strCD ) )
				{
					 m_pInterface->DoModal ( ID2GAMEEXTEXT ( "TOPUP_PAGE_CD_ERROR" ) );
					return ;
				}

				if ( !CheckString ( strPN ) )
				{
					 m_pInterface->DoModal ( ID2GAMEEXTEXT ( "TOPUP_PAGE_PN_ERROR" ) );
					return ;
				}

				if ( !CheckString ( strRP ) )
				{
					 m_pInterface->DoModal ( ID2GAMEEXTEXT ( "TOPUP_PAGE_CP_ERROR" ) );
					return ;
				}
				CNetClient* pNetClient = m_pGaeaClient->GetGlobalStage()->GetNetClient ();
				...
				if ( strCheck == strRP )
				{
					 pNetClient->SndTopUp( strCD, strPN, strRP );									  m_pInterface->DoModal ( ID2GAMEEXTEXT ("TOPUPSTAGE_7") );
				}
				else
				{
					 m_pInterface->DoModal ( ID2GAMEEXTEXT ("TOPUP_PAGE_CP_ERROR"), UI::MODAL_INFOMATION, UI::OK, OUTER_MODAL_TOPUPTOLOGIN );
				}
			}
		}
		break;
	...
	case TOPUP_CANCEL:
		{
			if ( CHECK_KEYFOCUSED ( dwMsg ) || CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{
				...
				m_pInterface->UiHideGroup ( TOPUP_PAGE, true );
			}
		}
		break;
	};
