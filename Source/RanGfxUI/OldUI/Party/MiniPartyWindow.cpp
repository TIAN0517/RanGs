...
void CMiniPartyWindow::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	//	파티 리스트 갱신
	//	
	GLPartyClient* const pMyParty = m_pGaeaClient->GetPartyManager()->GetParty( (DWORD)MASTER_PARTY );
	if ( !pMyParty )
		return;

	if ( pMyParty->GetNMember() == 0 )
		return;
	
	GLPartySlotClient* const pSelf = pMyParty->GetMember((DWORD)MASTER_PARTY);
	//	NOTE
	//		만약, 자신을 찾지 못하면
