...
const bool CItemImage::SetItem( SNATIVEID sNativeID )
{
	...
	EMCHARINDEX emClass = m_pGaeaClient->GetCharacter()->GETCHARINDEX();
	std::string strIconFile = pItemData->GetSubInventoryFile( emClass );
	if ( strIconFile.empty() )
		strIconFile = pItemData->GetInventoryFile();

    return SetItem( pItemData->sBasicOp.sICONID, strIconFile.c_str(), pItemData->sBasicOp.sNativeID );
}
