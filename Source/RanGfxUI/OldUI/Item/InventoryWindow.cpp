...
void CInventoryWindow::CreateSubControl()
{
	...
	m_pPage = new CRnPageInven( m_pGaeaClient, m_pInterface, m_pEngineDevice );
	m_pPage->CreateSub( this, "INVENTORY_PAGE", UI_FLAG_YSIZE, INVENTORY_PAGE );
	m_pPage->CreateSubControl();
	RegisterControl( m_pPage );
	...
	m_pLockButton = CreateFlipButton( "WEAR_LOCK_BUTTON", "WEAR_LOCK_BUTTON_F", ITEM_LOCK, CBasicButton::RADIO_FLIP ); //add lock
...
void CInventoryWindow::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	...
	case ITEM_LOCK:
		{
			if ( m_bLockInven != m_pGaeaClient->GetCharacter()->m_bInventoryLock )
			{
				m_bLockInven = m_pGaeaClient->GetCharacter()->m_bInventoryLock;
				m_pLockButton->SetFlip( m_bLockInven );
			}
			...
			if ( CHECK_MOUSE_IN( dwMsg ) )
			{
				...
				if ( m_pGaeaClient->GetCharacter()->m_bInventoryLock )
				{
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_1",0 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_1",1 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_1",2 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_1",3 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_1",4 ), NS_UITEXTCOLOR::WHITE );
				}
				else
				{
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_0",0 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_0",1 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_0",2 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_0",3 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_0",4 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_0",5 ), NS_UITEXTCOLOR::WHITE );
				}
				//m_pInterface->SHOW_COMMON_LINEINFO( strCombine.GetString() , m_pGaeaClient->GetCharacter()->m_bInventoryLock ? NS_UITEXTCOLOR::YELLOW : NS_UITEXTCOLOR::WHITE );

				m_pInterface->END_COMMON_LINEINFO_MULTI();
				// change password
				if ( (dwMsg & UIMSG_LB_UP) )
				{
					m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_03-1",0 ) );
					m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_03-1",1 ) );
					m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_03-1",2 ) );

					m_pInterface->BEGIN_COMMON_LINEINFO_MULTI( TEXT_ALIGN_LEFT );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_03-1",0 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_03-1",1 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_03-1",2 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->END_COMMON_LINEINFO_MULTI();

					m_pInterface->GetModalWindow()->SetInputText( ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",2) );
					m_pInterface->GetModalWindow()->SetReInputText( ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",4) );

					m_pInterface->DoModal ( ID2GAMEINTEXT ("WEAR_LOCK_DESC_03"), UI::MODAL_INPUT, UI::EDITBOX_CHECK_CHAR_LOCK_PWD, UI::MODAL_WEAR_CHANGE_PASSWORD, TRUE );
					return;
				}
				...
				if ( (dwMsg & UIMSG_RB_UP) )
				{
					if ( m_pGaeaClient->GetCharacter()->m_bInventoryLock ) 
					{
						m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_04-2",0 ) );
						m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_04-2",1 ) );
						m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_04-2",2 ) );
						strCombine = ID2GAMEINTEXT ("WEAR_LOCK_DESC_04");

						m_pInterface->BEGIN_COMMON_LINEINFO_MULTI( TEXT_ALIGN_LEFT );
						m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_04-2",0 ), NS_UITEXTCOLOR::WHITE );
						m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_04-2",1 ), NS_UITEXTCOLOR::WHITE );
						m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_04-2",2 ), NS_UITEXTCOLOR::WHITE );
						m_pInterface->END_COMMON_LINEINFO_MULTI();
					}
					else
					{
						m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_04-3",0 ) );
						m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_04-3",1 ) );
						m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_04-3",2 ) );
						strCombine = ID2GAMEINTEXT ("WEAR_LOCK_DESC_04-1");

						m_pInterface->BEGIN_COMMON_LINEINFO_MULTI( TEXT_ALIGN_LEFT );
						m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_04-3",0 ), NS_UITEXTCOLOR::WHITE );
						m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_04-3",1 ), NS_UITEXTCOLOR::WHITE );
						m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_04-3",2 ), NS_UITEXTCOLOR::WHITE );
						m_pInterface->END_COMMON_LINEINFO_MULTI();
					}
					m_pInterface->DoModal ( strCombine.GetString() , UI::MODAL_INPUT, UI::EDITBOX, UI::MODAL_WEAR_LOCKUNLOCK_PASSWORD, TRUE );
					return;
				}

				// reset
				if ( (dwMsg & UIMSG_MB_UP) )
				{
					m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_05-1",0 ) );
					m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_05-1",1 ) );
					m_pGaeaClient->PrintConsoleText( ID2GAMEWORD( "WEAR_LOCK_DESC_05-1",2 ) );

					m_pInterface->BEGIN_COMMON_LINEINFO_MULTI( TEXT_ALIGN_LEFT );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_05-1",0 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_05-1",1 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->ADD_COMMON_LINEINFO_MULTI( ID2GAMEWORD( "WEAR_LOCK_DESC_05-1",2 ), NS_UITEXTCOLOR::WHITE );
					m_pInterface->END_COMMON_LINEINFO_MULTI();
					m_pInterface->DoModal ( ID2GAMEINTEXT ("WEAR_LOCK_DESC_05"), UI::MODAL_INPUT, UI::EDITBOX_CHECK_CHAR_LOCK_PWD, UI::MODAL_WEAR_RESET_PASSWORD, TRUE );

					m_pInterface->GetModalWindow()->SetInputText( ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",0) );
					m_pInterface->GetModalWindow()->SetReInputText( ID2GAMEWORD("WEAR_LOCK_PASSWORD_TEXT",1) );
					return;
				}
			}
		}
...
void CInventoryWindow::GetSplitPos( WORD* pwPosX, WORD* pwPosY )
{
	...
	m_pPage->GetConsumeSplitInvenPos( *pwPosX, *pwPosY );
}
