	...
	case OUTER_MODAL_CHANGEPIN:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( UiIsVisibleGroup ( CHANGEPIN_PAGE ) )			UiHideGroup ( CHANGEPIN_PAGE );
				UiHideGroup ( SELECT_CHARACTER_PAGE, true );
				...
				ToSelectServerPage ( CHANGEPIN_PAGE );
			}
		}break;
	...
	case OUTER_MODAL_CHANGEPASS:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( UiIsVisibleGroup ( CHANGEPASS_PAGE ) )			UiHideGroup ( CHANGEPASS_PAGE );
				UiHideGroup ( SELECT_CHARACTER_PAGE, true );
				CNetClient* pNetClient = m_pGaeaClient->GetGlobalStage()->GetNetClient();
				if( pNetClient->IsOnline() == true )
				{
				...
				ToSelectServerPage ( CHANGEPASS_PAGE );
			}
		}break;
	...
	case OUTER_MODAL_CHANGEEMAIL:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( UiIsVisibleGroup ( CHANGEEMAIL_PAGE ) )			UiHideGroup ( CHANGEEMAIL_PAGE );
				UiHideGroup ( SELECT_CHARACTER_PAGE, true );
				CNetClient* pNetClient = m_pGaeaClient->GetGlobalStage()->GetNetClient();
				if( pNetClient->IsOnline() == true )
				{
				...
				ToSelectServerPage ( CHANGEEMAIL_PAGE );
			}
		}break;
	case OUTER_MODAL_RESETPIN:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( UiIsVisibleGroup ( RESETPIN_PAGE ) )			UiHideGroup ( RESETPIN_PAGE );
				UiHideGroup ( SELECT_CHARACTER_PAGE, true );
				CNetClient* pNetClient = m_pGaeaClient->GetGlobalStage()->GetNetClient();
				if( pNetClient->IsOnline() == true )
				{
				...
				ToSelectServerPage ( RESETPIN_PAGE );
	...
	case OUTER_MODAL_RESETPASS:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( UiIsVisibleGroup ( RESETPASS_PAGE ) )			UiHideGroup ( RESETPASS_PAGE );
				UiHideGroup ( SELECT_CHARACTER_PAGE, true );
				ToRPSPage2 ( RESETPASS_PAGE );
	...
	case OUTER_MODAL_RESETPASS2:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( UiIsVisibleGroup ( RESETPASS_PAGE2 ) )			UiHideGroup ( RESETPASS_PAGE2 );
				UiHideGroup ( SELECT_CHARACTER_PAGE, true );
				CNetClient* pNetClient = m_pGaeaClient->GetGlobalStage()->GetNetClient();
				if( pNetClient->IsOnline() == true )
				{
				...
				ToSelectServerPage ( RESETPASS_PAGE2 );
			}
		}break;
	...
	case OUTER_MODAL_TOPUP:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( UiIsVisibleGroup ( TOPUP_PAGE ) )			UiHideGroup ( TOPUP_PAGE );
				UiHideGroup ( SELECT_CHARACTER_PAGE, true );
				CNetClient* pNetClient = m_pGaeaClient->GetGlobalStage()->GetNetClient();
				if( pNetClient->IsOnline() == true )
				{
	...
	case OUTER_MODAL_GAMETIMECVT:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( UiIsVisibleGroup ( GAMETIMECVT_PAGE ) )			UiHideGroup ( GAMETIMECVT_PAGE );
				UiHideGroup ( SELECT_CHARACTER_PAGE, true );
				CNetClient* pNetClient = m_pGaeaClient->GetGlobalStage()->GetNetClient();
				if( pNetClient->IsOnline() == true )
				{
				...
				ToSelectServerPage ( GAMETIMECVT_PAGE );
			}
		}break;
	...
	case OUTER_MODAL_PREMIUMPOINTCVT:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( UiIsVisibleGroup ( PREMIUMPOINTCVT_PAGE ) )			UiHideGroup ( PREMIUMPOINTCVT_PAGE );
				CNetClient* pNetClient = m_pGaeaClient->GetGlobalStage()->GetNetClient();
				if( pNetClient->IsOnline() == true )
				{
					pNetClient->CloseConnect();
				...
				ToSelectServerPage ( PREMIUMPOINTCVT_PAGE );
			}
		}break;
		//userpanel end
