...
int	CGMOdbcManager::SaveCharacterSlots ( LPVOID _pbuffer )
{
	...
	for ( int i=0; i<EMCHAR_SLOT_DATA_SIZE; ++i )
	{
		...
		{
			se::ByteStream ByteStream;
			if ( sSlotData.GETEXPSKILLS_BYBUF( ByteStream ) )
			{
				LPBYTE pBuffer = NULL;
				DWORD dwSize;
				ByteStream.GetBuffer( pBuffer, dwSize);
				int nResult = WriteImageForSkillSlot("ChaSlotInfo.ChaSkills", dwChaDBNum, (char *) pBuffer, dwSize, i );
		...
		{
			se::ByteStream ByteStream;
			if ( sSlotData.GETSKILL_QUICKSLOT( ByteStream ) )
			{
				LPBYTE pBuffer = NULL;
				DWORD dwSize;
				ByteStream.GetBuffer( pBuffer, dwSize);
				int nResult = WriteImageForSkillSlot("ChaSlotInfo.ChaSkillSlot", dwChaDBNum, (char *) pBuffer, dwSize, i );
...
int	CGMOdbcManager::WriteImageForSkillSlot(const char* objName, int nChaNum, char* pData, int nSize, int nSlotNum )
{
	SQLRETURN	sReturn; // SQL return value (short)	
	SQLHSTMT	hStmt; // Handle of statement (void*)
	// SQLCHAR		pBinary[1024];	// Declare a binary buffer to retrieve 1024 bytes of data at a time.
	...
	std::strstream strTemp;
	strTemp << "UPDATE ChaSlotsInfo SET " << objName << "=? WHERE (ChaNum=" <<  nChaNum << ")";
	strTemp << '\0';
