...
void CPropertyChaSkill::GetData(SCHARDATA2* pChar)
{
	for ( int i = 0; i < m_vSkill.size(); ++i )
	{
		SCHARSKILL CharSkill = m_vSkill[i];
		
        PGLSKILL pSkillData = GLSkillMan::GetInstance().GetData ( CharSkill.sNativeID.wMainID, CharSkill.sNativeID.wSubID );
		if ( pSkillData )
		{
			pChar->m_ExpSkills[CharSkill.sNativeID.dwID] = CharSkill;
		}
	}
}