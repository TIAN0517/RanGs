...
void CPropertyChaItemMix::EnableProperties( HTREEITEM hItem )
{
	...
	// 다중 선택시;
	if( m_treeSelect.GetCount()/*.GetSelectedCount()*/ > 1 )
	{
		// 선택한 아이템의 타입;
		...
		HTREEITEM hTempItem = m_treeSelect.GetRootItem()/*.GetFirstSelectedItem()*/;
		while( hTempItem )
		{
			...
			hTempItem = m_treeSelect.GetNextItem/*.GetNextSelectedItem*/( hTempItem,TVGN_NEXT  );
		}
