...
enum EMCHARLOCK
{
	EMCHARLOCK_INVENTORY_PASSWORD = 0,
	EMCHARLOCK_INVENTORY_CHANGE_PASSWORD = 1,
	EMCHARLOCK_INVENTORY_RESET_PASSWORD = 2,

	EMCHARLOCK_LOCKER_PASSWORD = 3,
	EMCHARLOCK_LOCKER_CHANGE_PASSWORD = 4,
	EMCHARLOCK_LOCKER_RESET_PASSWORD = 5,
};

enum EMCHARLOCK_FB
{
	EMCHARLOCK_FB_INVENTORY_PASSWORD_OK = 0,
	EMCHARLOCK_FB_INVENTORY_CHANGE_PASSWORD_OK = 1,
	EMCHARLOCK_FB_INVENTORY_RESET_PASSWORD_OK = 2,

	EMCHARLOCK_FB_INVENTORY_PASSWORD_FAIL = 3,
	EMCHARLOCK_FB_INVENTORY_CHANGE_PASSWORD_FAIL = 4,
	EMCHARLOCK_FB_INVENTORY_RESET_PASSWORD_FAIL = 5,

	EMCHARLOCK_FB_LOCKER_PASSWORD_OK = 6,
	EMCHARLOCK_FB_LOCKER_CHANGE_PASSWORD_OK = 7,
	EMCHARLOCK_FB_LOCKER_RESET_PASSWORD_OK = 8,

	EMCHARLOCK_FB_LOCKER_PASSWORD_FAIL = 9,
	EMCHARLOCK_FB_LOCKER_CHANGE_PASSWORD_FAIL = 10,
	EMCHARLOCK_FB_LOCKER_RESET_PASSWORD_FAIL = 11,
};

enum EMCHARLOCK_REASON
{
	EMCHARLOCK_REASON_LENGTH = 0,
	EMCHARLOCK_REASON_SYMBOL = 1,
	EMCHARLOCK_REASON_ALREADY= 2,
	EMCHARLOCK_REASON_COMMON = 3,
};

enum EMREQ_RANKING_FROMDB_FB
{
	EMREQ_RANKING_FROMDB_FB_END		       = 0,	//! 마지막아이템
	EMREQ_RANKING_FROMDB_FB_OK		       = 1,	//! 성공
	EMREQ_RANKING_FROMDB_FB_RESET		   = 2, //! 마지막아이템, 최초 정보 UI 열지 않음
};
enum
{
	HOW_MANY_CHSTATISTICS_PER_ONE_MSG	= 10		//64 bytes each
};
...
namespace GLMSG
{
	...
	struct SNETPC_UPDATE_PK_SCORE : public NET_MSG_CHARACTER
	{
		DWORD				dwVal;
		char				szName[CHAR_SZNAME];
	...
	struct SNETPC_UPDATE_PK_DEATH : public NET_MSG_CHARACTER
	{
		DWORD				dwVal;
		char				szName[CHAR_SZNAME];
...
*/
	...
	struct SNETPC_REQ_TUTORIAL_ACK : public NET_MSG_CHARACTER
	{
		SNETPC_REQ_TUTORIAL_ACK (void) 
		{
			dwSize = sizeof(*this);
			nType = NET_MSG_REQ_TUTORIAL_ACK;
			MIN_STATIC_ASSERT( sizeof( SNETPC_REQ_TUTORIAL_ACK ) <= NET_DATA_BUFSIZE );
		}
	};

	struct SNETPC_REQ_TUTORIAL_COMPLETE : public NET_MSG_CHARACTER
	{
		SNETPC_REQ_TUTORIAL_COMPLETE (void) 
		{
			dwSize = sizeof(*this);
			nType = NET_MSG_REQ_TUTORIAL_COMPLETE;
			MIN_STATIC_ASSERT( sizeof( SNETPC_REQ_TUTORIAL_COMPLETE ) <= NET_DATA_BUFSIZE );
		}
	};

	struct SNETPC_REQ_TUTORIAL_COMPLETE_MOVE : public NET_MSG_CHARACTER
	{
		MapID   sMapID;
		DWORD	dwGateID;
		SNETPC_REQ_TUTORIAL_COMPLETE_MOVE (void)
		:	dwGateID ( 0 )
		{
			dwSize = sizeof(*this);
			nType = NET_MSG_REQ_TUTORIAL_COMPLETE_MOVE;
			MIN_STATIC_ASSERT( sizeof( SNETPC_REQ_TUTORIAL_COMPLETE_MOVE ) <= NET_DATA_BUFSIZE );
		}
	};

	//add addstats cmd
	struct SNETPC_REQ_STATSUPCMD : public NET_MSG_CHARACTER
	{
		EMSTATS				emStats;
		DWORD				dwValue;

		SNETPC_REQ_STATSUPCMD (void) :
		emStats(EMPOW),
			dwValue(0)
		{
			dwSize = sizeof(*this);
			nType = NET_MSG_GCTRL_REQ_STATSUPCMD;
			MIN_STATIC_ASSERT( sizeof( SNETPC_REQ_STATSUPCMD ) <= NET_DATA_BUFSIZE );
		}
	};

	//add addstats cmd
	struct SNETPC_REQ_STATSUPCMD_FB : public NET_MSG_CHARACTER
	{
		EMSTATS				emStats;
		DWORD				wStatsPoint;
		DWORD				dwValue;

		SNETPC_REQ_STATSUPCMD_FB (void) :
		emStats(EMPOW),
			wStatsPoint(0),
			dwValue(0)
		{
			dwSize = sizeof(*this);
			nType = NET_MSG_GCTRL_REQ_STATSUPCMD_FB;
			MIN_STATIC_ASSERT( sizeof( SNETPC_REQ_STATSUPCMD_FB ) <= NET_DATA_BUFSIZE );
		}
	};

	struct SNETPC_REQ_CHAR_LOCK_CF : public NET_MSG_CHARACTER
	{
		EMCHARLOCK			emReq;
		bool				bUnlock;
		char szPassword[CHAR_SZNAME];
		char szPIN[CHAR_SZNAME];
		SNETPC_REQ_CHAR_LOCK_CF (EMCHARLOCK _emReq, bool _bUnlock = false) 
			: emReq ( _emReq )
			, bUnlock ( _bUnlock )
		{
			dwSize = sizeof(*this);
			nType = NET_MSG_REQ_CHAR_LOCK_CF;
			memset (szPassword, 0, sizeof(char) * CHAR_SZNAME);		
			memset (szPIN, 0, sizeof(char) * CHAR_SZNAME);		
			MIN_STATIC_ASSERT( sizeof( SNETPC_REQ_CHAR_LOCK_CF ) <= NET_DATA_BUFSIZE );
		}
	};

	struct SNETPC_REQ_CHAR_LOCK_FB : public NET_MSG_CHARACTER
	{
		EMCHARLOCK	emReq;
		EMCHARLOCK_FB		emFB;
		EMCHARLOCK_REASON	emReason;
		bool				bUnlock;
		bool				bReset;
		bool				bInventoryLock;
		bool				bLockerLock;
		SNETPC_REQ_CHAR_LOCK_FB (void) 
			: emReq ( EMCHARLOCK_INVENTORY_PASSWORD )
			, emFB ( EMCHARLOCK_FB_INVENTORY_PASSWORD_FAIL )
			, emReason ( EMCHARLOCK_REASON_COMMON )
			, bUnlock ( false )
			, bInventoryLock ( false )
			, bLockerLock ( false )
		{
			dwSize = sizeof(*this);
			nType = NET_MSG_REQ_CHAR_LOCK_FB;
			MIN_STATIC_ASSERT( sizeof( SNETPC_REQ_CHAR_LOCK_FB ) <= NET_DATA_BUFSIZE );
		}
	};

	struct SNET_GET_RANKINGS_FROMDB : public NET_MSG_CHARACTER
	{
		DWORD					dwCharID;
		char					szUID[USR_ID_LENGTH+1];

		SNET_GET_RANKINGS_FROMDB ()
			: dwCharID(0)
		{
			dwSize = sizeof ( SNET_GET_RANKINGS_FROMDB );
			nType  = NET_MSG_GCTRL_GET_RANKINGS_FROMDB;
			memset ( szUID, 0, sizeof(char)*(USR_ID_LENGTH+1) );
			GASSERT(dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_GET_RANKINGS_FROMDB_FB : public NET_MSG_CHARACTER
	{
		EMREQ_RANKING_FROMDB_FB emFB;
		RANKING						sRank;

		SNET_GET_RANKINGS_FROMDB_FB ()
			: emFB(EMREQ_RANKING_FROMDB_FB_RESET)
			//sRank ( RANKING() )
		{
			dwSize = sizeof ( SNET_GET_RANKINGS_FROMDB_FB );
			nType  = NET_MSG_GCTRL_GET_RANKINGS_FROMDB_FB;
			GASSERT(dwSize<=NET_DATA_BUFSIZE);
		}
	};
