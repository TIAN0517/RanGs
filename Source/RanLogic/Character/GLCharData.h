...
public:
	...
	*/
	...
	bool operator += ( const PVP_PERFORMANCE& rhs)
	{
		this->nKill		+= rhs.nKill;
		this->nDeath		+= rhs.nDeath;
		this->nRebirthTo	+= rhs.nRebirthTo;
		this->nDamageTo	+= rhs.nDamageTo;
		this->nHealTo		+= rhs.nHealTo;
		this->nContributionPoint += rhs.nContributionPoint;
...
struct CTF_STATISTICS_4_ONEPLAYER
{
		...
		,m_dwMarkVER(0)
	{
		...
		ZeroMemory(szClubName, sizeof(szClubName));
	}
...
struct SPLAYER_STREAK
{
	enum { VERSION = 0x0100, };

	int nPKSTREAK[PKCOMBO_SIZE];

	MSGPACK_DEFINE(nPKSTREAK[0],nPKSTREAK[1],nPKSTREAK[2],nPKSTREAK[3],nPKSTREAK[4],nPKSTREAK[5]
	,nPKSTREAK[6],nPKSTREAK[7]);

	SPLAYER_STREAK ()
	{
		SecureZeroMemory( nPKSTREAK, sizeof(nPKSTREAK) );
	}

	void DoIncrease ( int nComboCount );
...
struct SSKILLFACT
{
    ...
    struct STIME
    {
        ...
        STIME( const float _fAGE, bool bExpire = false )
            : IsEXPIREDTIME(bExpire)
            , fAGE(_fAGE)
			, fLIFE(_fAGE)
        {
        }

        STIME( const UINT _nEXPIREDTIME, bool bExpire = false )
            : IsEXPIREDTIME(bExpire)
...
struct SCHARDATA2 : public SCHARDATA
{
	...
	//ADDED BY EJSAYAAA RANKINGS
	RANKINGMAP				m_mapRankings;						//	구입한 아이템 목록.
	RANKINGMAP				m_mapRankingsPK;						//	구입한 아이템 목록.
	RANKINGMAP				m_mapRankingsRich;						//	구입한 아이템 목록.
	...
	DWORD		GetPKStreakDataSize();
	BOOL		GetPKStreakData( BYTE* pDest, const DWORD dwDestSize );
	BOOL SETPKSTREAK ( se::ByteStream &ByteStream );
	BOOL GETPKSTREAK ( se::ByteStream &ByteStream ) const;
	...
	BOOL SETRANKINGS ( VECRANKING &vecRANK , bool bPk = false , bool bLevel = false , bool bRich = false );
