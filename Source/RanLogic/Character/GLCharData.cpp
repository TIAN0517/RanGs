...
void SCHARDATA2::Assign ( SCHARDATA2 &sDATA )
{
	...
	m_PKStreakData	= sDATA.m_PKStreakData;
...
//#endif
	...
	m_mapRankings = sDATA.m_mapRankings;
	m_mapRankingsPK = sDATA.m_mapRankingsPK;
	m_mapRankingsRich = sDATA.m_mapRankingsRich;
...
BOOL SCHARDATA2::SETRANKINGS ( VECRANKING &vecRANK , bool bPk , bool bLevel ,  bool bRich )
{
	if ( bLevel )
	{
		m_mapRankings.clear();
		DWORD dwSIZE = static_cast<DWORD>(vecRANK.size());
		for ( DWORD i=0; i<dwSIZE; ++i )
		{
			const RANKING &sRANKING = vecRANK[i];
			m_mapRankings.insert ( std::make_pair(sRANKING.dwCharID,sRANKING) );
		}
	}
	...
	if ( bPk )
	{
		...
		{
			const RANKING &sRANKING = vecRANK[i];
			m_mapRankingsPK.insert ( std::make_pair(sRANKING.dwCharID,sRANKING) );
		}
	}
	...
	if ( bRich )
	{
		...
		{
			const RANKING &sRANKING = vecRANK[i];
			m_mapRankingsRich.insert ( std::make_pair(sRANKING.dwCharID,sRANKING) );
		}
	}
	return TRUE;
}
...
BOOL SCHARDATA2::SETPKSTREAK ( se::ByteStream &ByteStream )
{
	...
	GASSERT ( (dwSize==sizeof(SPLAYER_STREAK)) && (SPLAYER_STREAK::VERSION==dwVersion) );

	SPLAYER_STREAK PKStreakData;

	ByteStream.ReadBuffer ( (LPBYTE)&PKStreakData, sizeof(SPLAYER_STREAK) );

	m_PKStreakData = PKStreakData;

	return TRUE;
}

DWORD SCHARDATA2::GetPKStreakDataSize()
{
	DWORD		dwDataSize = 0;

	dwDataSize += sizeof( SPLAYER_STREAK::VERSION );
	dwDataSize += sizeof( DWORD );

	dwDataSize += sizeof( SPLAYER_STREAK );

	return dwDataSize;
}

BOOL SCHARDATA2::GetPKStreakData( BYTE* pDest, const DWORD dwDestSize )
{
	se::SerializeWritableData	data( pDest, dwDestSize );

	data << SPLAYER_STREAK::VERSION
		<< (DWORD)sizeof(SPLAYER_STREAK);

	data.Write( (LPBYTE)&m_PKStreakData, sizeof( SPLAYER_STREAK ) );

	return TRUE;
}

BOOL SCHARDATA2::GETPKSTREAK ( se::ByteStream &ByteStream ) const
{
	ByteStream.ClearBuffer ();

	ByteStream << SPLAYER_STREAK::VERSION;
	ByteStream << (DWORD)sizeof(SPLAYER_STREAK);

	ByteStream.WriteBuffer ( (LPBYTE)&m_PKStreakData, (DWORD)sizeof(SPLAYER_STREAK) );
...
void SPLAYER_STREAK::DoIncrease ( int nComboCount )
{
	int AddStreak = nComboCount-PKCOMBO_PKLIMIT;
	if ( AddStreak < 0 ) return;
	if ( AddStreak >= PKCOMBO_SIZE ) AddStreak = PKCOMBO_SIZE-1;
	nPKSTREAK[AddStreak]++;
}