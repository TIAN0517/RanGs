...
BOOL SCHARDATA2::LOADFILE(const std::string& FileName)
{
	...
	if ( GLUseFeatures::GetInstance().IsUsingTestServer() )
	{
		int nBeginIndex = 0;
		int nEndIndex = 0;
		...
		case GLCC_ACTOR_W:
			{
				nBeginIndex = EMSKILL_ACTOR_01;
				nEndIndex = EMSKILL_ACTOR_08;
			}
			break;
		};
		...
		for ( int i = nBeginIndex; i <= nEndIndex; i++ )
		{
			for ( int j = 0; j < wIndex; j++ )
			{
				...
				if ( pSkill )
				{
					...
					CharSkill.wLevel = pSkill->m_sBASIC.dwMAXLEVEL - 1;
					m_ExpSkills[CharSkill.sNativeID.dwID] = CharSkill;
				}
			}
		} 
