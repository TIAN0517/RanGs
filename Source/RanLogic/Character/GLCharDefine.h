...
struct CHARACTER_POINT
{
	...
	__int64 m_PointBattle; //! 환불불가 포인트
	...
	inline __int64 BattlePoint() const { return m_PointBattle; }
	inline void SetBattlePoint(__int64 Point) { m_PointBattle = Point; }
	inline void AddBattlePoint(__int64 Point) { m_PointBattle += Point; }
...
struct RANKING
{
	...
	{			
		...
		if ( bLevel ) if ( nChaNum > sRank.nChaNum ) return true;
...
typedef std::vector<RANKING>		VECRANKING;
typedef std::map<DWORD,RANKING>		RANKINGMAP;
typedef RANKINGMAP::iterator		RANKINGMAP_ITER;
typedef RANKINGMAP::const_iterator	RANKINGMAP_CITER;
