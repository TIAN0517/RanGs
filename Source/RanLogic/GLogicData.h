...
#define CTF_RANKERS_NUM_4_NOTIFY			20	//CTFÅëÇÕ UI¿¡ Ãâ·ÂµÇ´Â ·©Ä¿Á¤º¸¸¦ ÃÖ´ë ¸î¸í±îÁö Àü¼ÛÇÒ°ÍÀÎ°¡?
...
struct CTF_BATTLE_TIME
{
	...
	CTF_BATTLE_TIME(WORD _wBattleH, WORD _wBattleM)
		:wBattleH(_wBattleH)
		,wBattleM(_wBattleM)
	{
	}

	bool operator < (const CTF_BATTLE_TIME& ctfTime )
	{
		if ( wBattleH < ctfTime.wBattleH ) return true;

		if ( wBattleH == ctfTime.wBattleH && wBattleM < ctfTime.wBattleM ) return true;
...
typedef std::vector<CTF_BATTLE_TIME>	VCTF_BATTLE_TIME;
typedef VCTF_BATTLE_TIME::iterator		VCTF_BATTLE_TIME_ITER;

struct TYRANNY_SCHED
{  
	WORD	wStartDay;		//schedule day
	WORD	wStartDayOfWeek;		//schedule day of week
	WORD	wStartMonth;			//schedule month
	WORD	wStartYear;			//schedule month
	...
	{			
		if ( wStartYear < sData.wStartYear ) 
			return true;

		else if ( wStartYear == sData.wStartYear && wStartMonth < sData.wStartMonth ) 
			return true;

		else if ( wStartYear == sData.wStartYear && wStartMonth == sData.wStartMonth && wStartDayOfWeek < sData.wStartDayOfWeek ) 
			return true;

		else if ( wStartYear == sData.wStartYear && wStartMonth == sData.wStartMonth && wStartDayOfWeek == sData.wStartDayOfWeek &&
			wStartDay < sData.wStartDay ) 
			return true;

		else if ( wStartYear == sData.wStartYear && wStartMonth == sData.wStartMonth && wStartDayOfWeek == sData.wStartDayOfWeek && wStartDay == sData.wStartDay && 
			wStartHour < sData.wStartHour ) 
			return true;

		else if ( wStartYear == sData.wStartYear && wStartMonth == sData.wStartMonth && wStartDayOfWeek == sData.wStartDayOfWeek && wStartDay == sData.wStartDay && 
			wStartHour == sData.wStartHour && wStartMinute < sData.wStartMinute ) 
			return true;
