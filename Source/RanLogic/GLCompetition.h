...
namespace PVP
{
	...
	struct CTF_RANKER: public CTF_RANKER_BASIC
	{
		...
		{
			if ( rhs.dwClubID != CLUB_NULL )
			{
				PVP_PERFORMANCE lhsPF;
				lhsPF.nKill = this->pvpperf.nKill; 
				lhsPF.nDeath = this->pvpperf.nDeath; 
				lhsPF.nRebirthTo = this->pvpperf.nRebirthTo; 

				PVP_PERFORMANCE rhsPF;
				rhsPF.nKill = rhs.pvpperf.nKill; 
				rhsPF.nDeath = rhs.pvpperf.nDeath; 
				rhsPF.nRebirthTo = rhs.pvpperf.nRebirthTo; 
	...
	typedef std::map<DWORD,CTF_RANKER>			CTF_RANKER_MAP;
	typedef CTF_RANKER_MAP::iterator			CTF_RANKER_MAP_ITER;
	...
	struct SCTFRESULTDATA
	{
		...
		{
			if ( this->dwClubID != CLUB_NULL && rhs.dwClubID != CLUB_NULL )
			{
				PVP_PERFORMANCE lhsPF;
