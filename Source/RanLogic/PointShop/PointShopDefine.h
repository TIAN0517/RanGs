...
struct SellItem
{
    ...
    SellItem(WORD Mid, WORD Sid, __int64 _Price, WORD _wCurrency)
        : m_ItemId(Mid, Sid)
		, m_Price(_Price)
		, m_Currency(_wCurrency)
    {
    }

    SellItem(SNATIVEID _ItemId, __int64 _Price, WORD _wCurrency)
        : m_ItemId(_ItemId)
		, m_Price(_Price)
		, m_Currency(_wCurrency)
    {
	...
	inline WORD Currency() const { return m_Currency; }
};
...
struct SellItemClient : public SellItem
{
    ...
    SellItemClient(WORD Mid, WORD Sid, __int64 Price,
        EMITEM_TYPE Type, EMSUIT Suit, const std::string& ItemName, WORD _wCurrency)
        : SellItem(Mid, Sid, Price, _wCurrency)
        , m_Type(Type)
        , m_Suit(Suit)
        , m_ItemName(ItemName)
    {
...
enum EM_POINT_SHOP_CURRENCY
{
	POINT_SHOP_PREMIUM_POINT = 1,
	POINT_SHOP_BATTLE_POINT = 2,
};
